...
#ifdef USE_HDFS
#ifndef CABINDB_HDFS_FILE_C
#define CABINDB_HDFS_FILE_C
...
//
// This file defines an HDFS environment for cabindb. It uses the libhdfs
// api to access HDFS. All HDFS files created by one instance of cabindb
...
class HdfsReadableFile : virtual public SequentialFile,
 ...
 public:
  HdfsReadableFile(hdfsFS fileSys, const std::string& fname)
    ...
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile opening file %s\n",
                    filename_.c_str());
    hfile_ = hdfsOpenFile(fileSys_, filename_.c_str(), O_RDONLY, 0, 0, 0);
    CABIN_LOG_DEBUG(mylog,
                    "[hdfs] HdfsReadableFile opened file %s hfile_=0x%p\n",
                    filename_.c_str(), hfile_);
  }

  virtual ~HdfsReadableFile() {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile closing file %s\n",
                    filename_.c_str());
    hdfsCloseFile(fileSys_, hfile_);
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile closed file %s\n",
                    filename_.c_str());
    hfile_ = nullptr;
  }
  ...
  virtual Status Read(size_t n, Slice* result, char* scratch) {
    Status s;
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile reading %s %ld\n",
                    filename_.c_str(), n);
    ...
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile read %s\n",
                    filename_.c_str());
  ...
  virtual Status Read(uint64_t offset, size_t n, Slice* result,
                      char* scratch) const {
    Status s;
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile preading %s\n",
                    filename_.c_str());
    tSize bytes_read =
        hdfsPread(fileSys_, hfile_, offset, static_cast<void*>(scratch),
                  static_cast<tSize>(n));
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile pread %s\n",
                    filename_.c_str());
  ...
  virtual Status Skip(uint64_t n) {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile skip %s\n",
                    filename_.c_str());
 ...
 private:
  ...
  bool feof() {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile feof %s\n",
                    filename_.c_str());
  ...
  tOffset fileSize() {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsReadableFile fileSize %s\n",
                    filename_.c_str());
 ...
 public:
  HdfsWritableFile(hdfsFS fileSys, const std::string& fname,
      ...
      : WritableFile(options),
        ...
        filename_(fname),
        hfile_(nullptr) {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile opening %s\n",
                    filename_.c_str());
    hfile_ = hdfsOpenFile(fileSys_, filename_.c_str(), O_WRONLY, 0, 0, 0);
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile opened %s\n",
                    filename_.c_str());
    assert(hfile_ != nullptr);
  }
  virtual ~HdfsWritableFile() {
    if (hfile_ != nullptr) {
      CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile closing %s\n",
                      filename_.c_str());
      hdfsCloseFile(fileSys_, hfile_);
      CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile closed %s\n",
                      filename_.c_str());
      hfile_ = nullptr;
    }
  }
  ...
  virtual Status Append(const Slice& data) {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile Append %s\n",
                    filename_.c_str());
    const char* src = data.data();
    size_t left = data.size();
    size_t ret = hdfsWrite(fileSys_, hfile_, src, static_cast<tSize>(left));
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile Appended %s\n",
                    filename_.c_str());
    if (ret != left) {
      return IOError(filename_, errno);
    }
  ...
  virtual Status Sync() {
    Status s;
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile Sync %s\n",
                    filename_.c_str());
    ...
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsWritableFile Synced %s\n",
                    filename_.c_str());
    return Status::OK();
  }
...
class HdfsLogger : public Logger {
 private:
  ...
  Status HdfsCloseHelper() {
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsLogger closed %s\n",
                    file_->getName().c_str());
 ...
 public:
  HdfsLogger(HdfsWritableFile* f, uint64_t (*gettid)())
    ...
    CABIN_LOG_DEBUG(mylog, "[hdfs] HdfsLogger opened %s\n",
                    file_->getName().c_str());
  }
...
Status HdfsEnv::NewDirectory(const std::string& name,
  ...
  switch (value) {
    ...
    default:  // fail if the directory doesn't exist
      CABIN_LOG_FATAL(mylog, "NewDirectory hdfsExists call failed");
...
Status HdfsEnv::FileExists(const std::string& fname) {
  ...
  switch (value) {
    ...
    default:  // anything else should be an error
      CABIN_LOG_FATAL(mylog, "FileExists hdfsExists call failed");
...
Status HdfsEnv::GetChildren(const std::string& path,
  ...
  switch (value) {
    ...
    if (numEntries >= 0) {
      ...
      CABIN_LOG_FATAL(mylog, "hdfsListDirectory call failed with error ");
  ...
  case HDFS_DOESNT_EXIST:  // directory does not exist, exit
    return Status::NotFound();
  default:          // anything else should be an error
    CABIN_LOG_FATAL(mylog, "GetChildren hdfsExists call failed");
    throw HdfsFatalException("hdfsExists call failed with error " +
                             ToString(value) + ".\n");
  }
  return Status::OK();
...
Status HdfsEnv::CreateDirIfMissing(const std::string& name) {
  ...
  switch (value) {
    ...
    default:  // anything else should be an error
      CABIN_LOG_FATAL(mylog, "CreateDirIfMissing hdfsExists call failed");
      throw HdfsFatalException("hdfsExists call failed with error " +
                               ToString(value) + ".\n");
  }
};
