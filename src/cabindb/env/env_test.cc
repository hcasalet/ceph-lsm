...
#ifdef CABINDB_FALLOCATE_PRESENT
#include <errno.h>
#endif
...
#ifndef CABINDB_NO_DYNAMIC_EXTENSION
TEST_F(EnvPosixTest, LoadCabinDBLibrary) {
  ...
  status = env_->LoadLibrary("cabindb", "", &library);
  if (status.ok()) {  // If we have can find a cabindb shared library
    ASSERT_NE(nullptr, library.get());
    ASSERT_OK(library->LoadFunction("cabindb_create_default_env",
...
#endif  // !CABINDB_NO_DYNAMIC_EXTENSION

#if !defined(OS_WIN) && !defined(CABINDB_NO_DYNAMIC_EXTENSION)
TEST_F(EnvPosixTest, LoadCabinDBLibraryWithSearchPath) {
  std::shared_ptr<DynamicLibrary> library;
  std::function<void*(void*, const char*)> function;
  ...
  ASSERT_EQ(nullptr, library.get());
  Status status = env_->LoadLibrary("cabindb", "/tmp:./", &library);
  if (status.ok()) {
    ASSERT_NE(nullptr, library.get());
    ASSERT_OK(env_->LoadLibrary(library->Name(), "", &library));
  }
  ...
  status = env_->LoadLibrary("cabindb", "/tmp:" + cwd, &library);
  if (status.ok()) {
    ASSERT_NE(nullptr, library.get());
    ASSERT_OK(env_->LoadLibrary(library->Name(), "", &library));
  }
}
#endif  // !OS_WIN && !CABINDB_NO_DYNAMIC_EXTENSION
...
class IoctlFriendlyTmpdir {
 public:
  explicit IoctlFriendlyTmpdir() {
    ...
    const char *fmt = "%s/cabindb.XXXXXX";
...
#define rmdir _rmdir
    ...
    // If $TEST_IOCTL_FRIENDLY_TMPDIR/cabindb.XXXXXX fits, use
...
#endif  // CABINDB_FALLOCATE_PRESENT
...
TEST_P(EnvPosixTestWithParam, LogBufferTest) {
  ...
  CABIN_LOG_BUFFER(&log_buffer, "x%sx", bytes200);
  CABIN_LOG_BUFFER(&log_buffer, "x%sx", bytes600);
  CABIN_LOG_BUFFER(&log_buffer, "x%sx%sx%sx", bytes200, bytes200, bytes200);
  CABIN_LOG_BUFFER(&log_buffer, "x%sx%sx", bytes200, bytes600);
  CABIN_LOG_BUFFER(&log_buffer, "x%sx%sx", bytes600, bytes9000);

  CABIN_LOG_BUFFER(&log_buffer_debug, "x%sx", bytes200);
  test_logger.SetInfoLogLevel(DEBUG_LEVEL);
  CABIN_LOG_BUFFER(&log_buffer_debug, "x%sx%sx%sx", bytes600, bytes9000,
                   bytes200);
...
TEST_P(EnvPosixTestWithParam, LogBufferMaxSizeTest) {
  ...
  for (size_t max_log_size = 256; max_log_size <= 1024;
    ...
    test_logger.SetInfoLogLevel(InfoLogLevel::INFO_LEVEL);
    LogBuffer log_buffer(InfoLogLevel::INFO_LEVEL, &test_logger);
    CABIN_LOG_BUFFER_MAX_SZ(&log_buffer, max_log_size, "%s", bytes9000);
    log_buffer.FlushBufferToLog();
  }
}
...
TEST_F(EnvTest, LogvWithInfoLogLevel) {
  ...
  CABIN_LOG_HEADER(&logger, "%s", kSampleMessage.c_str());
  CABIN_LOG_DEBUG(&logger, "%s", kSampleMessage.c_str());
  CABIN_LOG_INFO(&logger, "%s", kSampleMessage.c_str());
  CABIN_LOG_WARN(&logger, "%s", kSampleMessage.c_str());
  CABIN_LOG_ERROR(&logger, "%s", kSampleMessage.c_str());
  CABIN_LOG_FATAL(&logger, "%s", kSampleMessage.c_str());
}
