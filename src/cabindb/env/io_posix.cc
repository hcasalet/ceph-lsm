...
#ifdef CABINDB_RANGESYNC_PRESENT
...
bool IsSyncFileRangeSupported(int fd) {
  ...
  // (`CABINDB_RANGESYNC_PRESENT`). If we are unsure, or if any of the checks
...
#endif  // CABINDB_RANGESYNC_PRESENT
...
PosixRandomAccessFile::PosixRandomAccessFile(
    ...
    const EnvOptions& options
#if defined(CABINDB_IOURING_PRESENT)
    ,
    ...
    for (size_t i = 0; i < this_reqs; i++) {
      ...
      if (cqe->res < 0) {
        ...
        if (bytes_read == req_wrap->iov.iov_len) {
          ...
          // comment
          // https://github.com/facebook/cabindb/pull/6441#issuecomment-589843435
...
PosixMmapReadableFile::~PosixMmapReadableFile() {
  ...
  if (ret != 0) {
    fprintf(stdout, "failed to munmap %p length %" CABINDB_PRIszt " \n",
...
IOStatus PosixMmapFile::UnmapCurrentRegion() {
  TEST_KILL_RANDOM("PosixMmapFile::UnmapCurrentRegion:0", cabindb_kill_odds);
  if (base_ != nullptr) {
...
IOStatus PosixMmapFile::Msync() {
  ...
  last_sync_ = dst_;
  TEST_KILL_RANDOM("PosixMmapFile::Msync:0", cabindb_kill_odds);
...
IOStatus PosixMmapFile::Append(const Slice& data, const IOOptions& /*opts*/,
  ...
  while (left > 0) {
    ...
    if (avail == 0) {
      ...
      TEST_KILL_RANDOM("PosixMmapFile::Append:0", cabindb_kill_odds);
    }
...
IOStatus PosixMmapFile::Allocate(uint64_t offset, uint64_t len,
  ...
  TEST_KILL_RANDOM("PosixMmapFile::Allocate:0", cabindb_kill_odds);
  int alloc_status = 0;
  if (allow_fallocate_) {
    alloc_status =
...
#if defined(CABINDB_FALLOCATE_PRESENT) && defined(FALLOC_FL_PUNCH_HOLE) && \
    !defined(TRAVIS)
...
IOStatus PosixWritableFile::Allocate(uint64_t offset, uint64_t len,
  ...
  assert(offset <= static_cast<uint64_t>(std::numeric_limits<off_t>::max()));
  assert(len <= static_cast<uint64_t>(std::numeric_limits<off_t>::max()));
  TEST_KILL_RANDOM("PosixWritableFile::Allocate:0", cabindb_kill_odds);
  IOSTATS_TIMER_GUARD(allocate_nanos);
  int alloc_status = 0;
  if (allow_fallocate_) {
    alloc_status =
