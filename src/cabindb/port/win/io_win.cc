...
size_t GetUniqueIdFromFile(HANDLE /*hFile*/, char* /*id*/,
  ...
  // https://github.com/facebook/cabindb/pull/5844.
  return 0;
}
...
Status WinMmapFile::Allocate(uint64_t offset, uint64_t len) {
  Status status;
  TEST_KILL_RANDOM("WinMmapFile::Allocate", cabindb_kill_odds);
...
Status WinWritableImpl::AllocateImpl(uint64_t offset, uint64_t len) {
  Status status;
  TEST_KILL_RANDOM("WinWritableFile::Allocate", cabindb_kill_odds);
