...
Status WinEnvIO::Truncate(const std::string& fname, size_t size) {
  Status s;
  int result = CABINDB_NAMESPACE::port::Truncate(fname, size);
  if (result != 0) {
...
Status WinEnvIO::NewLogger(const std::string& fname,
  ...
  HANDLE hFile = 0;
  {
    ...
    hFile = RX_CreateFile(
        ...
        FILE_SHARE_READ | FILE_SHARE_DELETE,  // In CabinDb log files are
...
void WinEnvThreads::StartThread(void(*function)(void* arg), void* arg) {
  ...
  state->arg = arg;
  try {
    CABINDB_NAMESPACE::port::WindowsThread th(&StartThreadWrapper, state.get());
    state.release();
