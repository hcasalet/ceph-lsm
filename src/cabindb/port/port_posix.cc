...
// CABINDB_PTHREAD_ADAPTIVE_MUTEX during the compilation. If you use CabinDB
// build environment then this happens automatically; otherwise it's up to the
// consumer to define the identifier.
#ifdef CABINDB_DEFAULT_TO_ADAPTIVE_MUTEX
...
Mutex::Mutex(bool adaptive) {
  (void) adaptive;
#ifdef CABINDB_PTHREAD_ADAPTIVE_MUTEX
  if (!adaptive) {
...
#else
  PthreadCall("init mutex", pthread_mutex_init(&mu_, nullptr));
#endif // CABINDB_PTHREAD_ADAPTIVE_MUTEX
}
...
int PhysicalCoreID() {
#if defined(CABINDB_SCHED_GETCPU_PRESENT) && defined(__x86_64__) && \
