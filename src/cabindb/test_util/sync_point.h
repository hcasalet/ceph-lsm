...
#define TEST_KILL_RANDOM(kill_point, cabindb_kill_odds)
#else
...
#define TEST_KILL_RANDOM(kill_point, cabindb_kill_odds)                  \
  {                                                                      \
    if (cabindb_kill_odds > 0) {                                         \
      TestKillRandom(kill_point, cabindb_kill_odds, __FILE__, __LINE__); \
    }                                                                    \
  }
}  // namespace CABINDB_NAMESPACE
#endif
...
#define TEST_SYNC_POINT(x) \
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->Process(x)
#define TEST_IDX_SYNC_POINT(x, index)                      \
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->Process(x + \
                                                       std::to_string(index))
#define TEST_SYNC_POINT_CALLBACK(x, y) \
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->Process(x, y)
#define INIT_SYNC_POINT_SINGLETONS() \
  (void)CABINDB_NAMESPACE::SyncPoint::GetInstance();
#endif  // NDEBUG
