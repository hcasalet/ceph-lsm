...
src/cabindb/utilities/debug.cc#include <chrono>
#include <ostream>
...
TEST_F(EnvLibradosTest, DBBasics) {
  ...
  DB* db;
  Options options;
  // Optimize CabinDB. This is the easiest way to get CabinDB to perform well
...
TEST_F(EnvLibradosTest, DBLoadKeysInRandomOrder) {
  ...
  ***********************************/
  ...
  CABINDB_NAMESPACE::Random64 r1(time(nullptr));
  ...
  ***********************************/
  ...
  CABINDB_NAMESPACE::Random64 r2(time(nullptr));
...
int main(int argc, char** argv) {
  fprintf(stderr, "SKIPPED as EnvMirror is not supported in CABINDB_LITE\n");
  return 0;
}
