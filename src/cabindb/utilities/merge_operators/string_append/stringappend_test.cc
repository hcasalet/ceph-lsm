...
/**
 * An persistent map : key -> (list of strings), using cabindb merge.
...
namespace {
// OpenDb opens a (possibly new) cabindb database with a StringAppendOperator
...
#ifndef CABINDB_LITE  // TtlDb is not supported in Lite
...
class StringLists {
 public:
  ...
  //Constructor: specifies the cabindb db
  /* implicit */
...
TEST_P(StringAppendOperatorTest, RandomMixGetAppend) {
  ...
  for (int q=0; q<kNumQueries; ++q) {
    ...
    if (query == APPEND_OP) {
      ...
      // Apply the cabindb test-harness Append defined above
      slists.Append(key, word);  //apply the cabindb append
...
TEST_P(StringAppendOperatorTest, PersistentFlushAndCompaction) {
  // Perform the following operations in limited scope
  {
    ...
    // Append, Flush, Get
    slists.Append("c", "asdasd");
    db->Flush(CABINDB_NAMESPACE::FlushOptions());
...
TEST_P(StringAppendOperatorTest, SimpleTestNullDelimiter) {
  ...
  // Check that the cabindb result string matches the desired string
