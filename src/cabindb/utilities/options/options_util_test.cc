...
TEST_F(OptionsUtilTest, SaveAndLoad) {
  ...
  ASSERT_OK(PersistCabinDBOptions(db_opt, cf_names, cf_opts, kFileName,
  ...
  ASSERT_OK(
      CabinDBOptionsParser::VerifyDBOptions(exact, db_opt, loaded_db_opt));
  test::RandomInitDBOptions(&db_opt, &rnd_);
  ASSERT_NOK(
      CabinDBOptionsParser::VerifyDBOptions(exact, db_opt, loaded_db_opt));
  ...
  for (size_t i = 0; i < kCFCount; ++i) {
    ...
    ASSERT_OK(CabinDBOptionsParser::VerifyTableFactory(
...
TEST_F(OptionsUtilTest, LoadLatestOptions) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(config_opts,
                                                  db->GetDBOptions(), db_opts));
  ASSERT_OK(handles[0]->GetDescriptor(&cf_desc));
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(config_opts, cf_desc.options,
                                                  cf_descs[0].options));
...
TEST_F(OptionsUtilTest, BadLatestOptions) {
  ...
  // Option
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0001", CABINDB_MAJOR - 1,
                   CABINDB_MINOR, "unknown_db_opt=true", "");
  ...
  // Option
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0002", CABINDB_MAJOR,
                   CABINDB_MINOR - 1, "", "unknown_cf_opt=true");
  s = LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs);
  ASSERT_NOK(s);
  ASSERT_TRUE(s.IsInvalidArgument());
  // Even though ignore_unknown_options=true, we still return an error...
  ...
  // Option
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0003", CABINDB_MAJOR,
                   CABINDB_MINOR - 1, "", "", "unknown_bbt_opt=true");
  s = LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs);
  ASSERT_NOK(s);
  ASSERT_TRUE(s.IsInvalidArgument());
  // Even though ignore_unknown_options=true, we still return an error...
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0004", CABINDB_MAJOR,
                   CABINDB_MINOR, "unknown_db_opt=true", "");
  s = LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs);
  ASSERT_NOK(s);
  ASSERT_TRUE(s.IsInvalidArgument());
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0005", CABINDB_MAJOR,
                   CABINDB_MINOR, "", "unknown_cf_opt=true");
  s = LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs);
  ASSERT_NOK(s);
  ASSERT_TRUE(s.IsInvalidArgument());
  // Even though ignore_unknown_options=true, we still return an error...
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0006", CABINDB_MAJOR,
                   CABINDB_MINOR, "create_if_missing=hello", "");
  s = LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs);
  ASSERT_NOK(s);
  ASSERT_TRUE(s.IsInvalidArgument());
  // Even though ignore_unknown_options=true, we still return an error...
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0007", CABINDB_MAJOR,
                   CABINDB_MINOR + 1, "create_if_missing=hello", "");
  ASSERT_NOK(LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs));
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0008", CABINDB_MAJOR,
                   CABINDB_MINOR + 1, "unknown_db_opt=true", "");
  ASSERT_NOK(LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs));
  ...
  WriteOptionsFile(options.env, dbname_, "OPTIONS-0009", CABINDB_MAJOR + 1,
                   CABINDB_MINOR, "", "unknown_cf_opt=true");
  ASSERT_NOK(LoadLatestOptions(config_opts, dbname_, &db_opts, &cf_descs));
  // Ignore the errors for future releases when ignore_unknown_options=true
  ASSERT_OK(LoadLatestOptions(ignore_opts, dbname_, &db_opts, &cf_descs));
...
int main(int /*argc*/, char** /*argv*/) {
  printf("Skipped in CabinDBLite as utilities are not supported.\n");
  return 0;
}
#endif  // !CABINDB_LITE
