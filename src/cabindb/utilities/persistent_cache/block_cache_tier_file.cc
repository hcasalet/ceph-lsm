...
bool RandomAccessCacheFile::OpenImpl(const bool enable_direct_reads) {
  ...
  CABIN_LOG_DEBUG(log_, "Opening cache file %s", Path().c_str());
...
bool WriteableCacheFile::Create(const bool /*enable_direct_writes*/,
  ...
  CABIN_LOG_DEBUG(log_, "Creating new cache %s (max size is %d B)",
  ...
  if (s.ok()) {
    CABIN_LOG_WARN(log_, "File %s already exists. %s", Path().c_str(),
                   s.ToString().c_str());
  }

  s = NewWritableCacheFile(env_, Path(), &file_);
  if (!s.ok()) {
    CABIN_LOG_WARN(log_, "Unable to create file %s. %s", Path().c_str(),
                   s.ToString().c_str());
    return false;
  }
...
bool WriteableCacheFile::Append(const Slice& key, const Slice& val, LBA* lba) {
  ...
  if (!ExpandBuffer(rec_size)) {
    ...
    CABIN_LOG_DEBUG(log_, "Error expanding buffers. size=%d", rec_size);
    return false;
  }
...
bool WriteableCacheFile::ExpandBuffer(const size_t size) {
  ...
  while (free < size) {
    ...
    if (!buf) {
      CABIN_LOG_DEBUG(log_, "Unable to allocate buffers");
      return false;
    }
