...
TEST_F(BackupableDBTest, FlushCompactDuringBackupCheckpoint) {
  ...
  for (const auto& sopt : kAllShareOptions) {
    ...
    SyncPoint::GetInstance()->EnableProcessing();
    CABINDB_NAMESPACE::port::Thread flush_thread{[this]() {
      TEST_SYNC_POINT(
          "BackupableDBTest::FlushCompactDuringBackupCheckpoint:Before");
...
TEST_F(BackupableDBTest, BackupOptions) {
  ...
  for (int i = 1; i < 5; i++) {
    ...
    db_.reset(OpenDB());
    ASSERT_OK(backup_engine_->CreateNewBackup(db_.get(), true));
    CABINDB_NAMESPACE::GetLatestOptionsFileName(db_->GetName(), options_.env,
                                                &name);
...
TEST_F(BackupableDBTest, SetOptionsBackupRaceCondition) {
  ...
  SyncPoint::GetInstance()->EnableProcessing();
  CABINDB_NAMESPACE::port::Thread setoptions_thread{[this]() {
    TEST_SYNC_POINT(
        "BackupableDBTest::SetOptionsBackupRaceCondition:BeforeSetOptions");
    DBImpl* dbi = static_cast<DBImpl*>(db_.get());
...
// see https://github.com/facebook/cabindb/issues/921
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr, "SKIPPED as BackupableDB is not supported in CABINDB_LITE\n");
  return 0;
}
