...
#include "include/cabindb/utilities/env_librados.h"
 ...
 * @details
 * Because cabindb only need a 2-level structure (dir/file), all input path will be shortened to dir/file format
 *  For example:
 *    b/c => dir '/b', file 'c'
 *    /a/b/c => dir '/b', file 'c'
 *
...
public:
   ...
   * @brief noop
   * @details
   *  cabindb has it's own caching capabilities that we should be able to use,
 ...
 * @details
 *  CabinDB has a 2-level structure, so all keys
 ...
 * @details [long description]
 *
 * It seems that LockFile is used for preventing other instance of CabinDB
 * from opening up the database at the same time. From CabinDB source code,
 ...
 *
 * When db recovery and db destroy, CabinDB will call LockFile
 *
 * @param fname [description]
