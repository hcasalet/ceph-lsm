...
#include "include/cabindb/metadata.h"
...
// Builds an openable snapshot of CabinDB
Status CheckpointImpl::CreateCheckpoint(const std::string& checkpoint_dir,
  ...
  if (s.ok()) {
    ...
    s = CreateCustomCheckpoint(
        ...
        [&](const std::string& src_dirname, const std::string& fname,
            FileType) {
          CABIN_LOG_INFO(db_options.info_log, "Hard Linking %s", fname.c_str());
        ...
        [&](const std::string& src_dirname, const std::string& fname,
          ...
          CABIN_LOG_INFO(db_options.info_log, "Copying %s", fname.c_str());
          return CopyFile(db_->GetFileSystem(), src_dirname + fname,
                          full_private_path + fname, size_limit_bytes,
                          db_options.use_fsync);
        } /* copy_file_cb */,
        [&](const std::string& fname, const std::string& contents, FileType) {
          CABIN_LOG_INFO(db_options.info_log, "Creating %s", fname.c_str());
  ...
  if (s.ok()) {
    ...
    CABIN_LOG_INFO(db_options.info_log, "Snapshot DONE. All is good");
    CABIN_LOG_INFO(db_options.info_log, "Snapshot sequence number: %" PRIu64,
                   sequence_number);
  } else {
    // clean all the files we might have created
    CABIN_LOG_INFO(db_options.info_log, "Snapshot failed -- %s",
                   s.ToString().c_str());
    CleanStagingDirectory(full_private_path, db_options.info_log.get());
  }
  return s;
...
Status CheckpointImpl::CreateCustomCheckpoint(
  ...
  *sequence_number = db_->GetLatestSequenceNumber();
  ...
  CABIN_LOG_INFO(db_options.info_log, "Number of log files %" CABINDB_PRIszt,
                 live_wal_files.size());
...
Status CheckpointImpl::ExportColumnFamily(
  ...
  CABIN_LOG_INFO(db_options.info_log,
  ...
  if (s.ok()) {
    s = db_->Flush(CABINDB_NAMESPACE::FlushOptions(), handle);
  }
  ...
  if (s.ok()) {
    ...
    if (s.ok()) {
      ...
      s = ExportFilesInMetaData(
          ...
          [&](const std::string& src_dirname, const std::string& fname) {
            CABIN_LOG_INFO(db_options.info_log, "[%s] HardLinking %s",
                           cf_name.c_str(), fname.c_str());
            return db_->GetEnv()->LinkFile(src_dirname + fname,
                                           tmp_export_dir + fname);
          } /*link_file_cb*/,
          [&](const std::string& src_dirname, const std::string& fname) {
            CABIN_LOG_INFO(db_options.info_log, "[%s] Copying %s",
                           cf_name.c_str(), fname.c_str());
            return CopyFile(db_->GetFileSystem(), src_dirname + fname,
  ...
  if (s.ok()) {
    ...
    CABIN_LOG_INFO(db_options.info_log, "[%s] Export succeeded.",
                   cf_name.c_str());
  } else {
    // Failure: Clean up all the files/directories created.
    CABIN_LOG_INFO(db_options.info_log, "[%s] Export failed. %s",
    ...
    for (const auto& subchild : subchildren) {
      ...
      if (!status.ok()) {
        CABIN_LOG_WARN(db_options.info_log, "Failed to cleanup file %s: %s",
                       subchild_path.c_str(), status.ToString().c_str());
      }
    }
    const auto status = db_->GetEnv()->DeleteDir(cleanup_dir);
    if (!status.ok()) {
      CABIN_LOG_WARN(db_options.info_log, "Failed to cleanup dir %s: %s",
...
Status CheckpointImpl::ExportFilesInMetaData(
  ...
  CABIN_LOG_INFO(db_options.info_log, "Number of table files %" CABINDB_PRIszt,
                 num_files);
