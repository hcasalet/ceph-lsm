...

  {
    ...
    for (double deletion_ratio : {-1.5, -1.0, 0.0, 1.5, 2.0}) {
      ...
      for (size_t i = 0; i < kTotalEntries; i++) {
        ...
        collector->AddUserKey("hello", "cabindb", kEntryDelete, 0, 0);
      ...
      for (int delta : {-1, 0, 1}) {
        ...
        for (size_t i = 0; i < kTotalEntries; i++) {
          if (i < actual_deletion_entries) {
            ...
            collector->AddUserKey("hello", "cabindb", kEntryPut, 0, 0);
          }
          ASSERT_FALSE(collector->NeedCompact());
        }
        collector->Finish(nullptr);
...
#else
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr, "SKIPPED as CabinDBLite does not include utilities.\n");
  return 0;
}
#endif  // !CABINDB_LITE
