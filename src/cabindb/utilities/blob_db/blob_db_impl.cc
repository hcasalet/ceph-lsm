...
Status BlobDBImpl::Open(std::vector<ColumnFamilyHandle*>* handles) {
  ...
  CABIN_LOG_INFO(db_options_.info_log, "Opening BlobDB...");

  if ((cf_options_.compaction_filter != nullptr ||
       cf_options_.compaction_filter_factory != nullptr)) {
    CABIN_LOG_INFO(db_options_.info_log,
  ...
  if (!s.ok()) {
    CABIN_LOG_ERROR(db_options_.info_log,
  ...
  CABIN_LOG_INFO(db_options_.info_log, "BlobDB pointer %p", this);
...
Status BlobDBImpl::GetAllBlobFiles(std::set<uint64_t>* file_numbers) {
  ...
  for (const auto& file_name : all_files) {
    ...
    if (success && type == kBlobFile) {
      ...
      CABIN_LOG_WARN(db_options_.info_log,
...
Status BlobDBImpl::OpenAllBlobFiles() {
  ...
  CABIN_LOG_INFO(db_options_.info_log,
                 "Found %" CABINDB_PRIszt " blob files: %s", blob_files_.size(),
                 blob_file_oss.str().c_str());
  CABIN_LOG_INFO(
      db_options_.info_log, "Found %" CABINDB_PRIszt " non-TTL blob files: %s",
      live_imm_non_ttl_blob_files_.size(), live_imm_oss.str().c_str());
  CABIN_LOG_INFO(db_options_.info_log,
                 "Found %" CABINDB_PRIszt
...
std::shared_ptr<BlobFile> BlobDBImpl::NewBlobFile(
  ...
  CABIN_LOG_DEBUG(db_options_.info_log, "New blob file created: %s reason='%s'",
...
Status BlobDBImpl::CreateWriterLocked(const std::shared_ptr<BlobFile>& bfile) {
  ...
  if (debug_level_ >= 2 && boffset) {
    CABIN_LOG_DEBUG(db_options_.info_log,
...
Status BlobDBImpl::PutBlobValue(const WriteOptions& /*options*/,
  ...
  if (value.size() < bdb_options_.min_blob_size) {
    ...
    if (s.ok()) {
      ...
      CABIN_LOG_ERROR(
          db_options_.info_log,
          "Failed to append blob to FILE: %s: KEY: %s VALSZ: %" CABINDB_PRIszt
  ...
  *compression_type = blob_file->GetCompressionType();
  ...
  if (!s.ok()) {
    CABIN_LOG_DEBUG(
        ...
        ", blob_size: %" PRIu64 ", key_size: %" CABINDB_PRIszt ", status: '%s'",
  ...
  if (blob_record.size() != record_size) {
    CABIN_LOG_DEBUG(
        db_options_.info_log,
        "Failed to read blob from blob file %" PRIu64 ", blob_offset: %" PRIu64
        ", blob_size: %" PRIu64 ", key_size: %" CABINDB_PRIszt
        ", read %" CABINDB_PRIszt " bytes, expected %" PRIu64 " bytes",
  ...
  if (!GetFixed32(&crc_slice, &crc_exp)) {
    CABIN_LOG_DEBUG(
        ...
        ", blob_size: %" PRIu64 ", key size: %" CABINDB_PRIszt ", status: '%s'",
        file_number, offset, size, key.size(), s.ToString().c_str());
...
std::pair<bool, int64_t> BlobDBImpl::SanityCheck(bool aborted) {
  ...
  CABIN_LOG_INFO(db_options_.info_log, "Starting Sanity Check");
  CABIN_LOG_INFO(db_options_.info_log, "Number of files %" CABINDB_PRIszt,
                 blob_files_.size());
  CABIN_LOG_INFO(db_options_.info_log, "Number of open files %" CABINDB_PRIszt,
  ...
  for (auto blob_file_pair : blob_files_) {
    ...
    CABIN_LOG_INFO(db_options_.info_log, "%s", buf);
  }
...
std::pair<bool, int64_t> BlobDBImpl::DeleteObsoleteFiles(bool aborted) {
  ...
  if (file_deleted) {
    ...
    if (!s.ok()) {
      CABIN_LOG_ERROR(db_options_.info_log, "Failed to sync dir %s: %s",
                      blob_dir_.c_str(), s.ToString().c_str());
    }
  }
