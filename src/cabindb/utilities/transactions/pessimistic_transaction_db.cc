...
Status PessimisticTransactionDB::Initialize(
  ...
  for (auto it = rtrxs.begin(); it != rtrxs.end(); ++it) {
    ...
    // MyCabin uses for auto-inc columns. It is safe to do so, since (i) if
    // there is a conflict between the keys of two transactions that must be
    // avoided, it is already avoided by the application, MyCabin, before the
    // restart (ii) application, MyCabin, guarntees to rollback/commit the
...
Status TransactionDB::Open(
  ...
  if (s.ok()) {
    CABIN_LOG_WARN(db->GetDBOptions().info_log,
