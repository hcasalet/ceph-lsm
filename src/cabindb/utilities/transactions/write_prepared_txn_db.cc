...
Status WritePreparedTxnDB::WriteInternal(const WriteOptions& write_options_orig,
  ...
  CABIN_LOG_DETAILS(db_impl_->immutable_db_options().info_log,
                    "CommitBatchInternal");
  if (batch->Count() == 0) {
  ...
  if (batch_cnt == 0) {  // not provided, then compute it
    ...
    CABIN_LOG_DETAILS(info_log_, "Duplicate key overhead: %" PRIu64 " batches",
...
void WritePreparedTxnDB::AddPrepared(uint64_t seq, bool locked) {
  CABIN_LOG_DETAILS(info_log_, "Txn %" PRIu64 " Preparing with max %" PRIu64,
...
void WritePreparedTxnDB::AddCommitted(uint64_t prepare_seq, uint64_t commit_seq,
                                      uint8_t loop_cnt) {
  CABIN_LOG_DETAILS(info_log_, "Txn %" PRIu64 " Committing with %" PRIu64,
...
void WritePreparedTxnDB::RemovePrepared(const uint64_t prepare_seq,
  ...
  CABIN_LOG_DETAILS(info_log_,
                    "RemovePrepared %" PRIu64 " cnt: %" CABINDB_PRIszt,
  ...
  for (size_t i = 0; i < batch_cnt; i++) {
    ...
    if (!was_empty) {
      ...
      if (it != delayed_prepared_commits_.end()) {
        CABIN_LOG_DETAILS(info_log_, "delayed_prepared_commits_.erase %" PRIu64,
                          prepare_seq + i);
...
void WritePreparedTxnDB::AdvanceSeqByOne() {
  ...
  char name[64];
  snprintf(name, 64, "txn%" CABINDB_PRIszt, hasher(std::this_thread::get_id()));
...
const std::vector<SequenceNumber> WritePreparedTxnDB::GetSnapshotListFromDB(
    SequenceNumber max) {
  CABIN_LOG_DETAILS(info_log_, "GetSnapshotListFromDB with max %" PRIu64, max);
...
void WritePreparedTxnDB::ReleaseSnapshotInternal(
  ...
  if (snap_seq <= max_evicted_seq_.load(std::memory_order_acquire)) {
    ...
    bool need_gc = false;
    {
      ...
      CABIN_LOG_WARN(info_log_, "old_commit_map_mutex_ overhead for %" PRIu64,
                     snap_seq);
...
void WritePreparedTxnDB::UpdateSnapshots(
  ...
  CABIN_LOG_DETAILS(info_log_, "UpdateSnapshots with version %" PRIu64,
                    version);
  ...
  size_t sync_i = 0;
#endif
  CABIN_LOG_DETAILS(info_log_, "snapshots_mutex_ overhead");
