...
#include "include/cabindb/utilities/debug.h"
...
TEST(PreparedHeap, Concurrent) {
  ...
  CABINDB_NAMESPACE::port::Thread t[t_cnt + 1];
  WritePreparedTxnDB::PreparedHeap heap;
  ...
  for (size_t n = 0; n < 100; n++) {
    last = 0;
    t[0] = CABINDB_NAMESPACE::port::Thread([&]() {
      Random rnd(1103);
    ...
    for (size_t i = 1; i <= t_cnt; i++) {
      t[i] =
          CABINDB_NAMESPACE::port::Thread([&heap, &prepared_mutex, &last, i]() {
            auto seq = i;
            do {
              std::this_thread::yield();
            } while (last.load() < seq);
 ...
 protected:
  ...
  void SnapshotConcurrentAccessTestInternal(
    ...
    CABINDB_NAMESPACE::SyncPoint::GetInstance()->EnableProcessing();
    {
      ...
      CABINDB_NAMESPACE::port::Thread t2(
...
TEST_P(WritePreparedTransactionTest, SmallestUnCommittedSeq) {
  ...
  port::Mutex mutex;
  Random rnd(1103);
  CABINDB_NAMESPACE::port::Thread commit_thread([&]() {
    for (int i = 0; i < cnt; i++) {
  ...
  CABINDB_NAMESPACE::port::Thread read_thread([&]() {
    while (1) {
      MutexLock l(&mutex);
      if (txns.empty()) {
        break;
...
TEST_P(SeqAdvanceConcurrentTest, SeqAdvanceConcurrent) {
  ...
  printf("Number of cases being tested is %" CABINDB_PRIszt "\n", max_n);
  for (size_t n = 0; n < max_n; n++, ReOpen()) {
    if (n % split_cnt_ != split_id_) continue;
    if (n % 1000 == 0) {
      printf("Tested %" CABINDB_PRIszt " cases so far\n", n);
    }
...
TEST_P(WritePreparedTransactionTest, AddPreparedBeforeMax) {
  ...
  CABINDB_NAMESPACE::port::Thread write_thread([&]() {
    txn_mutex_.Lock();
    ASSERT_OK(txn->Prepare());
    txn_mutex_.Unlock();
  });
...
TEST_P(WritePreparedTransactionTest, CommitOfDelayedPrepared) {
  ...
  for (const size_t commit_cache_bits : {0, 2, 3}) {
    for (const size_t sub_batch_cnt : {1, 2, 3}) {
      ...
      CABINDB_NAMESPACE::port::Thread callback_thread;
      ...
      auto callback = [&](void* param) {
        ...
        if (prep_seq == exp_prepare.load()) {  // only for write_thread
          ...
          // the write queue.
          callback_thread = CABINDB_NAMESPACE::port::Thread(snap_callback);
      ...
      CABINDB_NAMESPACE::port::Thread eviction_thread([&]() {
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr,
          "SKIPPED as Transactions are not supported in CABINDB_LITE\n");
  return 0;
}
