...
#if !defined(CABINDB_LITE) && !defined(__ARM_ARCH) && \
    !defined(CABINDB_VALGRIND_RUN)
...
int main(int /*argc*/, char** /*argv*/) {
  printf(
      "DistributedMutex is not supported in CABINDB_LITE, on ARM, or in "
      "valgrind_test runs\n");
  return 0;
}
#endif  // !CABINDB_LITE && !__ARM_ARCH && !CABINDB_VALGRIND_RUN
