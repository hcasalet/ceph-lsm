...
bool SstFileManagerImpl::EnoughRoomForCompaction(
  ...
  if (bg_error == Status::NoSpace() && CheckFreeSpace()) {
    ...
    if (free_space < needed_headroom + size_added_by_compaction) {
      ...
      CABIN_LOG_ERROR(logger_,
                      "free space [%" PRIu64
                      " bytes] is less than "
                      "needed headroom [%" CABINDB_PRIszt " bytes]\n",
...
void SstFileManagerImpl::ClearError() {
  while (true) {
    ...
    if (error_handler_list_.empty()) {
      CABIN_LOG_INFO(logger_, "Clearing error\n");
      bg_err_ = Status::OK();
      return;
    }
  }
...
SstFileManager* NewSstFileManager(Env* /*env*/,
  ...
  if (status) {
    *status =
        Status::NotSupported("SstFileManager is not supported in CABINDB_LITE");
  }
  return nullptr;
}
