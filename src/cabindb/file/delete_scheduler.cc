...
Status DeleteScheduler::DeleteFile(const std::string& file_path,
  ...
  CABIN_LOG_INFO(info_log_, "Mark file: %s as trash -- %s", trash_file.c_str(),
                 s.ToString().c_str());

  if (!s.ok()) {
    CABIN_LOG_ERROR(info_log_, "Failed to mark %s as trash -- %s",
                    file_path.c_str(), s.ToString().c_str());
    s = fs_->DeleteFile(file_path, IOOptions(), nullptr);
    if (s.ok()) {
      s = sst_file_manager_->OnDeleteFile(file_path);
      CABIN_LOG_INFO(info_log_, "Deleted file %s immediately",
                     trash_file.c_str());
      InstrumentedMutexLock l(&mu_);
      RecordTick(stats_.get(), FILES_DELETED_IMMEDIATELY);
    }
  ...
  while (true) {
    ...
    while (!queue_.empty() && !closing_) {
      if (current_delete_rate != rate_bytes_per_sec_.load()) {
        ...
        CABIN_LOG_INFO(info_log_, "rate_bytes_per_sec is changed to %" PRIi64,
                       current_delete_rate);
      }
...
Status DeleteScheduler::DeleteTrashFile(const std::string& path_in_trash,
  ...
  *is_complete = true;
  ...
  if (!s.ok()) {
    ...
    CABIN_LOG_ERROR(info_log_, "Failed to delete %s from trash -- %s",
