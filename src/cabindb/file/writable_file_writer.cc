...
IOStatus WritableFileWriter::Append(const Slice& data) {
  ...
  TEST_KILL_RANDOM("WritableFileWriter::Append:1", cabindb_kill_odds);
  if (s.ok()) {
    filesize_ += data.size();
  }
  return s;
...
IOStatus WritableFileWriter::Close() {
  ...
  TEST_KILL_RANDOM("WritableFileWriter::Close:0", cabindb_kill_odds);
  {
#ifndef CABINDB_LITE
    FileOperationInfo::StartTimePoint start_ts;
    if (ShouldNotifyListeners()) {
  ...
  TEST_KILL_RANDOM("WritableFileWriter::Close:1", cabindb_kill_odds);
...
IOStatus WritableFileWriter::Sync(bool use_fsync) {
  ...
  TEST_KILL_RANDOM("WritableFileWriter::Sync:0", cabindb_kill_odds);
  ...
  TEST_KILL_RANDOM("WritableFileWriter::Sync:1", cabindb_kill_odds);
  pending_sync_ = false;
  return IOStatus::OK();
}
...
IOStatus WritableFileWriter::WriteBuffered(const char* data, size_t size) {
  ...
  while (left > 0) {
    ...
    TEST_KILL_RANDOM("WritableFileWriter::WriteBuffered:0", cabindb_kill_odds);
