...
static const std::string kCabinDbTFileExt = "sst";
static const std::string kLevelDbTFileExt = "ldb";
static const std::string kCabinDBBlobFileExt = "blob";
...
std::string BlobFileName(uint64_t number) {
  assert(number > 0);
  return MakeFileName(number, kCabinDBBlobFileExt.c_str());
}

std::string BlobFileName(const std::string& blobdirname, uint64_t number) {
  assert(number > 0);
  return MakeFileName(blobdirname, number, kCabinDBBlobFileExt.c_str());
}
...
std::string MakeTableFileName(const std::string& path, uint64_t number) {
  return MakeFileName(path, number, kCabinDbTFileExt.c_str());
}

std::string MakeTableFileName(uint64_t number) {
  return MakeFileName(number, kCabinDbTFileExt.c_str());
}

std::string Cabin2LevelTableFileName(const std::string& fullname) {
  assert(fullname.size() > kCabinDbTFileExt.size() + 1);
  if (fullname.size() <= kCabinDbTFileExt.size() + 1) {
    return "";
  }
  return fullname.substr(0, fullname.size() - kCabinDbTFileExt.size()) +
         kLevelDbTFileExt;
}
...
bool ParseFileName(const std::string& fname, uint64_t* number,
  ...
  if (rest == "IDENTITY") {
    ...
    *type = is_temp_file ? kTempFile : kOptionsFile;
    ...
    } else if (suffix == Slice(kCabinDbTFileExt) ||
               suffix == Slice(kLevelDbTFileExt)) {
      *type = kTableFile;
    } else if (suffix == Slice(kCabinDBBlobFileExt)) {
      *type = kBlobFile;
...
IOStatus SetCurrentFile(FileSystem* fs, const std::string& dbname,
  ...
  if (s.ok()) {
    TEST_KILL_RANDOM("SetCurrentFile:0", cabindb_kill_odds * REDUCE_ODDS2);
    s = fs->RenameFile(tmp, CurrentFileName(dbname), IOOptions(), nullptr);
    TEST_KILL_RANDOM("SetCurrentFile:1", cabindb_kill_odds * REDUCE_ODDS2);
  }
  if (s.ok()) {
...
IOStatus SyncManifest(Env* env, const ImmutableDBOptions* db_options,
  ...
  TEST_KILL_RANDOM("SyncManifest:0", cabindb_kill_odds * REDUCE_ODDS2);
