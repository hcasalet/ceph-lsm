...
CABINDB_NAMESPACE::DbStressEnvWrapper* db_stress_env = nullptr;
#ifndef NDEBUG
// If non-null, injects read error at a rate specified by the
// read_fault_one_in flag
std::shared_ptr<CABINDB_NAMESPACE::FaultInjectionTestFS> fault_fs_guard;
#endif // NDEBUG
enum CABINDB_NAMESPACE::CompressionType compression_type_e =
    CABINDB_NAMESPACE::kSnappyCompression;
enum CABINDB_NAMESPACE::CompressionType bottommost_compression_type_e =
    CABINDB_NAMESPACE::kSnappyCompression;
enum CABINDB_NAMESPACE::ChecksumType checksum_type_e =
    CABINDB_NAMESPACE::kCRC32c;
...
void PrintKeyValue(int cf, uint64_t key, const char* value, size_t sz) {
  ...
  fprintf(stdout, "[CF %d] %s (%" PRIi64 ") == > (%" CABINDB_PRIszt ") %s\n",
