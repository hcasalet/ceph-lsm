...
void StressTest::TestGetProperty(ThreadState* thread) const {
  ...
  if (thread->rand.OneIn(100)) {
    if (db_->GetProperty("cabindb.invalid_property_name", &prop)) {
...
void StressTest::PrintEnv() const {
  fprintf(stdout, "CabinDB version           : %d.%d\n", kMajorVersion,
          kMinorVersion);
  ...
  if (!cabindb_kill_exclude_prefixes.empty()) {
    fprintf(stdout, "Skipping kill points prefixes:\n");
    for (auto& p : cabindb_kill_exclude_prefixes) {
      fprintf(stdout, "  %s\n", p.c_str());
    }
  }
...
void StressTest::Open() {
  ...
  if (FLAGS_options_file.empty()) {
    ...
    options_.compaction_style =
        static_cast<CABINDB_NAMESPACE::CompactionStyle>(FLAGS_compaction_style);
  ...
  switch (FLAGS_rep_factory) {
    ...
    default:
      fprintf(stderr,
              "CabindbLite only supports skip list mem table. Skip "
              "--rep_factory\n");
#endif  // CABINDB_LITE
  }
  ...
  if (FLAGS_ttl == -1) {
    ...
    if (FLAGS_test_secondary) {
      ...
      assert(s.ok());
#else
      fprintf(stderr, "Secondary is not supported in CabinDBLite\n");
      exit(1);
#endif
    }
...
#else
    fprintf(stderr, "TTL is not supported in CabinDBLite\n");
    exit(1);
#endif
  }
  if (!s.ok()) {
