...
#include "cabindb/utilities/checkpoint.h"
#include "cabindb/utilities/db_ttl.h"
#include "cabindb/utilities/debug.h"
...
extern CABINDB_NAMESPACE::DbStressEnvWrapper* db_stress_env;
#ifndef NDEBUG
namespace CABINDB_NAMESPACE {
class FaultInjectionTestFS;
}  // namespace CABINDB_NAMESPACE
extern std::shared_ptr<CABINDB_NAMESPACE::FaultInjectionTestFS> fault_fs_guard;
#endif

extern enum CABINDB_NAMESPACE::CompressionType compression_type_e;
extern enum CABINDB_NAMESPACE::CompressionType bottommost_compression_type_e;
extern enum CABINDB_NAMESPACE::ChecksumType checksum_type_e;
...
namespace CABINDB_NAMESPACE {
inline enum CABINDB_NAMESPACE::CompressionType StringToCompressionType(
    const char* ctype) {
  assert(ctype);

  CABINDB_NAMESPACE::CompressionType ret_compression_type;

  if (!strcasecmp(ctype, "disable")) {
    ret_compression_type = CABINDB_NAMESPACE::kDisableCompressionOption;
  } else if (!strcasecmp(ctype, "none")) {
    ret_compression_type = CABINDB_NAMESPACE::kNoCompression;
  } else if (!strcasecmp(ctype, "snappy")) {
    ret_compression_type = CABINDB_NAMESPACE::kSnappyCompression;
  } else if (!strcasecmp(ctype, "zlib")) {
    ret_compression_type = CABINDB_NAMESPACE::kZlibCompression;
  } else if (!strcasecmp(ctype, "bzip2")) {
    ret_compression_type = CABINDB_NAMESPACE::kBZip2Compression;
  } else if (!strcasecmp(ctype, "lz4")) {
    ret_compression_type = CABINDB_NAMESPACE::kLZ4Compression;
  } else if (!strcasecmp(ctype, "lz4hc")) {
    ret_compression_type = CABINDB_NAMESPACE::kLZ4HCCompression;
  } else if (!strcasecmp(ctype, "xpress")) {
    ret_compression_type = CABINDB_NAMESPACE::kXpressCompression;
  } else if (!strcasecmp(ctype, "zstd")) {
    ret_compression_type = CABINDB_NAMESPACE::kZSTD;
  } else {
    fprintf(stderr, "Cannot parse compression type '%s'\n", ctype);
    ret_compression_type =
        CABINDB_NAMESPACE::kSnappyCompression;  // default value
  }
  if (ret_compression_type != CABINDB_NAMESPACE::kDisableCompressionOption &&
...
inline enum CABINDB_NAMESPACE::ChecksumType StringToChecksumType(
    const char* ctype) {
  assert(ctype);
  auto iter = CABINDB_NAMESPACE::checksum_type_string_map.find(ctype);
  if (iter != CABINDB_NAMESPACE::checksum_type_string_map.end()) {
    return iter->second;
  }
  fprintf(stderr, "Cannot parse checksum type '%s'\n", ctype);
  return CABINDB_NAMESPACE::kCRC32c;
}

inline std::string ChecksumTypeToString(CABINDB_NAMESPACE::ChecksumType ctype) {
  auto iter = std::find_if(
      CABINDB_NAMESPACE::checksum_type_string_map.begin(),
      CABINDB_NAMESPACE::checksum_type_string_map.end(),
      [&](const std::pair<std::string, CABINDB_NAMESPACE::ChecksumType>&
              name_and_enum_val) { return name_and_enum_val.second == ctype; });
  assert(iter != CABINDB_NAMESPACE::checksum_type_string_map.end());
  return iter->first;
}
...
#endif
inline bool GetNextPrefix(const CABINDB_NAMESPACE::Slice& src, std::string* v) {
