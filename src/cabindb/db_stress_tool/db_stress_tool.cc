...
static std::shared_ptr<CABINDB_NAMESPACE::DbStressEnvWrapper> env_wrapper_guard;
...
int db_stress_tool(int argc, char** argv) {
  ...
  if (FLAGS_statistics) {
    ...
    if (FLAGS_test_secondary) {
      dbstats_secondaries = CABINDB_NAMESPACE::CreateDBStatistics();
    }
  }
  ...
  if (!FLAGS_hdfs.empty()) {
    raw_env = new CABINDB_NAMESPACE::HdfsEnv(FLAGS_hdfs);
  ...
  cabindb_kill_odds = FLAGS_kill_random_test;
  cabindb_kill_exclude_prefixes = SplitString(FLAGS_kill_exclude_prefixes);
  ...
  std::unique_ptr<CABINDB_NAMESPACE::StressTest> stress;
