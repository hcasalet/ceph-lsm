...
#ifdef CABINDB_PRINT_EVENTS_TO_STDOUT
...
void EventLogger::Log(Logger* logger, const JSONWriter& jwriter) {
  ...
  printf("%s\n", jwriter.Get().c_str());
#else
  CABINDB_NAMESPACE::Log(logger, "%s %s", Prefix(), jwriter.Get().c_str());
#endif
}
...
#else
  assert(log_buffer);
  CABINDB_NAMESPACE::LogToBuffer(log_buffer, max_log_size, "%s %s", Prefix(),
                                 jwriter.Get().c_str());
#endif
}
