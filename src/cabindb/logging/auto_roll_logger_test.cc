...
// call CABIN_LOG_INFO(logger, log_message) directly.
namespace {
void LogMessage(Logger* logger, const char* message) {
  CABIN_LOG_INFO(logger, "%s", message);
}
...
TEST_F(AutoRollLoggerTest, OpenLogFilesMultipleTimesWithOptionLog_max_size) {
  ...
  // when cabindb is restarted, a new empty log file will be created.
  InitTestDb();
  // WORKAROUND:
...
TEST_F(AutoRollLoggerTest, LogFlushWhileRolling) {
  ...
  AutoRollLogger* auto_roll_logger =
      dynamic_cast<AutoRollLogger*>(logger.get());
  ASSERT_TRUE(auto_roll_logger);
  CABINDB_NAMESPACE::port::Thread flush_thread;
  ...
  for (int test_num = 0; test_num < 2; test_num++) {
    ...
    if (test_num == 0) {
      ...
      for (size_t i = 0; i < MAX_HEADERS; i++) {
        Header(&logger, "%s %" CABINDB_PRIszt, HEADER_STR.c_str(), i);
      }
    } else if (test_num == 1) {
      // HEADER_LEVEL should make this behave like calling Header()
      for (size_t i = 0; i < MAX_HEADERS; i++) {
        CABIN_LOG_HEADER(&logger, "%s %" CABINDB_PRIszt, HEADER_STR.c_str(), i);
      }
    }
...
TEST_F(AutoRollLoggerTest, LogFileExistence) {
  ...
  ASSERT_OK(CABINDB_NAMESPACE::DB::Open(options, kTestDir, &db));
  ASSERT_OK(default_env->FileExists(kLogFile));
  delete db;
}
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr,
          "SKIPPED as AutoRollLogger is not supported in CABINDB_LITE\n");
  return 0;
}
