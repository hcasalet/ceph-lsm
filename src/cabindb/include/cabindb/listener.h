...
enum class CompactionReason : int {
  ...
  kTtl,
  // According to the comments in flush_job.cc, CabinDB treats flush as
...
struct FlushJobInfo {
  ...
  int job_id;
  // If true, then cabindb is currently slowing-down all writes to prevent
  ...
  // If true, then cabindb is currently blocking any writes to prevent
...
class EventListener {
 public:
  // A callback function to CabinDB which will be called whenever a
  // registered CabinDB flushes a file.  The default implementation is
  // no-op.
  //
  ...
  // CabinDB starts to flush memtables.  The default implementation is
  // no-op.
  //
  // Note that the this function must be implemented in a way such that
  // it should not run for an extended period of time before the function
