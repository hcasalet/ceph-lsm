...
struct ColumnFamilyOptions : public AdvancedColumnFamilyOptions {
  ...
  ColumnFamilyOptions* OldDefaults(int cabindb_major_version = 4,
                                   int cabindb_minor_version = 6);

  // Some functions that make it easier to optimize CabinDB
...
enum class WALRecoveryMode : char {
  ...
  // back even after a crash-recovery. In this recovery mode, CabinDB guarantees
  ...
  // guarantee in face of power-loss crash-recovery), CabinDB offers various
...
struct DBOptions {
  ...
  DBOptions* OldDefaults(int cabindb_major_version = 4,
                         int cabindb_minor_version = 6);
...
#ifndef CABINDB_LITE
  // By default, CabinDB uses only one background thread for flush and
  // compaction. Calling this function will set it up such that total of
  // `total_threads` is used. Good value for `total_threads` is the number of
  // cores. You almost definitely want to call this function if your system is
  // bottlenecked by CabinDB.
  ...
  // If true, CabinDB will aggressively check consistency of the data.
  ...
  // NOT SUPPORTED ANYMORE: CabinDB automatically decides this based on the
  ...
  // Default: 0 (disabled)
  // Not supported in CABINDB_LITE mode!
  ...
  // if not zero, dump cabindb.stats to LOG every stats_dump_period_sec
  //
  // Default: 600 (10 min)
  //
  // Dynamically changeable through SetDBOptions() API.
  ...
  // if not zero, dump cabindb.stats to CabinDB every stats_persist_period_sec
  // Default: 600
  unsigned int stats_persist_period_sec = 600;

  // If true, automatically persist stats to a hidden column family (column
  // family name: ___cabindb_stats_history___) every
  ...
  // running CabinDB on spinning disks, you should set this to at least 2MB.
  // That way CabinDB's compaction is doing sequential instead of random reads.
  //
  ...
  // when specific CabinDB event happens.
  ...
  // CabinDB may decide to slow down more if the compaction still
  // gets behind further.
  ...
  // unordered_write.
  //
  // By default, i.e., when it is false, cabindb does not advance the sequence
  ...
  // set properly) increasing this value is likely to increase CabinDB
  ...
  // If true, then print malloc stats together with cabindb.stats
  // when printing to LOG.
  // DEFAULT: false
  bool dump_malloc_stats = false;

  // By default CabinDB replay WAL logs and flush them on DB open, which may
  // create very small SST files. If this option is enabled, CabinDB will try
  ...
  // By default CabinDB will flush all memtables on DB close if there are
  ...
  // If true, CabinDB supports flushing multiple column families and committing
  ...
  // For auto-triggered flush, CabinDB atomically flushes ALL column families.
  //
  ...
  // By default, CabinDB recovery fails if any table file referenced in
...
struct Options : public DBOptions, public ColumnFamilyOptions {
  ...
  // The function recovers options to the option as in version 4.6.
  Options* OldDefaults(int cabindb_major_version = 4,
                       int cabindb_minor_version = 6);
...
struct ReadOptions {
  ...
  // CabinDB does auto-readahead for iterators on noticing more than two reads
  ...
  // When true, by default use total_order_seek = true, and CabinDB can
  // selectively enable prefix seek mode if won't generate a different result
  // from total_order_seek, based on seek key, and iterator upper bound.
  // Not suppported in CABINDB_LITE mode, in the way that even with value true
  ...
  // Iterator's property "cabindb.iterator.is-key-pinned" is guaranteed to
  // return 1.
  // Default: false
  bool pin_data;
...
struct WriteOptions {
  ...
  // slowed down. The slowdown value is determined by CabinDB to guarantee
  ...
  // all write operations must be associated with timestamp because CabinDB, as
...
struct CompactionOptions {
  ...
  // Default: snappy
  // If set to `kDisableCompressionOption`, CabinDB will choose compression type
...
struct IngestExternalFileOptions {
  ...
  // CabinDB writes a global_seqno to a given offset within ingested SST files,
  // and new versions of CabinDB do not. If you ingest an external SST using
  // new version of CabinDB and would like to be able to downgrade to an
  // older version of CabinDB, you should set 'write_global_seqno' to true. If
  // your service is just starting to use the new CabinDB, we recommend that
  ...
  // When verify_checksums_before_ingest = true, CabinDB uses default
