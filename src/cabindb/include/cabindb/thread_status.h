...
// cabindb-related thread.  Such run-time status can be obtained via
// GetThreadList() API.
//
...
#if !defined(CABINDB_LITE) && !defined(NCABINDB_THREAD_STATUS) && \
    defined(CABINDB_SUPPORT_THREAD_LOCAL)
#define CABINDB_USING_THREAD_STATUS
#endif
...
// CABINDB_NAMESPACE::GetThreadList().
struct ThreadStatus {
  // The type of a thread.
  enum ThreadType : int {
    HIGH_PRIORITY = 0,  // CabinDB BG thread in high-pri thread pool
    LOW_PRIORITY,       // CabinDB BG thread in low-pri thread pool
    USER,               // User thread (Non-CabinDB BG thread)
    BOTTOM_PRIORITY,    // CabinDB BG thread in bottom-pri thread pool
    NUM_THREAD_TYPES
  };
