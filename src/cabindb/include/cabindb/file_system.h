...
//
// A FileSystem is an interface used by the cabindb implementation to access
...
// implementation to CabinDB in case of an IO error
struct IODebugContext {
  // file_path to be filled in by CabinDB in case of an error
...
// CabinDB and storage systems, such as Posix filesystems,
...
class FileSystem {
 public:
  ...
  //
  // The result of Default() belongs to cabindb and must never be deleted.
...
class FSRandomAccessFile {
 public:
  ...
  // If it's not implemented (default: `NotSupported`), CabinDB will create
...
class FSWritableFile {
 public:
  ...
  // the future. Currently, CabinDB does not use this API.
  virtual IOStatus Append(const Slice& data, const IOOptions& options,
...
// CABINDB_NAMESPACE::FSSequentialFileWrapper {
//  public:
//   MySequentialFileWrapper(CABINDB_NAMESPACE::FSSequentialFile* target):
//     CABINDB_NAMESPACE::FSSequentialFileWrapper(target) {}
...
//     return CABINDB_NAMESPACE::FSSequentialFileWrapper::Read(n, options,
//     result,
//                                                 scratch, dbg);
//   }
...
//     cabindb class. Unless you actually want to override the behavior.
//     (And unless cabindb people forgot to update the *Wrapper class.)
