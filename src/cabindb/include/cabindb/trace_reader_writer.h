...
// By default, CabinDB provides a way to capture the traces to a file using the
...
// TraceWriter allows exporting CabinDB traces to any system, one operation at
// a time.
class TraceWriter {
 public:
  TraceWriter() {}
...
// TraceReader allows reading CabinDB traces from any system, one operation at
// a time. A CabinDB Replayer could depend on this to replay opertions.
class TraceReader {
 public:
  TraceReader() {}
  virtual ~TraceReader() {}
