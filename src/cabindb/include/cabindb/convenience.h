...
struct ConfigOptions {
  // This enum defines the CabinDB options sanity level.
  enum SanityLevel : unsigned char {
    kSanityLevelNone = 0x01,  // Performs no sanity check at all.
    // Performs minimum check to ensure the CabinDB instance can be
...
// The following set of functions provide a way to construct CabinDB Options
// from a string or a string-to-string map.  Here're the general rule of
// setting option values from strings by type.  Some CabinDB types are also
// supported in these APIs.  Please refer to the comment of the function itself
// to find more information about how to config those CabinDB types.
//
// * Strings:
