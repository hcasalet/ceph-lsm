...
struct CompressionOptions {
  // CabinDB's generic default compression level. Internally it'll be translated
...
struct AdvancedColumnFamilyOptions {
  ...
  // usage to 52MB which is below the limit, CabinDB will stop trimming.
  //
  ...
  // In this case. (In this case, the snapshot-semantics of the cabindb
  ...
  // If true, CabinDB will pick target size of each level dynamically.
  ...
  // In debug mode, CabinDB runs consistency checks on the LSM every time the
  ...
  // 0 means disabling.
  // UINT64_MAX - 1 (0xfffffffffffffffe) is special flag to allow CabinDB to
  // pick default.
  //
  ...
  // UINT64_MAX - 1 (i.e 0xfffffffffffffffe): Let CabinDB control this feature
  //     as needed. For now, CabinDB will change this value to 30 days
  ...
  //
  // Default: UINT64_MAX - 1 (allow CabinDB to auto-tune)
  //
  // Dynamically changeable through SetOptions() API
