...
#define CABINDB_DEPRECATED_FUNC __attribute__((__deprecated__))
#elif _WIN32
#define CABINDB_DEPRECATED_FUNC __declspec(deprecated)
#endif
...
static const int kMajorVersion = __CABINDB_MAJOR__;
static const int kMinorVersion = __CABINDB_MINOR__;
...
class DB {
 public:
  ...
  // will happen.
  //
  // Not supported in CABINDB_LITE, in which case the function will
  ...
  // in CABINDB_NAMESPACE::kDefaultColumnFamilyName
  //
  // Not supported in CABINDB_LITE, in which case the function will
  // return Status::NotSupported.
  ...
  struct Properties {
    //  "cabindb.num-files-at-level<N>" - returns string containing the number
    //      of files at level <N>, where <N> is an ASCII representation of a
    //      level number (e.g., "0").
    static const std::string kNumFilesAtLevelPrefix;

    //  "cabindb.compression-ratio-at-level<N>" - returns string containing the
    ...
    //  "cabindb.stats" - returns a multi-line string containing the data
    //      described by kCFStats followed by the data described by kDBStats.
    static const std::string kStats;

    //  "cabindb.sstables" - returns a multi-line string summarizing current
    //      SST files.
    static const std::string kSSTables;

    //  "cabindb.cfstats" - Both of "cabindb.cfstats-no-file-histogram" and
    //      "cabindb.cf-file-histogram" together. See below for description
    //      of the two.
    static const std::string kCFStats;

    //  "cabindb.cfstats-no-file-histogram" - returns a multi-line string with
    ...
    //  "cabindb.cf-file-histogram" - print out how many file reads to every
    //      level, as well as the histogram of latency of single requests.
    static const std::string kCFFileHistogram;

    //  "cabindb.dbstats" - returns a multi-line string with general database
    //      stats, both cumulative (over the db's lifetime) and interval (since
    //      the last retrieval of kDBStats).
    static const std::string kDBStats;

    //  "cabindb.levelstats" - returns multi-line string containing the number
    //      of files per level and total size of each level (MB).
    static const std::string kLevelStats;

    //  "cabindb.num-immutable-mem-table" - returns number of immutable
    //      memtables that have not yet been flushed.
    static const std::string kNumImmutableMemTable;

    //  "cabindb.num-immutable-mem-table-flushed" - returns number of immutable
    //      memtables that have already been flushed.
    static const std::string kNumImmutableMemTableFlushed;

    //  "cabindb.mem-table-flush-pending" - returns 1 if a memtable flush is
    //      pending; otherwise, returns 0.
    static const std::string kMemTableFlushPending;

    //  "cabindb.num-running-flushes" - returns the number of currently running
    //      flushes.
    static const std::string kNumRunningFlushes;

    //  "cabindb.compaction-pending" - returns 1 if at least one compaction is
    //      pending; otherwise, returns 0.
    static const std::string kCompactionPending;

    //  "cabindb.num-running-compactions" - returns the number of currently
    //      running compactions.
    static const std::string kNumRunningCompactions;

    //  "cabindb.background-errors" - returns accumulated number of background
    //      errors.
    static const std::string kBackgroundErrors;

    //  "cabindb.cur-size-active-mem-table" - returns approximate size of active
    //      memtable (bytes).
    static const std::string kCurSizeActiveMemTable;

    //  "cabindb.cur-size-all-mem-tables" - returns approximate size of active
    //      and unflushed immutable memtables (bytes).
    static const std::string kCurSizeAllMemTables;

    //  "cabindb.size-all-mem-tables" - returns approximate size of active,
    //      unflushed immutable, and pinned immutable memtables (bytes).
    static const std::string kSizeAllMemTables;

    //  "cabindb.num-entries-active-mem-table" - returns total number of entries
    //      in the active memtable.
    static const std::string kNumEntriesActiveMemTable;

    //  "cabindb.num-entries-imm-mem-tables" - returns total number of entries
    //      in the unflushed immutable memtables.
    static const std::string kNumEntriesImmMemTables;

    //  "cabindb.num-deletes-active-mem-table" - returns total number of delete
    //      entries in the active memtable.
    static const std::string kNumDeletesActiveMemTable;

    //  "cabindb.num-deletes-imm-mem-tables" - returns total number of delete
    //      entries in the unflushed immutable memtables.
    static const std::string kNumDeletesImmMemTables;

    //  "cabindb.estimate-num-keys" - returns estimated number of total keys in
    //      the active and unflushed immutable memtables and storage.
    static const std::string kEstimateNumKeys;

    //  "cabindb.estimate-table-readers-mem" - returns estimated memory used for
    ...
    //  "cabindb.is-file-deletions-enabled" - returns 0 if deletion of obsolete
    //      files is enabled; otherwise, returns a non-zero number.
    static const std::string kIsFileDeletionsEnabled;

    //  "cabindb.num-snapshots" - returns number of unreleased snapshots of the
    //      database.
    static const std::string kNumSnapshots;

    //  "cabindb.oldest-snapshot-time" - returns number representing unix
    //      timestamp of oldest unreleased snapshot.
    static const std::string kOldestSnapshotTime;

    //  "cabindb.oldest-snapshot-sequence" - returns number representing
    //      sequence number of oldest unreleased snapshot.
    static const std::string kOldestSnapshotSequence;

    //  "cabindb.num-live-versions" - returns number of live versions. `Version`
    //      is an internal data structure. See version_set.h for details. More
    //      live versions often mean more SST files are held from being deleted,
    //      by iterators or unfinished compactions.
    static const std::string kNumLiveVersions;

    //  "cabindb.current-super-version-number" - returns number of current LSM
    ...
    //  "cabindb.estimate-live-data-size" - returns an estimate of the amount of
    //      live data in bytes.
    static const std::string kEstimateLiveDataSize;

    //  "cabindb.min-log-number-to-keep" - return the minimum log number of the
    //      log files that should be kept.
    static const std::string kMinLogNumberToKeep;

    //  "cabindb.min-obsolete-sst-number-to-keep" - return the minimum file
    ...
    //  "cabindb.total-sst-files-size" - returns total size (bytes) of all SST
    //      files.
    //  WARNING: may slow down online queries if there are too many files.
    static const std::string kTotalSstFilesSize;

    //  "cabindb.live-sst-files-size" - returns total size (bytes) of all SST
    //      files belong to the latest LSM tree.
    static const std::string kLiveSstFilesSize;

    //  "cabindb.base-level" - returns number of level to which L0 data will be
    //      compacted.
    static const std::string kBaseLevel;

    //  "cabindb.estimate-pending-compaction-bytes" - returns estimated total
    ...
    //  "cabindb.aggregated-table-properties" - returns a string representation
    //      of the aggregated table properties of the target column family.
    static const std::string kAggregatedTableProperties;

    //  "cabindb.aggregated-table-properties-at-level<N>", same as the previous
    ...
    //  "cabindb.actual-delayed-write-rate" - returns the current actual delayed
    //      write rate. 0 means no delay.
    static const std::string kActualDelayedWriteRate;

    //  "cabindb.is-write-stopped" - Return 1 if write has been stopped.
    static const std::string kIsWriteStopped;

    //  "cabindb.estimate-oldest-key-time" - returns an estimation of
    ...
    //  "cabindb.block-cache-capacity" - returns block cache capacity.
    static const std::string kBlockCacheCapacity;

    //  "cabindb.block-cache-usage" - returns the memory size for the entries
    //      residing in block cache.
    static const std::string kBlockCacheUsage;

    // "cabindb.block-cache-pinned-usage" - returns the memory size for the
    //      entries being pinned.
    static const std::string kBlockCachePinnedUsage;

    // "cabindb.options-statistics" - returns multi-line string
  ...
  // properties:
  //  "cabindb.num-immutable-mem-table"
  //  "cabindb.mem-table-flush-pending"
  //  "cabindb.compaction-pending"
  //  "cabindb.background-errors"
  //  "cabindb.cur-size-active-mem-table"
  //  "cabindb.cur-size-all-mem-tables"
  //  "cabindb.size-all-mem-tables"
  //  "cabindb.num-entries-active-mem-table"
  //  "cabindb.num-entries-imm-mem-tables"
  //  "cabindb.num-deletes-active-mem-table"
  //  "cabindb.num-deletes-imm-mem-tables"
  //  "cabindb.estimate-num-keys"
  //  "cabindb.estimate-table-readers-mem"
  //  "cabindb.is-file-deletions-enabled"
  //  "cabindb.num-snapshots"
  //  "cabindb.oldest-snapshot-time"
  //  "cabindb.num-live-versions"
  //  "cabindb.current-super-version-number"
  //  "cabindb.estimate-live-data-size"
  //  "cabindb.min-log-number-to-keep"
  //  "cabindb.min-obsolete-sst-number-to-keep"
  //  "cabindb.total-sst-files-size"
  //  "cabindb.live-sst-files-size"
  //  "cabindb.base-level"
  //  "cabindb.estimate-pending-compaction-bytes"
  //  "cabindb.num-running-compactions"
  //  "cabindb.num-running-flushes"
  //  "cabindb.actual-delayed-write-rate"
  //  "cabindb.is-write-stopped"
  //  "cabindb.estimate-oldest-key-time"
  //  "cabindb.block-cache-capacity"
  //  "cabindb.block-cache-usage"
  //  "cabindb.block-cache-pinned-usage"
  ...
  CABINDB_DEPRECATED_FUNC virtual void GetApproximateSizes(
  ...
  CABINDB_DEPRECATED_FUNC virtual Status CompactRange(
  ...
  // Note that CabinDB 5.15 and earlier may not be able to open later versions
...
// Windows API macro interference
#undef DeleteFile
  // WARNING: This API is planned for removal in CabinDB 7.0 since it does not
  ...
  // > 1, then CabinDB 5.15 and earlier will not be able to open it.
  //
  ...
  CABINDB_DEPRECATED_FUNC virtual Status AddFile(
      ColumnFamilyHandle* column_family,
