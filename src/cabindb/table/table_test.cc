...
#include "include/cabindb/file_checksum.h"
...
class ReverseKeyComparator : public Comparator {
 public:
  const char* Name() const override {
    return "cabindb.ReverseBytewiseComparator";
  }
 ...
 public:
  explicit TableConstructor(const Comparator* cmp,
      ...
      : Constructor(cmp),
        ...
        level_(level) {
    env_ = CABINDB_NAMESPACE::Env::Default();
  }
  ...
  test::StringSink* TEST_GetSink() {
    return CABINDB_NAMESPACE::test::GetStringSinkFromLegacyWriter(
        file_writer_.get());
  }
...
class FixedOrLessPrefixTransform : public SliceTransform {
 ...
 public:
  ...
  const char* Name() const override { return "cabindb.FixedPrefix"; }
  ...
  explicit HarnessTest(const TestArgs& args)
    ...
    switch (args_.type) {
      case BLOCK_BASED_TABLE_TEST:
        ...
        break;
// Plain table is not supported in CABINDB_LITE
#ifndef CABINDB_LITE
 ...
 public:
  ...
  Status CalculateFileChecksum(FileChecksumGenerator* file_checksum_generator,
    ...
    test::StringSink* ss_rw =
        CABINDB_NAMESPACE::test::GetStringSinkFromLegacyWriter(
            file_writer_.get());
...
TEST_P(BlockBasedTableTest, BlockBasedTableProperties2) {
...

  {
    ...
    ASSERT_EQ("cabindb.ReverseBytewiseComparator", props.comparator_name);
    ASSERT_EQ("UInt64AddOperator", props.merge_operator_name);
    ASSERT_EQ("cabindb.Noop", props.prefix_extractor_name);
    ASSERT_EQ(
...
TEST_P(BlockBasedTableTest, FilterPolicyNameProperties) {
  ...
  c.Finish(options, ioptions, moptions, table_options,
           GetPlainInternalComparator(options.comparator), &keys, &kvmap);
  auto& props = *c.GetTableReader()->GetTableProperties();
  ASSERT_EQ("cabindb.BuiltinBloomFilter", props.filter_policy_name);
  c.ResetTableReader();
}
...
TEST_F(PlainTableTest, BasicPlainTableProperties) {
  ...
  test::StringSink* ss =
      CABINDB_NAMESPACE::test::GetStringSinkFromLegacyWriter(file_writer.get());
...
TEST_F(DBHarnessTest, RandomizedLongDB) {
  ...
  for (int level = 0; level < db()->NumberLevels(); level++) {
    std::string value;
    char name[100];
    snprintf(name, sizeof(name), "cabindb.num-files-at-level%d", level);
...
class TestPrefixExtractor : public CABINDB_NAMESPACE::SliceTransform {
 public:
  ~TestPrefixExtractor() override{};
  const char* Name() const override { return "TestPrefixExtractor"; }

  CABINDB_NAMESPACE::Slice Transform(
      const CABINDB_NAMESPACE::Slice& src) const override {
    assert(IsValid(src));
    return CABINDB_NAMESPACE::Slice(src.data(), 3);
  }

  bool InDomain(const CABINDB_NAMESPACE::Slice& src) const override {
    assert(IsValid(src));
    return true;
  }

  bool InRange(const CABINDB_NAMESPACE::Slice& /*dst*/) const override {
    return true;
  }

  bool IsValid(const CABINDB_NAMESPACE::Slice& src) const {
    if (src.size() != 4) {
      return false;
    }
    if (src[0] != '[') {
...
TEST_F(PrefixTest, PrefixAndWholeKeyTest) {
  ...
  options.compaction_style = CABINDB_NAMESPACE::kCompactionStyleUniversal;
  ...
  CABINDB_NAMESPACE::BlockBasedTableOptions bbto;
  bbto.filter_policy.reset(CABINDB_NAMESPACE::NewBloomFilterPolicy(10));
  ...
  ASSERT_OK(CABINDB_NAMESPACE::DB::Open(options, kDBPath, &db));
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      ASSERT_OK(db->Put(CABINDB_NAMESPACE::WriteOptions(), key, "1"));
    }
  }
...
/*
 * Disable TableWithGlobalSeqno since CabinDB does not store global_seqno in
 * the SST file any more. Instead, CabinDB deduces global_seqno from the
...
TEST_P(BlockBasedTableTest, BadOptions) {
  ...
  DestroyDB(kDBPath, options);
  CABINDB_NAMESPACE::DB* db;
  ASSERT_NOK(CABINDB_NAMESPACE::DB::Open(options, kDBPath, &db));
