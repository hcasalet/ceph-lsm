...
void LogPropertiesCollectionError(
  ...
  CABIN_LOG_ERROR(info_log, "%s", msg.c_str());
}
...
Status ReadProperties(const ReadOptions& read_options,
  ...
  for (iter.SeekToFirstOrReport(); iter.Valid(); iter.NextOrReport()) {
    ...
    if (pos != predefined_uint64_properties.end()) {
      ...
      // handle predefined cabindb properties
      uint64_t val;
      if (!GetVarint64(&raw_val, &val)) {
        ...
        CABIN_LOG_ERROR(ioptions.info_log, "%s", error_msg.c_str());
        continue;
      }
      *(pos->second) = val;
  ...
  if (s.ok()) {
    *table_properties = new_table_properties;
    ...
    if (verification_buf != nullptr) {
      ...
      *verification_buf =
          CABINDB_NAMESPACE::AllocateBlock(len, memory_allocator);
