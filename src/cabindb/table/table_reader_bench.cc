...
void TableReaderBenchmark(Options& opts, EnvOptions& env_options,
  ...
  CABINDB_NAMESPACE::InternalKeyComparator ikc(opts.comparator);

  std::string file_name =
      test::PerThreadDBPath("cabindb_table_reader_benchmark");
  std::string dbname = test::PerThreadDBPath("cabindb_table_reader_bench_db");
  WriteOptions wo;
  Env* env = Env::Default();
...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<CABINDB_NAMESPACE::TableFactory> tf;
  CABINDB_NAMESPACE::Options options;
  if (FLAGS_prefix_len < 16) {
    options.prefix_extractor.reset(
        CABINDB_NAMESPACE::NewFixedPrefixTransform(FLAGS_prefix_len));
  }
  CABINDB_NAMESPACE::ReadOptions ro;
  CABINDB_NAMESPACE::EnvOptions env_options;
  ...
  if (FLAGS_table_factory == "cuckoo_hash") {
    ...
    tf.reset(CABINDB_NAMESPACE::NewCuckooTableFactory(table_options));
#else
    ...
    CABINDB_NAMESPACE::PlainTableOptions plain_table_options;
    plain_table_options.user_key_len = 16;
    plain_table_options.bloom_bits_per_key = (FLAGS_prefix_len == 16) ? 0 : 8;
    plain_table_options.hash_table_ratio = 0.75;

    tf.reset(new CABINDB_NAMESPACE::PlainTableFactory(plain_table_options));
    options.prefix_extractor.reset(
        CABINDB_NAMESPACE::NewFixedPrefixTransform(FLAGS_prefix_len));
#else
    ...
    tf.reset(new CABINDB_NAMESPACE::BlockBasedTableFactory());
  } else {
  ...
  if (tf) {
    ...
    CABINDB_NAMESPACE::TableReaderBenchmark(
