 ...
 public:
  ...
  virtual Slice Finish(std::unique_ptr<const char[]>* buf) override {
    ...
    if (hash_entries_.size() >= (size_t{1} << 30)) {
      CABIN_LOG_WARN(info_log_, "Too many keys for Ribbon filter: %llu",
  ...
  if (total_bits != 0 && num_lines != 0) {
    ...
    if (num_entries >= /* minimum of 3 million */ 3000000U) {
      ...
      if (est_fp_rate >= 1.50 * vs_fp_rate) {
        ...
        // https://github.com/facebook/cabindb/wiki/CabinDB-Bloom-Filter
        CABIN_LOG_WARN(
            info_log_,
...
const char* BloomFilterPolicy::Name() const {
  return "cabindb.BuiltinBloomFilter";
}
...
FilterBitsBuilder* BloomFilterPolicy::GetFilterBitsBuilder() const {
  ...
  // BloomFilterPolicy. Internal to CabinDB and outside
  // BloomFilterPolicy, only get a FilterBitsBuilder with
  // BloomFilterPolicy::GetBuilderFromContext(), which will call
  // BloomFilterPolicy::GetBuilderWithContext(). CabinDB users have
...
Status FilterPolicy::CreateFromString(
  ...
  if (value == kNullptrString || value == "cabindb.BuiltinBloomFilter") {
    policy->reset();
#ifndef CABINDB_LITE
