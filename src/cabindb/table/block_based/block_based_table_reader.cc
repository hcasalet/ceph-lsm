...
bool IsFeatureSupported(const TableProperties& table_properties,
  ...
  if (pos != props.end()) {
    if (pos->second == kPropFalse) {
      ...
      CABIN_LOG_WARN(info_log, "Property %s has invalidate value %s",
...
Status BlockBasedTable::TryReadPropertiesWithGlobalSeqno(
  ...
  if (s.ok() && tmp_buf) {
    ...
    s = CABINDB_NAMESPACE::VerifyBlockChecksum(
...
Status BlockBasedTable::ReadPropertiesBlock(
  ...
  if (!s.ok()) {
    CABIN_LOG_WARN(rep_->ioptions.info_log,
    ...
    CABIN_LOG_ERROR(rep_->ioptions.info_log,
  ...
  if (rep_->table_properties) {
    ...
    s = GetGlobalSequenceNumber(*(rep_->table_properties), largest_seqno,
                                &(rep_->global_seqno));
    if (!s.ok()) {
      CABIN_LOG_ERROR(rep_->ioptions.info_log, "%s", s.ToString().c_str());
    }
  }
  return s;
}
