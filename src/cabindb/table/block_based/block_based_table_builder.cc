...
// format_version is the block format as defined in include/cabindb/table.h
...
//    echo cabindb.table.block_based | sha1sum
...
struct BlockBasedTableBuilder::Rep {
  ...
  Rep(const ImmutableCFOptions& _ioptions, const MutableCFOptions& _moptions,
    ...
    if (!ReifyDbHostIdProperty(ioptions.env, &db_host_id).ok()) {
      CABIN_LOG_INFO(ioptions.info_log, "db_host_id property will not be set");
    }
  }
    ...
    if (*type != kNoCompression && r->table_options.verify_compression) {
      ...
      if (stat.ok()) {
        ...
        if (!compressed_ok) {
          ...
          CABIN_LOG_ERROR(r->ioptions.info_log,
...
void BlockBasedTableBuilder::WriteFooter(BlockHandle& metaindex_block_handle,
  ...
  // We're writing legacy magic number because we want old versions of CabinDB
  // be able to read files generated with new release (just in case if
  // somebody wants to roll back after an upgrade)
  // TODO(icanadi) at some point in the future, when we're absolutely sure
  // nobody will roll back to CabinDB 2.x versions, retire the legacy magic
