...
#include "include/cabindb/sst_partitioner.h"
...
void ColumnFamilyOptions::Dump(Logger* log) const {
  CABIN_LOG_HEADER(log, "              Options.comparator: %s",
                   comparator->Name());
  CABIN_LOG_HEADER(log, "          Options.merge_operator: %s",
                   merge_operator ? merge_operator->Name() : "None");
  CABIN_LOG_HEADER(log, "       Options.compaction_filter: %s",
  ...
  CABIN_LOG_HEADER(log, "        Options.memtable_factory: %s",
                   memtable_factory->Name());
  CABIN_LOG_HEADER(log, "           Options.table_factory: %s",
                   table_factory->Name());
  CABIN_LOG_HEADER(log, "           table_factory options: %s",
                   table_factory->GetPrintableOptions().c_str());
  CABIN_LOG_HEADER(log, "       Options.write_buffer_size: %" CABINDB_PRIszt,
                   write_buffer_size);
  CABIN_LOG_HEADER(log, " Options.max_write_buffer_number: %d",
  ...
  if (!compression_per_level.empty()) {
    for (unsigned int i = 0; i < compression_per_level.size(); i++) {
      ...
      CABIN_LOG_HEADER(log, "         Options.compression: %s",
    ...
    CABIN_LOG_HEADER(log, "            Options.num_levels: %d", num_levels);
    CABIN_LOG_HEADER(log, "       Options.min_write_buffer_number_to_merge: %d",
                     min_write_buffer_number_to_merge);
    CABIN_LOG_HEADER(log, "    Options.max_write_buffer_number_to_maintain: %d",
                     max_write_buffer_number_to_maintain);
    CABIN_LOG_HEADER(log,
    ...
    CABIN_LOG_HEADER(log, "           Options.compression_opts.window_bits: %d",
                     compression_opts.window_bits);
    CABIN_LOG_HEADER(log, "                 Options.compression_opts.level: %d",
                     compression_opts.level);
    CABIN_LOG_HEADER(log, "              Options.compression_opts.strategy: %d",
    ...
    CABIN_LOG_HEADER(log, "     Options.level0_file_num_compaction_trigger: %d",
                     level0_file_num_compaction_trigger);
    CABIN_LOG_HEADER(log, "         Options.level0_slowdown_writes_trigger: %d",
                     level0_slowdown_writes_trigger);
    CABIN_LOG_HEADER(log, "             Options.level0_stop_writes_trigger: %d",
                     level0_stop_writes_trigger);
    CABIN_LOG_HEADER(
        log, "                  Options.target_file_size_base: %" PRIu64,
        target_file_size_base);
    CABIN_LOG_HEADER(log, "            Options.target_file_size_multiplier: %d",
                     target_file_size_multiplier);
    CABIN_LOG_HEADER(
        log, "               Options.max_bytes_for_level_base: %" PRIu64,
        max_bytes_for_level_base);
    CABIN_LOG_HEADER(log, "Options.level_compaction_dynamic_level_bytes: %d",
                     level_compaction_dynamic_level_bytes);
    CABIN_LOG_HEADER(log, "         Options.max_bytes_for_level_multiplier: %f",
                     max_bytes_for_level_multiplier);
    for (size_t i = 0; i < max_bytes_for_level_multiplier_additional.size();
         i++) {
      CABIN_LOG_HEADER(
          log, "Options.max_bytes_for_level_multiplier_addtl[%" CABINDB_PRIszt
               "]: %d",
    ...
    CABIN_LOG_HEADER(
        log,
        "                       Options.arena_block_size: %" CABINDB_PRIszt,
        arena_block_size);
    CABIN_LOG_HEADER(log,
    ...
    CABIN_LOG_HEADER(log,
                     ...
                     hard_pending_compaction_bytes_limit);
    CABIN_LOG_HEADER(log, "      Options.rate_limit_delay_max_milliseconds: %u",
                     rate_limit_delay_max_milliseconds);
    CABIN_LOG_HEADER(log, "               Options.disable_auto_compactions: %d",
    ...
    CABIN_LOG_HEADER(
        log,
        "                Options.inplace_update_num_locks: %" CABINDB_PRIszt,
    ...
    CABIN_LOG_HEADER(log, "  Options.memtable_huge_page_size: %" CABINDB_PRIszt,
    ...
    CABIN_LOG_HEADER(
        log,
        "                   Options.max_successive_merges: %" CABINDB_PRIszt,
        max_successive_merges);
    CABIN_LOG_HEADER(log,
                     "               Options.optimize_filters_for_hits: %d",
                     optimize_filters_for_hits);
    CABIN_LOG_HEADER(log, "               Options.paranoid_file_checks: %d",
                     paranoid_file_checks);
    CABIN_LOG_HEADER(log, "               Options.force_consistency_checks: %d",
                     force_consistency_checks);
    CABIN_LOG_HEADER(log, "               Options.report_bg_io_stats: %d",
                     report_bg_io_stats);
    CABIN_LOG_HEADER(log, "                              Options.ttl: %" PRIu64,
                     ttl);
    CABIN_LOG_HEADER(log,
                     "         Options.periodic_compaction_seconds: %" PRIu64,
                     periodic_compaction_seconds);
    CABIN_LOG_HEADER(log, "                   Options.enable_blob_files: %s",
    ...
    CABIN_LOG_HEADER(log,
                     ...
                     blob_file_size);
    CABIN_LOG_HEADER(log, "               Options.blob_compression_type: %s",
                     CompressionTypeToString(blob_compression_type).c_str());
    CABIN_LOG_HEADER(log, "      Options.enable_blob_garbage_collection: %s",
                     enable_blob_garbage_collection ? "true" : "false");
    CABIN_LOG_HEADER(log, "  Options.blob_garbage_collection_age_cutoff: %f",
                     blob_garbage_collection_age_cutoff);
...
Options* Options::OldDefaults(int cabindb_major_version,
                              int cabindb_minor_version) {
  ColumnFamilyOptions::OldDefaults(cabindb_major_version,
                                   cabindb_minor_version);
  DBOptions::OldDefaults(cabindb_major_version, cabindb_minor_version);
  return this;
}

DBOptions* DBOptions::OldDefaults(int cabindb_major_version,
                                  int cabindb_minor_version) {
  if (cabindb_major_version < 4 ||
      (cabindb_major_version == 4 && cabindb_minor_version < 7)) {
    max_file_opening_threads = 1;
    table_cache_numshardbits = 4;
  }
  if (cabindb_major_version < 5 ||
      (cabindb_major_version == 5 && cabindb_minor_version < 2)) {
    delayed_write_rate = 2 * 1024U * 1024U;
  } else if (cabindb_major_version < 5 ||
             (cabindb_major_version == 5 && cabindb_minor_version < 6)) {
...
ColumnFamilyOptions* ColumnFamilyOptions::OldDefaults(
    int cabindb_major_version, int cabindb_minor_version) {
  if (cabindb_major_version < 5 ||
      (cabindb_major_version == 5 && cabindb_minor_version <= 18)) {
  ...
  } else if (cabindb_major_version == 5 && cabindb_minor_version < 2) {
...
DBOptions* DBOptions::OptimizeForSmallDb(std::shared_ptr<Cache>* cache) {
  ...
  std::shared_ptr<CABINDB_NAMESPACE::WriteBufferManager> wbm =
      std::make_shared<CABINDB_NAMESPACE::WriteBufferManager>(
