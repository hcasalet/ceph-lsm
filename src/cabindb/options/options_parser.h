...
#define CABINDB_OPTION_FILE_MAJOR 1
#define CABINDB_OPTION_FILE_MINOR 1
...
Status PersistCabinDBOptions(const ConfigOptions& config_options,
                             const DBOptions& db_opt,
                             const std::vector<std::string>& cf_names,
                             const std::vector<ColumnFamilyOptions>& cf_opts,
                             const std::string& file_name, FileSystem* fs);

class CabinDBOptionsParser {
 public:
  explicit CabinDBOptionsParser();
  ...
  static Status VerifyCabinDBOptionsFromFile(
  ...
  static Status ExtraParserCheck(const CabinDBOptionsParser& input_parser);
