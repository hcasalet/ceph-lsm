...
#include "include/cabindb/utilities/leveldb_options.h"
...
TEST_F(OptionsTest, GetOptionsFromMapTest) {
  ...
  ASSERT_NOK(GetColumnFamilyOptionsFromMap(exact, base_cf_opt, cf_options_map,
                                           &new_cf_opt));
  ASSERT_OK(
      CabinDBOptionsParser::VerifyCFOptions(exact, base_cf_opt, new_cf_opt));
  ...
  ASSERT_OK(GetColumnFamilyOptionsFromMap(loose, base_cf_opt, cf_options_map,
                                          &new_cf_opt));
  ASSERT_OK(
      CabinDBOptionsParser::VerifyCFOptions(loose, base_cf_opt, new_cf_opt));
  ASSERT_NOK(
      CabinDBOptionsParser::VerifyCFOptions(exact, base_cf_opt, new_cf_opt));
  ...
  ASSERT_OK(
      CabinDBOptionsParser::VerifyDBOptions(exact, base_db_opt, new_db_opt));
  ASSERT_OK(
      CabinDBOptionsParser::VerifyDBOptions(loose, base_db_opt, new_db_opt));
...
#ifndef CABINDB_LITE  // GetColumnFamilyOptionsFromString is not supported in
                      // CABINDB_LITE
TEST_F(OptionsTest, GetColumnFamilyOptionsFromStringTest) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(config_options, base_cf_opt,
                                                  new_cf_opt));
  ...
  ASSERT_EQ(prefix_name, "cabindb.CappedPrefix.8");
...
#ifndef CABINDB_LITE  // GetBlockBasedTableOptionsFromString is not supported
...
#ifndef CABINDB_LITE  // GetPlainTableOptionsFromString is not supported
...
#ifndef CABINDB_LITE  // GetMemTableRepFactoryFromString is not supported
...
#ifndef CABINDB_LITE  // GetOptionsFromString is not supported in CabinDB Lite
...
TEST_F(OptionsTest, DBOptionsSerialization) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(config_options, base_options,
                                                  new_options));
}
...
TEST_F(OptionsTest, OptionsComposeDecompose) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(config_options, base_db_opts,
                                                  new_db_opts));
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(config_options, base_cf_opts,
                                                  new_cf_opts));
...
TEST_F(OptionsTest, ColumnFamilyOptionsSerialization) {
  ...
  ASSERT_OK(
      CabinDBOptionsParser::VerifyCFOptions(config_options, base_opt, new_opt));
...
#ifndef CABINDB_LITE  // StringToMap is not supported in CABINDB_LITE
...
TEST_F(OptionsOldApiTest, GetOptionsFromMapTest) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(exact, base_cf_opt, new_cf_opt));
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(
      loose, base_cf_opt, new_cf_opt, nullptr /* new_opt_map */));
  ASSERT_NOK(CabinDBOptionsParser::VerifyCFOptions(
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(exact, base_db_opt, new_db_opt));
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(loose, base_db_opt, new_db_opt));
  ...
  ASSERT_OK(GetDBOptionsFromMap(base_db_opt, db_options_map, &new_db_opt,
                                ...
                                true /* ignore_unknown_options */));
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(loose, base_db_opt, new_db_opt));
  ASSERT_NOK(CabinDBOptionsParser::VerifyDBOptions(exact, base_db_opt, new_db_opt));
}
...
TEST_F(OptionsOldApiTest, DBOptionsSerialization) {
  ...
  ASSERT_OK(GetDBOptionsFromString(DBOptions(), base_options_file_content,
                                   &new_options));
  ConfigOptions config_options;
  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(config_options, base_options, new_options));
}
...
TEST_F(OptionsOldApiTest, ColumnFamilyOptionsSerialization) {
  ...
  ConfigOptions config_options;
  ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(config_options, base_opt, new_opt));
  if (base_opt.compaction_filter) {
    delete base_opt.compaction_filter;
  }
}
...
TEST_F(OptionsParserTest, Comment) {
  ...
  const std::string kTestFileName = "test-cabindb-options.ini";
  ASSERT_OK(env_->WriteToNewFile(kTestFileName, options_file_content));
  CabinDBOptionsParser parser;
  ASSERT_OK(
  ...
  exact.sanity_level = ConfigOptions::kSanityLevelExactMatch;
  ASSERT_OK(
      CabinDBOptionsParser::VerifyDBOptions(exact, *parser.db_opt(), db_opt));
...
TEST_F(OptionsParserTest, IgnoreUnknownOptions) {
  for (int case_id = 0; case_id < 5; case_id++) {
    ...
    if (case_id == 0) {
      ...
      should_ignore = false;
      version_string =
          ToString(CABINDB_MAJOR) + "." + ToString(CABINDB_MINOR) + ".0";
    } else if (case_id == 1) {
      // higher minor version
      should_ignore = true;
      version_string =
          ToString(CABINDB_MAJOR) + "." + ToString(CABINDB_MINOR + 1) + ".0";
    } else if (case_id == 2) {
      // higher major version.
      should_ignore = true;
      version_string = ToString(CABINDB_MAJOR + 1) + ".0.0";
    } else if (case_id == 3) {
      // lower minor version
#if CABINDB_MINOR == 0
      continue;
      ...
      should_ignore = false;
      version_string =
          ToString(CABINDB_MAJOR - 1) + "." + ToString(CABINDB_MINOR) + ".0";
    }
...
TEST_F(OptionsParserTest, Readahead) {
  ...
  ASSERT_OK(PersistCabinDBOptions(base_db_opt, cf_names, base_cf_opts,
...
TEST_F(OptionsParserTest, DumpAndParse) {
  ...
  ASSERT_OK(CabinDBOptionsParser::VerifyCabinDBOptionsFromFile(
      config_options, base_db_opt, cf_names, base_cf_opts, kOptionsFileName,
      fs_.get()));

  ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(
  ...
  ASSERT_NOK(CabinDBOptionsParser::VerifyCabinDBOptionsFromFile(
      config_options, base_db_opt, cf_names, base_cf_opts, kOptionsFileName,
      fs_.get()));
...
TEST_F(OptionsParserTest, DifferentDefault) {
  ...
  ASSERT_OK(PersistCabinDBOptions(DBOptions(), {"default", "universal"},
...
class OptionsSanityCheckTest : public OptionsParserTest {
 ...
 protected:
  Status SanityCheckCFOptions(const ColumnFamilyOptions& cf_opts,
    ...
    return CabinDBOptionsParser::VerifyCabinDBOptionsFromFile(
  ...
  Status PersistCFOptions(const ColumnFamilyOptions& cf_opts) {
    ...
    return PersistCabinDBOptions(DBOptions(), {"default"}, {cf_opts},
                                 kOptionsFileName, fs_.get());
  }
...
TEST_F(OptionsParserTest, EscapeOptionString) {
  ...
  ASSERT_EQ(CabinDBOptionsParser::TrimAndRemoveComment(
