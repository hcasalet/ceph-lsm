...
#include "include/cabindb/wal_filter.h"
...
void ImmutableDBOptions::Dump(Logger* log) const {
  CABIN_LOG_HEADER(log, "                        Options.error_if_exists: %d",
                   error_if_exists);
  CABIN_LOG_HEADER(log, "                      Options.create_if_missing: %d",
                   create_if_missing);
  CABIN_LOG_HEADER(log, "                        Options.paranoid_checks: %d",
                   paranoid_checks);
  CABIN_LOG_HEADER(log,
                   "                              "
                   "Options.track_and_verify_wals_in_manifest: %d",
                   track_and_verify_wals_in_manifest);
  CABIN_LOG_HEADER(log, "                                    Options.env: %p",
                   env);
  CABIN_LOG_HEADER(log, "                                     Options.fs: %s",
                   fs->Name());
  CABIN_LOG_HEADER(log, "                               Options.info_log: %p",
                   info_log.get());
  CABIN_LOG_HEADER(log, "               Options.max_file_opening_threads: %d",
                   max_file_opening_threads);
  CABIN_LOG_HEADER(log, "                             Options.statistics: %p",
                   statistics.get());
  CABIN_LOG_HEADER(log, "                              Options.use_fsync: %d",
                   use_fsync);
  CABIN_LOG_HEADER(
      log, "                      Options.max_log_file_size: %" CABINDB_PRIszt,
      max_log_file_size);
  CABIN_LOG_HEADER(log,
                   "                 Options.max_manifest_file_size: %" PRIu64,
                   max_manifest_file_size);
  CABIN_LOG_HEADER(
      log, "                  Options.log_file_time_to_roll: %" CABINDB_PRIszt,
      log_file_time_to_roll);
  CABIN_LOG_HEADER(
      log, "                      Options.keep_log_file_num: %" CABINDB_PRIszt,
      keep_log_file_num);
  CABIN_LOG_HEADER(
      log, "                   Options.recycle_log_file_num: %" CABINDB_PRIszt,
      recycle_log_file_num);
  CABIN_LOG_HEADER(log, "                        Options.allow_fallocate: %d",
                   allow_fallocate);
  CABIN_LOG_HEADER(log, "                       Options.allow_mmap_reads: %d",
                   allow_mmap_reads);
  CABIN_LOG_HEADER(log, "                      Options.allow_mmap_writes: %d",
                   allow_mmap_writes);
  CABIN_LOG_HEADER(log, "                       Options.use_direct_reads: %d",
                   use_direct_reads);
  CABIN_LOG_HEADER(log,
                   "                       "
                   "Options.use_direct_io_for_flush_and_compaction: %d",
                   use_direct_io_for_flush_and_compaction);
  CABIN_LOG_HEADER(log, "         Options.create_missing_column_families: %d",
                   create_missing_column_families);
  CABIN_LOG_HEADER(log, "                             Options.db_log_dir: %s",
                   db_log_dir.c_str());
  CABIN_LOG_HEADER(log, "                                Options.wal_dir: %s",
                   wal_dir.c_str());
  CABIN_LOG_HEADER(log, "               Options.table_cache_numshardbits: %d",
                   table_cache_numshardbits);
  CABIN_LOG_HEADER(log,
  ...
  CABIN_LOG_HEADER(log,
                   ...
                   max_write_batch_group_size_bytes);
  CABIN_LOG_HEADER(
      log, "            Options.manifest_preallocation_size: %" CABINDB_PRIszt,
      manifest_preallocation_size);
  CABIN_LOG_HEADER(log, "                    Options.is_fd_close_on_exec: %d",
                   is_fd_close_on_exec);
  CABIN_LOG_HEADER(log, "                  Options.advise_random_on_open: %d",
                   advise_random_on_open);
  CABIN_LOG_HEADER(
      log, "                   Options.db_write_buffer_size: %" CABINDB_PRIszt,
      db_write_buffer_size);
  CABIN_LOG_HEADER(log, "                   Options.write_buffer_manager: %p",
                   write_buffer_manager.get());
  CABIN_LOG_HEADER(log, "        Options.access_hint_on_compaction_start: %d",
                   static_cast<int>(access_hint_on_compaction_start));
  CABIN_LOG_HEADER(log, " Options.new_table_reader_for_compaction_inputs: %d",
                   new_table_reader_for_compaction_inputs);
  CABIN_LOG_HEADER(
      log, "          Options.random_access_max_buffer_size: %" CABINDB_PRIszt,
      random_access_max_buffer_size);
  CABIN_LOG_HEADER(log, "                     Options.use_adaptive_mutex: %d",
                   use_adaptive_mutex);
  CABIN_LOG_HEADER(log, "                           Options.rate_limiter: %p",
                   rate_limiter.get());
  Header(
      log, "    Options.sst_file_manager.rate_bytes_per_sec: %" PRIi64,
      sst_file_manager ? sst_file_manager->GetDeleteRateBytesPerSecond() : 0);
  CABIN_LOG_HEADER(log, "                      Options.wal_recovery_mode: %d",
                   static_cast<int>(wal_recovery_mode));
  CABIN_LOG_HEADER(log, "                 Options.enable_thread_tracking: %d",
                   enable_thread_tracking);
  CABIN_LOG_HEADER(log, "                 Options.enable_pipelined_write: %d",
                   enable_pipelined_write);
  CABIN_LOG_HEADER(log, "                 Options.unordered_write: %d",
                   unordered_write);
  CABIN_LOG_HEADER(log, "        Options.allow_concurrent_memtable_write: %d",
                   allow_concurrent_memtable_write);
  CABIN_LOG_HEADER(log, "     Options.enable_write_thread_adaptive_yield: %d",
                   enable_write_thread_adaptive_yield);
  CABIN_LOG_HEADER(log,
  ...
  if (row_cache) {
    CABIN_LOG_HEADER(
        log,
        "                              Options.row_cache: %" CABINDB_PRIszt,
...
#ifndef CABINDB_LITE
  CABIN_LOG_HEADER(log, "                             Options.wal_filter: %s",
                   wal_filter ? wal_filter->Name() : "None");
#endif  // ROCKDB_LITE

  CABIN_LOG_HEADER(log, "            Options.avoid_flush_during_recovery: %d",
                   avoid_flush_during_recovery);
  CABIN_LOG_HEADER(log, "            Options.allow_ingest_behind: %d",
                   allow_ingest_behind);
  CABIN_LOG_HEADER(log, "            Options.preserve_deletes: %d",
                   preserve_deletes);
  CABIN_LOG_HEADER(log, "            Options.two_write_queues: %d",
                   two_write_queues);
  CABIN_LOG_HEADER(log, "            Options.manual_wal_flush: %d",
                   manual_wal_flush);
  CABIN_LOG_HEADER(log, "            Options.atomic_flush: %d", atomic_flush);
  CABIN_LOG_HEADER(log,
                   "            Options.avoid_unnecessary_blocking_io: %d",
                   avoid_unnecessary_blocking_io);
  CABIN_LOG_HEADER(log, "                Options.persist_stats_to_disk: %u",
                   persist_stats_to_disk);
  CABIN_LOG_HEADER(log, "                Options.write_dbid_to_manifest: %d",
                   write_dbid_to_manifest);
  CABIN_LOG_HEADER(
      log, "                Options.log_readahead_size: %" CABINDB_PRIszt,
      log_readahead_size);
  CABIN_LOG_HEADER(log, "                Options.file_checksum_gen_factory: %s",
                   file_checksum_gen_factory ? file_checksum_gen_factory->Name()
                                             : kUnknownFileChecksumFuncName);
  CABIN_LOG_HEADER(log, "                Options.best_efforts_recovery: %d",
                   static_cast<int>(best_efforts_recovery));
  CABIN_LOG_HEADER(log, "               Options.max_bgerror_resume_count: %d",
                   max_bgerror_resume_count);
  CABIN_LOG_HEADER(log,
                   "           Options.bgerror_resume_retry_interval: %" PRIu64,
                   bgerror_resume_retry_interval);
  CABIN_LOG_HEADER(log, "            Options.allow_data_in_errors: %d",
                   allow_data_in_errors);
  CABIN_LOG_HEADER(log, "            Options.db_host_id: %s",
                   db_host_id.c_str());
}
...
void MutableDBOptions::Dump(Logger* log) const {
  CABIN_LOG_HEADER(log, "            Options.max_background_jobs: %d",
                   max_background_jobs);
  CABIN_LOG_HEADER(log, "            Options.max_background_compactions: %d",
                   max_background_compactions);
  CABIN_LOG_HEADER(log, "            Options.max_subcompactions: %" PRIu32,
                   max_subcompactions);
  CABIN_LOG_HEADER(log, "            Options.avoid_flush_during_shutdown: %d",
                   avoid_flush_during_shutdown);
  CABIN_LOG_HEADER(
      log, "          Options.writable_file_max_buffer_size: %" CABINDB_PRIszt,
      writable_file_max_buffer_size);
  CABIN_LOG_HEADER(log, "            Options.delayed_write_rate : %" PRIu64,
                   delayed_write_rate);
  CABIN_LOG_HEADER(log, "            Options.max_total_wal_size: %" PRIu64,
                   max_total_wal_size);
  CABIN_LOG_HEADER(
      log, "            Options.delete_obsolete_files_period_micros: %" PRIu64,
      delete_obsolete_files_period_micros);
  CABIN_LOG_HEADER(log, "                  Options.stats_dump_period_sec: %u",
                   stats_dump_period_sec);
  CABIN_LOG_HEADER(log, "                Options.stats_persist_period_sec: %d",
                   stats_persist_period_sec);
  CABIN_LOG_HEADER(
      log,
      "                Options.stats_history_buffer_size: %" CABINDB_PRIszt,
      stats_history_buffer_size);
  CABIN_LOG_HEADER(log, "                         Options.max_open_files: %d",
                   max_open_files);
  CABIN_LOG_HEADER(log,
  ...
  CABIN_LOG_HEADER(log,
                   ...
                   strict_bytes_per_sync);
  CABIN_LOG_HEADER(log,
                   "      Options.compaction_readahead_size: %" CABINDB_PRIszt,
                   compaction_readahead_size);
  CABIN_LOG_HEADER(log, "                 Options.max_background_flushes: %d",
                          max_background_flushes);
}
