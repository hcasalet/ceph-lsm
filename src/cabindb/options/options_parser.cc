...
Status PersistCabinDBOptions(const DBOptions& db_opt,
  ...
  return PersistCabinDBOptions(config_options, db_opt, cf_names, cf_opts,
                               file_name, fs);
}

Status PersistCabinDBOptions(const ConfigOptions& config_options_in,
  ...
  TEST_SYNC_POINT("PersistCabinDBOptions:start");
  ...
  s = writable->Append(option_file_header + "[" +
                       ...
                       "]\n"
                       "  cabindb_version=" +
                       ToString(CABINDB_MAJOR) + "." + ToString(CABINDB_MINOR) +
                       "." + ToString(CABINDB_PATCH) + "\n");
  if (s.ok()) {
    s = writable->Append(
        "  options_file_version=" + ToString(CABINDB_OPTION_FILE_MAJOR) + "." +
        ToString(CABINDB_OPTION_FILE_MINOR) + "\n");
  }
  if (s.ok()) {
...
CabinDBOptionsParser::CabinDBOptionsParser() { Reset(); }

void CabinDBOptionsParser::Reset() {
  db_opt_ = DBOptions();
  db_opt_map_.clear();
  cf_names_.clear();
  cf_opts_.clear();
...
bool CabinDBOptionsParser::IsSection(const std::string& line) {
  if (line.size() < 2) {
    return false;
  }
...
Status CabinDBOptionsParser::ParseSection(OptionSection* section,
                                          std::string* title,
                                          std::string* argument,
...
Status CabinDBOptionsParser::InvalidArgument(const int line_num,
                                             const std::string& message) {
  return Status::InvalidArgument(
      "[CabinDBOptionsParser Error] ",
      message + " (at line " + ToString(line_num) + ")");
}

Status CabinDBOptionsParser::ParseStatement(std::string* name,
                                            std::string* value,
                                            const std::string& line,
                                            const int line_num) {
...
Status CabinDBOptionsParser::Parse(const std::string& file_name, FileSystem* fs,
...
Status CabinDBOptionsParser::Parse(const ConfigOptions& config_options_in,
  ...
  for (int line_num = 1; ReadOneLine(&iss, &sf_reader, &line, &has_data, &s);
    ...
    if (IsSection(line)) {
      ...
      if (config_options.ignore_unknown_options &&
        ...
        if (db_version[0] < CABINDB_MAJOR || (db_version[0] == CABINDB_MAJOR &&
                                              db_version[1] <= CABINDB_MINOR)) {
...
Status CabinDBOptionsParser::CheckSection(const OptionSection section,
...
Status CabinDBOptionsParser::ParseVersionNumber(const std::string& ver_name,
...
Status CabinDBOptionsParser::EndSection(
...
Status CabinDBOptionsParser::ValidityCheck() {
  if (!has_db_options_) {
    return Status::Corruption(
        "A CabinDB Option file must have a single DBOptions section");
  }
  if (!has_default_cf_options_) {
    return Status::Corruption(
        "A CabinDB Option file must have a single CFOptions:default section");
  }

  return Status::OK();
}

std::string CabinDBOptionsParser::TrimAndRemoveComment(const std::string& line,
                                                       bool trim_only) {
  size_t start = 0;
...
Status CabinDBOptionsParser::VerifyCabinDBOptionsFromFile(
  ...
  if (cf_names.size() != parser.cf_names()->size()) {
    if (config_options.sanity_level >=
      ...
      return Status::InvalidArgument(
          "[CabinDBOptionParser Error] The persisted options does not have "
  ...
  for (size_t i = 0; i < cf_names.size(); ++i) {
    if (cf_names[i] != parser.cf_names()->at(i)) {
      return Status::InvalidArgument(
          "[CabinDBOptionParser Error] The persisted options and the db"
...
Status CabinDBOptionsParser::VerifyDBOptions(
...
Status CabinDBOptionsParser::VerifyCFOptions(
...
Status CabinDBOptionsParser::VerifyTableFactory(
  ...
  if (base_tf && file_tf) {
    if (config_options.sanity_level > ConfigOptions::kSanityLevelNone &&
      ...
      return Status::Corruption(std::string("[CabinDBOptionsParser]:"
