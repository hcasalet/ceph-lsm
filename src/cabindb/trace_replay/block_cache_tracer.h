...
struct BlockCacheTraceHeader {
  uint64_t start_time;
  uint32_t cabindb_major_version;
  uint32_t cabindb_minor_version;
};

// BlockCacheTraceWriter captures all CabinDB block cache accesses using a
// user-provided TraceWriter. Every CabinDB operation is written as a single
...
class BlockCacheTraceWriter {
 public:
  ...
  // with some metadata like a magic number and CabinDB version.
  Status WriteHeader();
...
// https://github.com/facebook/cabindb/wiki/Block-cache-analysis-and-simulation-tools#trace-format
// for details.
