...
Status BlockCacheTraceReader::ReadHeader(BlockCacheTraceHeader* header) {
  ...
  if (!GetFixed32(&enc_slice, &header->cabindb_major_version)) {
    return Status::Corruption(
        "Corrupted header in the trace file: Failed to read cabindb major "
        "version number.");
  }
  if (!GetFixed32(&enc_slice, &header->cabindb_minor_version)) {
    return Status::Corruption(
        "Corrupted header in the trace file: Failed to read cabindb minor "
        "version number.");
  }
