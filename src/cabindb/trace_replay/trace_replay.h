...
// Tracer captures all CabinDB operations using a user-provided TraceWriter.
// Every CabinDB operation is written as a single trace. Each trace will have a
...
class Tracer {
 ...
 private:
  ...
  // with some metadata like a magic number, trace version, CabinDB version, and
  // trace format.
  Status WriteHeader();
...
// Replayer helps to replay the captured CabinDB operations, using a user
