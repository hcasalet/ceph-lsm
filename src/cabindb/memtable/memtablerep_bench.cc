...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<CABINDB_NAMESPACE::MemTableRepFactory> factory;
  if (FLAGS_memtablerep == "skiplist") {
    factory.reset(new CABINDB_NAMESPACE::SkipListFactory);
#ifndef CABINDB_LITE
  } else if (FLAGS_memtablerep == "vector") {
    factory.reset(new CABINDB_NAMESPACE::VectorRepFactory);
  } else if (FLAGS_memtablerep == "hashskiplist") {
    factory.reset(CABINDB_NAMESPACE::NewHashSkipListRepFactory(
        FLAGS_bucket_count, FLAGS_hashskiplist_height,
        FLAGS_hashskiplist_branching_factor));
    options.prefix_extractor.reset(
        CABINDB_NAMESPACE::NewFixedPrefixTransform(FLAGS_prefix_length));
  } else if (FLAGS_memtablerep == "hashlinklist") {
    factory.reset(CABINDB_NAMESPACE::NewHashLinkListRepFactory(
  ...
  CABINDB_NAMESPACE::InternalKeyComparator internal_key_comp(
      CABINDB_NAMESPACE::BytewiseComparator());
  CABINDB_NAMESPACE::MemTable::KeyComparator key_comp(internal_key_comp);
  CABINDB_NAMESPACE::Arena arena;
  CABINDB_NAMESPACE::WriteBufferManager wb(FLAGS_write_buffer_size);
  uint64_t sequence;
  ...
  std::unique_ptr<CABINDB_NAMESPACE::MemTableRep> memtablerep;
  CABINDB_NAMESPACE::Random64 rng(FLAGS_seed);
  const char* benchmarks = FLAGS_benchmarks.c_str();
  while (benchmarks != nullptr) {
    std::unique_ptr<CABINDB_NAMESPACE::KeyGenerator> key_gen;
    const char* sep = strchr(benchmarks, ',');
    CABINDB_NAMESPACE::Slice name;
    if (sep == nullptr) {
      name = benchmarks;
      benchmarks = nullptr;
    } else {
      name = CABINDB_NAMESPACE::Slice(benchmarks, sep - benchmarks);
      benchmarks = sep + 1;
    }
    std::unique_ptr<CABINDB_NAMESPACE::Benchmark> benchmark;
    if (name == CABINDB_NAMESPACE::Slice("fillseq")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::SEQUENTIAL, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::FillBenchmark(
          memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == CABINDB_NAMESPACE::Slice("fillrandom")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::UNIQUE_RANDOM, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::FillBenchmark(
          memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == CABINDB_NAMESPACE::Slice("readrandom")) {
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::RANDOM, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::ReadBenchmark(
          memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == CABINDB_NAMESPACE::Slice("readseq")) {
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::SEQUENTIAL, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::SeqReadBenchmark(memtablerep.get(),
                                                              &sequence));
    } else if (name == CABINDB_NAMESPACE::Slice("readwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::RANDOM, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::ReadWriteBenchmark<
                      CABINDB_NAMESPACE::ConcurrentReadBenchmarkThread>(
          memtablerep.get(), key_gen.get(), &sequence));
    } else if (name == CABINDB_NAMESPACE::Slice("seqreadwrite")) {
      memtablerep.reset(createMemtableRep());
      key_gen.reset(new CABINDB_NAMESPACE::KeyGenerator(
          &rng, CABINDB_NAMESPACE::RANDOM, FLAGS_num_operations));
      benchmark.reset(new CABINDB_NAMESPACE::ReadWriteBenchmark<
                      CABINDB_NAMESPACE::SeqConcurrentReadBenchmarkThread>(
          memtablerep.get(), key_gen.get(), &sequence));
    } else {
