...
#include <jni.h>
#include "include/org_cabindb_BlockBasedTableConfig.h"
#include "include/org_cabindb_PlainTableConfig.h"
#include "portal.h"
#include "cabindb/cache.h"
#include "cabindb/filter_policy.h"

/*
 * Class:     org_cabindb_PlainTableConfig
 * Method:    newTableFactoryHandle
 * Signature: (IIDIIBZZ)J
 */
jlong Java_org_cabindb_PlainTableConfig_newTableFactoryHandle(
  ...
  CABINDB_NAMESPACE::PlainTableOptions options =
      CABINDB_NAMESPACE::PlainTableOptions();
  ...
  options.encoding_type =
      static_cast<CABINDB_NAMESPACE::EncodingType>(jencoding_type);
...
/*
 * Class:     org_cabindb_BlockBasedTableConfig
 * Method:    newTableFactoryHandle
 * Signature: (ZZZZBBDBZJJJJIIIJZZZJZZIIZZBJIJI)J
 */
jlong Java_org_cabindb_BlockBasedTableConfig_newTableFactoryHandle(
  ...
  CABINDB_NAMESPACE::BlockBasedTableOptions options;
  ...
  options.index_type =
      CABINDB_NAMESPACE::IndexTypeJni::toCppIndexType(jindex_type_value);
  options.data_block_index_type =
      CABINDB_NAMESPACE::DataBlockIndexTypeJni::toCppDataBlockIndexType(
          jdata_block_index_type_value);
  options.data_block_hash_table_util_ratio =
      static_cast<double>(jdata_block_hash_table_util_ratio);
  options.checksum = CABINDB_NAMESPACE::ChecksumTypeJni::toCppChecksumType(
  ...
  if (options.no_block_cache) {
    ...
    if (jblock_cache_handle > 0) {
      std::shared_ptr<CABINDB_NAMESPACE::Cache> *pCache =
      ...
      if (jblock_cache_num_shard_bits > 0) {
        options.block_cache = CABINDB_NAMESPACE::NewLRUCache(
  ...
  if (jpersistent_cache_handle > 0) {
    std::shared_ptr<CABINDB_NAMESPACE::PersistentCache> *pCache =
  ...
  if (jblock_cache_compressed_handle > 0) {
    ...
    if (jblock_cache_compressed_num_shard_bits > 0) {
      options.block_cache_compressed = CABINDB_NAMESPACE::NewLRUCache(
  ...
  if (jfilter_policy_handle > 0) {
    std::shared_ptr<CABINDB_NAMESPACE::FilterPolicy> *pFilterPolicy =
        reinterpret_cast<std::shared_ptr<CABINDB_NAMESPACE::FilterPolicy> *>(
  ...
  options.index_shortening =
      CABINDB_NAMESPACE::IndexShorteningModeJni::toCppIndexShorteningMode(
          jindex_shortening);

  return reinterpret_cast<jlong>(
      CABINDB_NAMESPACE::NewBlockBasedTableFactory(options));
}
