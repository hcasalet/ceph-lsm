...
// CABINDB_NAMESPACE::WriteBatch::Handler.

#ifndef JAVA_CABINJNI_WRITEBATCHHANDLERJNICALLBACK_H_
#define JAVA_CABINJNI_WRITEBATCHHANDLERJNICALLBACK_H_
 ...
 * called back from the CabinDB storage engine (C++)
...
class WriteBatchHandlerJniCallback : public JniCallback, public WriteBatch::Handler {
 ...
 private:
    ...
    /**
     * @return A pointer to a CABINDB_NAMESPACE::Status or nullptr if an
     * unexpected exception occurred
     */
    std::unique_ptr<CABINDB_NAMESPACE::Status> kv_op(
     ...
     * unexpected exception occurred
     */
    std::unique_ptr<CABINDB_NAMESPACE::Status> k_op(
        const Slice& key, std::function<void(jbyteArray)> kFn);
};
}  // namespace CABINDB_NAMESPACE

#endif  // JAVA_CABINJNI_WRITEBATCHHANDLERJNICALLBACK_H_
