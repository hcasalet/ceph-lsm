...
import java.util.concurrent.TimeUnit;
import org.cabindb.*;
import org.cabindb.CabinMemEnv;
import org.cabindb.util.SizeUnit;
...
public class DbBenchmark {
  ...
  abstract class BenchmarkTask implements Callable<Stats> {
    ...
    @Override public Stats call() throws CabinDBException {
    ...
    abstract protected void runTask() throws CabinDBException;
  ...
  abstract class WriteTask extends BenchmarkTask {
    ...
    @Override public void runTask() throws CabinDBException {
  ...
  class ReadSequentialTask extends BenchmarkTask {
    ...
    @Override public void runTask() throws CabinDBException {
      CabinIterator iter = db_.newIterator();
      long i;
  ...
  private void prepareOptions(Options options) throws CabinDBException {
    if (!useExisting_) {
    ...
    if (useMemenv_) {
      options.setEnv(new CabinMemEnv(Env.getDefault()));
    }
    switch (memtable_) {
      case "skip_list":
    ...
    // TODO(yhchiang): add CabinDB.openForReadOnly() to enable Flag.readonly
  ...
  private void run() throws CabinDBException {
    if (!useExisting_) {
      destroyDb();
    }
  ...
  private void open(Options options) throws CabinDBException {
    System.out.println("Using database directory: " + databaseDir_);
    db_ = CabinDB.open(options, databaseDir_);
  }
  ...
  private enum Flag {
    ...
    /* TODO(yhchiang): enable the following
    direct_reads(cabindb::EnvOptions().use_direct_reads,
        "Allow direct I/O reads.") {
      @Override public Object parseValue(String value) {
        return parseBoolean(value);
      }
      },
    direct_writes(cabindb::EnvOptions().use_direct_reads,
      "Allow direct I/O reads.") {
      @Override public Object parseValue(String value) {
      return parseBoolean(value);
      }
    ...
    */
    ...
    db(getTempDir("cabindb-jni"),
       "Use the db with the following name.") {
      @Override public Object parseValue(String value) {
        return value;
      }
    },
    use_mem_env(false, "Use CabinMemEnv instead of default filesystem based\n" +
        "environment.") {
      @Override public Object parseValue(String value) {
        return parseBoolean(value);
      }
