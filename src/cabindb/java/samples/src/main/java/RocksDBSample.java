...
public class CabinDBSample {
  ...
  public static void main(final String[] args) {
    if (args.length < 1) {
      System.out.println("usage: CabinDBSample db_path");
      System.exit(-1);
    }
    ...
    System.out.println("CabinDBSample");
    try (final Options options = new Options();
      ...
      try (final CabinDB db = CabinDB.open(options, db_path_not_found)) {
        assert (false);
      ...
      try (final CabinDB db = CabinDB.open(options, db_path)) {
        db.put("hello".getBytes(), "world".getBytes());

        final byte[] value = db.get("hello".getBytes());
        assert ("world".equals(new String(value)));

        final String str = db.getProperty("cabindb.stats");
