...
public class CabinDBColumnFamilySample {
  static {
    CabinDB.loadLibrary();
  }

  public static void main(final String[] args) throws CabinDBException {
    if (args.length < 1) {
      System.out.println(
          "usage: CabinDBColumnFamilySample db_path");
      System.exit(-1);
    }

    final String db_path = args[0];

    System.out.println("CabinDBColumnFamilySample");
    try(final Options options = new Options().setCreateIfMissing(true);
        final CabinDB db = CabinDB.open(options, db_path)) {
    ...
    columnFamilyDescriptors.add(new ColumnFamilyDescriptor(
        CabinDB.DEFAULT_COLUMN_FAMILY, new ColumnFamilyOptions()));
