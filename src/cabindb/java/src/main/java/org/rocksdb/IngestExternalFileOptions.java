 ...
 * {@link CabinDB#ingestExternalFile(ColumnFamilyHandle, List, IngestExternalFileOptions)}.
 */
public class IngestExternalFileOptions extends CabinObject {
  ...
  /**
   * If set to false, {@link CabinDB#ingestExternalFile(ColumnFamilyHandle, List, IngestExternalFileOptions)}
   ...
   * in the external SST file for backward compatibility.
   *
   * Older versions of CabinDB write the global_seqno to a given offset within
   * the ingested SST files, and new versions of CabinDB do not.
   *
   * If you ingest an external SST using new version of CabinDB and would like
   * to be able to downgrade to an older version of CabinDB, you should set
   * {@link #writeGlobalSeqno()} to true.
   *
   * If your service is just starting to use the new CabinDB, we recommend that
