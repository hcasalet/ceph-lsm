 ...
 * during the creation of a {@link org.cabindb.CabinDB} (i.e., CabinDB.open()).
 *
 ...
 */
public class Options extends CabinObject
  ...
  /**
   * Construct options for opening a CabinDB.
   *
   * This constructor will create (by allocating a block of memory)
   * an {@code cabindb::Options} in the c++ side.
   */
  public Options() {
    super(newOptions());
    env_ = Env.getDefault();
  ...
  /**
   * Construct options for opening a CabinDB. Reusing database options
   * and column family options.
   *
   * @param dbOptions {@link org.cabindb.DBOptions} instance
   * @param columnFamilyOptions {@link org.cabindb.ColumnFamilyOptions}
   *     instance
   */
  ...
  public Options setListeners(final List<AbstractEventListener> listeners) {
    assert (isOwningHandle());
    setEventListeners(nativeHandle_, CabinCallbackObject.toNativeHandleList(listeners));
    return this;
  }
