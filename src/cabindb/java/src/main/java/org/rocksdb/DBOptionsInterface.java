...
public interface DBOptionsInterface<T extends DBOptionsInterface<T>> {
  ...
  /**
   * Returns the set CabinEnv instance.
   *
   * @return {@link CabinEnv} instance set in the options.
   */
  Env getEnv();

  /**
   * <p>By default, CabinDB uses only one background thread for flush and
   * compaction. Calling this function will set it up such that total of
   * `total_threads` is used.</p>
   *
   * <p>You almost definitely want to call this function if your system is
   * bottlenecked by CabinDB.</p>
   *
   * @param totalThreads The total number of threads to be used by CabinDB.
   ...
   * if it is missing during {@code CabinDB.open()}.
   * Default: false
   *
   * @param flag a flag indicating whether to create a database the
   *     specified database in {@link CabinDB#open(org.cabindb.Options, String)} operation
   *     is missing.
   * @return the instance of the current Options
   * @see CabinDB#open(org.cabindb.Options, String)
   */
  ...
  /**
   * If true, an error will be thrown during CabinDB.open() if the
   * database already exists.
   * Default: false
   *
   * @param errorIfExists if true, an exception will be thrown
   *     during {@code CabinDB.open()} if the database already exists.
   ...
   * Default: nullptr
   *
   * @param rateLimiter {@link org.cabindb.RateLimiter} instance.
   * @return the instance of the current object.
   *
   * @since 3.10.0
   */
  ...
  /**
   * <p>Sets the CabinDB log level. Default level is INFO</p>
   *
   ...
   * @return the instance of the current object.
   *
   * @see CabinEnv#setBackgroundThreads(int)
   * @see CabinEnv#setBackgroundThreads(int, Priority)
   ...
   *
   * Check <a href="https://github.com/facebook/cabindb/wiki/Write-Buffer-Manager">
   *     https://github.com/facebook/cabindb/wiki/Write-Buffer-Manager</a>
   ...
   * will be called when specific CabinDB event happens.
   *
   * Note: the CabinJava API currently only supports EventListeners implemented in Java.
  ...
  /**
   * If true, then {@link CabinDB#open(String)} will not fetch and check sizes of all sst files.
   ...
   *                                         when calling {@link CabinDB#open(String)}.
   * @return the reference to the current options.
   */
   ...
   * Default: false
   *
   * @return true, if file sizes will not be checked when calling {@link CabinDB#open(String)}.
   */
   ...
   * flush atomically in {@link CabinDB#flush(FlushOptions, List)}.
   ...
   * Default: false
   *
   * @param bestEffortsRecovery if true, CabinDB will use best-efforts mode when recovering.
   * @return the instance of the current object.
   */
   ...
   * Default: false
   *
   * @return true, if CabinDB uses best-efforts mode when recovering.
   */
