 ...
 * be called when specific CabinDB event happens such as flush.  It can
 * be used as a building block for developing custom features such as
 * stats-collector or external compaction algorithm.
 *
 * Note that callback functions should not run for an extended period of
 * time before the function returns, otherwise CabinDB may be blocked.
 * For example, it is not suggested to do
 * {@link CabinDB#compactFiles(CompactionOptions, ColumnFamilyHandle, List, int, int,
 * CompactionJobInfo)} (as it may run for a long while) or issue many of
 * {@link CabinDB#put(ColumnFamilyHandle, WriteOptions, byte[], byte[])}
 * (as Put may be blocked in certain cases) in the same thread in the
 * EventListener callback.
 *
 * However, doing
 * {@link CabinDB#compactFiles(CompactionOptions, ColumnFamilyHandle, List, int, int,
 * CompactionJobInfo)} and {@link CabinDB#put(ColumnFamilyHandle, WriteOptions, byte[], byte[])} in
 * another thread is considered safe.
 *
 * [Threading] All EventListener callback will be called using the
 * actual thread that involves in that specific event. For example, it
 * is the CabinDB background flush thread that does the actual flush to
 * call {@link #onFlushCompleted(CabinDB, FlushJobInfo)}.
 *
...
public interface EventListener {
  /**
   * A callback function to CabinDB which will be called before a
   * CabinDB starts to flush memtables.
   *
   * Note that the this function must be implemented in a way such that
   * it should not run for an extended period of time before the function
   * returns. Otherwise, CabinDB may be blocked.
   *
   * @param db the database
   * @param flushJobInfo the flush job info, contains data copied from
   *     respective native structure.
   */
  void onFlushBegin(final CabinDB db, final FlushJobInfo flushJobInfo);

  /**
   * callback function to CabinDB which will be called whenever a
   * registered CabinDB flushes a file.
   *
   * Note that the this function must be implemented in a way such that
   * it should not run for an extended period of time before the function
   * returns. Otherwise, CabinDB may be blocked.
   ...
   *     respective native structure.
   */
  void onFlushCompleted(final CabinDB db, final FlushJobInfo flushJobInfo);

  /**
   * A callback function for CabinDB which will be called whenever
   * a SST file is deleted. Different from
   * {@link #onCompactionCompleted(CabinDB, CompactionJobInfo)} and
   * {@link #onFlushCompleted(CabinDB, FlushJobInfo)},
   * this callback is designed for external logging
   * service and thus only provide string parameters instead
   * of a pointer to DB.  Applications that build logic basic based
   * on file creations and deletions is suggested to implement
   * {@link #onFlushCompleted(CabinDB, FlushJobInfo)} and
   * {@link #onCompactionCompleted(CabinDB, CompactionJobInfo)}.
   *
  ...
  /**
   * A callback function to CabinDB which will be called before a
   * CabinDB starts to compact. The default implementation is
   * no-op.
   *
   * Note that the this function must be implemented in a way such that
   * it should not run for an extended period of time before the function
   * returns. Otherwise, CabinDB may be blocked.
   *
   * @param db a pointer to the cabindb instance which just compacted
   *     a file.
   ...
   */
  void onCompactionBegin(final CabinDB db, final CompactionJobInfo compactionJobInfo);

  /**
   * A callback function for CabinDB which will be called whenever
   * a registered CabinDB compacts a file. The default implementation
   * is a no-op.
   *
   ...
   *     if it is needed outside of this function.
   */
  void onCompactionCompleted(final CabinDB db, final CompactionJobInfo compactionJobInfo);
  ...
  /**
   * A callback function for CabinDB which will be called before
  ...
  /**
   * A callback function for CabinDB which will be called after an external
  ...
  void onExternalFileIngested(
      final CabinDB db, final ExternalFileIngestionInfo externalFileIngestionInfo);

  /**
   * A callback function for CabinDB which will be called before setting the
  ...
  /**
   * A callback function for CabinDB which will be called whenever a change
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file read
   * operation finishes.
   *
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file write
   * operation finishes.
   *
   * @param fileOperationInfo file operation info,
   *     contains data copied from respective native structure.
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file flush
   * operation finishes.
   *
   * @param fileOperationInfo file operation info,
   *     contains data copied from respective native structure.
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file sync
   * operation finishes.
   *
   * @param fileOperationInfo file operation info,
   *     contains data copied from respective native structure.
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file
  ...
  /**
   * A callback function for CabinDB which will be called whenever a file close
   * operation finishes.
   *
   * @param fileOperationInfo file operation info,
   *     contains data copied from respective native structure.
  ...
  /**
   * A callback function for CabinDB which will be called just before
  ...
  /**
   * A callback function for CabinDB which will be called once the database
