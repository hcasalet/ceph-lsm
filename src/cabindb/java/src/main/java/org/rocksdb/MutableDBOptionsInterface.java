...
public interface MutableDBOptionsInterface<T extends MutableDBOptionsInterface<T>> {
   ...
   *     {@link CabinDB#setOptions(ColumnFamilyHandle, MutableColumnFamilyOptions)}
   *     API.
   *
   ...
   * compression. CabinDB may decide to slow down more if the compaction still
   * gets behind further.
   ...
   * Default: 0
   *
   * Dynamically changeable through {@link CabinDB#setDBOptions(MutableDBOptions)}.
   *
  ...
  /**
   * If not zero, dump cabindb.stats to CabinDB every
