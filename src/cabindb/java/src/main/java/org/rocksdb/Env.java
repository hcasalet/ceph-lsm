...
/**
 * Base class for all Env implementations in CabinDB.
 */
public abstract class Env extends CabinObject {

  static {
    CabinDB.loadLibrary();
  }

  private static final Env DEFAULT_ENV = new CabinEnv(getDefaultEnvInternal());
  static {
    /**
   ...
   * belongs to cabindb c++.  As a result, the returned CabinEnv will not
   * have the ownership of its c++ resource, and calling its dispose()/close()
   * will be no-op.</p>
   *
   * @return the default {@link org.cabindb.CabinEnv} instance.
   */
   ...
   *
   * @return current {@link CabinEnv} instance.
   */
   ...
   *
   * @throws CabinDBException if the thread list cannot be acquired.
   */
  public List<ThreadStatus> getThreadList() throws CabinDBException {
