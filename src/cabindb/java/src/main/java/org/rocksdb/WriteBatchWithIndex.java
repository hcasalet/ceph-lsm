...
/**
 * Similar to {@link org.cabindb.WriteBatch} but with a binary searchable
 * index built for all the keys inserted.
 *
 * Calling put, merge, remove or putLogData calls the same function
 * as with {@link org.cabindb.WriteBatch} whilst also building an index.
 *
 * A user can call {@link org.cabindb.WriteBatchWithIndex#newIterator()} to
 * create an iterator over the write batch or
 * {@link org.cabindb.WriteBatchWithIndex#newIteratorWithBase(org.cabindb.CabinIterator)}
...
public class WriteBatchWithIndex extends AbstractWriteBatch {
   ...
   * {@link org.cabindb.CabinIteratorInterface#seek(byte[])} to
   ...
   * family
   */
  public WBWICabinIterator newIterator(
      final ColumnFamilyHandle columnFamilyHandle) {
    return new WBWICabinIterator(this, iterator1(nativeHandle_,
   ...
   */
  public WBWICabinIterator newIterator() {
    return new WBWICabinIterator(this, iterator0(nativeHandle_));
  }

  /**
   * Provides Read-Your-Own-Writes like functionality by
   * creating a new Iterator that will use {@link org.cabindb.WBWICabinIterator}
   ...
   *   e.g. {@link org.cabindb.CabinDB#newIterator()}
   ...
   */
  public CabinIterator newIteratorWithBase(
      final ColumnFamilyHandle columnFamilyHandle,
      final CabinIterator baseIterator) {
   ...
   * point-in-time from baseIterator and modifications made in this write batch.
   */
  public CabinIterator newIteratorWithBase(final ColumnFamilyHandle columnFamilyHandle,
      final CabinIterator baseIterator, /* @Nullable */ final ReadOptions readOptions) {
    final CabinIterator iterator = new CabinIterator(baseIterator.parent_,
   ...
   */
  public CabinIterator newIteratorWithBase(final CabinIterator baseIterator) {
  ...
  /**
   * Similar to {@link CabinDB#get(ColumnFamilyHandle, byte[])} but will only
   ...
   *
   * @throws CabinDBException if the batch does not have enough data to resolve
  ...
  public byte[] getFromBatch(final ColumnFamilyHandle columnFamilyHandle,
      final DBOptions options, final byte[] key) throws CabinDBException {
    return getFromBatch(nativeHandle_, options.nativeHandle_,
        key, key.length, columnFamilyHandle.nativeHandle_);
  }

  /**
   * Similar to {@link CabinDB#get(byte[])} but will only
   * read the key from this batch.
   *
   * @param options The database options to use
   * @param key The key to read the value for
  ...
  /**
   * Similar to {@link CabinDB#get(ColumnFamilyHandle, byte[])} but will also
   ...
   *     any. null if it does not find the specified key.
   *
   * @throws CabinDBException if the value for the key cannot be read
   */
  public byte[] getFromBatchAndDB(final CabinDB db, final ColumnFamilyHandle columnFamilyHandle,
      final ReadOptions options, final byte[] key) throws CabinDBException {
  ...
  /**
   * Similar to {@link CabinDB#get(byte[])} but will also
   * read writes from this batch.
   *
   * This function will query both this batch and the DB and then merge
   * the results using the DB's merge operator (if the batch contains any
   ...
   * @throws CabinDBException if the value for the key cannot be read
   */
  public byte[] getFromBatchAndDB(final CabinDB db, final ReadOptions options,
  ...
  @Override final native void popSavePoint(final long handle) throws CabinDBException;
