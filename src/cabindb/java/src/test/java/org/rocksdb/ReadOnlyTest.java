...
public class ReadOnlyTest {
  ...
  @Test
  public void readOnlyOpen() throws CabinDBException {
    ...
    try (final CabinDB db = CabinDB.openReadOnly(dbFolder.getRoot().getAbsolutePath())) {
      assertThat("value").isEqualTo(new String(db.get("key".getBytes())));
    }

    try (final ColumnFamilyOptions cfOpts = new ColumnFamilyOptions()) {
      final List<ColumnFamilyDescriptor> cfDescriptors = new ArrayList<>();
      cfDescriptors.add(new ColumnFamilyDescriptor(CabinDB.DEFAULT_COLUMN_FAMILY, cfOpts));
      ...
      try (final CabinDB db = CabinDB.openReadOnly(
               dbFolder.getRoot().getAbsolutePath(), cfDescriptors, columnFamilyHandleList)) {
  ...
  public void failToWriteInReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true)) {
      try (final CabinDB db = CabinDB.open(options, dbFolder.getRoot().getAbsolutePath())) {
        // no-op
      }
    }
    ...
    try (final ColumnFamilyOptions cfOpts = new ColumnFamilyOptions()) {
      final List<ColumnFamilyDescriptor> cfDescriptors =
          Arrays.asList(new ColumnFamilyDescriptor(CabinDB.DEFAULT_COLUMN_FAMILY, cfOpts));

      final List<ColumnFamilyHandle> readOnlyColumnFamilyHandleList = new ArrayList<>();
      try (final CabinDB rDb = CabinDB.openReadOnly(dbFolder.getRoot().getAbsolutePath(),
  ...
  @Test(expected = CabinDBException.class)
  public void failToCFWriteInReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      //no-op
    ...
    try (final ColumnFamilyOptions cfOpts = new ColumnFamilyOptions()) {
      final List<ColumnFamilyDescriptor> cfDescriptors = Arrays.asList(
          new ColumnFamilyDescriptor(CabinDB.DEFAULT_COLUMN_FAMILY, cfOpts)
      );
      final List<ColumnFamilyHandle> readOnlyColumnFamilyHandleList =
          new ArrayList<>();
      try (final CabinDB rDb = CabinDB.openReadOnly(
  ...
  @Test(expected = CabinDBException.class)
  public void failToRemoveInReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      //no-op
  ...
  @Test(expected = CabinDBException.class)
  public void failToCFRemoveInReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      //no-op
  ...
  @Test(expected = CabinDBException.class)
  public void failToWriteBatchReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      //no-op
  ...
  @Test(expected = CabinDBException.class)
  public void failToCFWriteBatchReadOnly() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      //no-op
  ...
  @Test(expected = CabinDBException.class)
  public void errorIfWalFileExists() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
    ...
    try (final ColumnFamilyOptions cfOpts = new ColumnFamilyOptions()) {
      ...
      try (final DBOptions options = new DBOptions();
           final CabinDB rDb = CabinDB.openReadOnly(options, dbFolder.getRoot().getAbsolutePath(),
