...
public class MemoryUtilTest {
  ...
  private static final String MEMTABLE_SIZE = "cabindb.size-all-mem-tables";
  private static final String UNFLUSHED_MEMTABLE_SIZE = "cabindb.cur-size-all-mem-tables";
  private static final String TABLE_READERS = "cabindb.estimate-table-readers-mem";
   ...
   */
  @Test
  public void getApproximateMemoryUsageByType() throws CabinDBException {
    try (final Cache cache = new LRUCache(8 * 1024 * 1024);
         ...
         final CabinDB db =
                 CabinDB.open(options, dbFolder1.getRoot().getAbsolutePath())) {

      List<CabinDB> dbs = new ArrayList<>(1);
      dbs.add(db);
   ...
   */
  @Test
  public void getApproximateMemoryUsageByTypeNulls() throws CabinDBException {
   ...
   */
  @Test
  public void getApproximateMemoryUsageByTypeMultiple() throws CabinDBException {
    try (final Cache cache1 = new LRUCache(1 * 1024 * 1024);
         ...
         final CabinDB db1 =
                 CabinDB.open(options1, dbFolder1.getRoot().getAbsolutePath());
         ...
         final CabinDB db2 =
                 CabinDB.open(options2, dbFolder2.getRoot().getAbsolutePath());
         final FlushOptions flushOptions =
                 new FlushOptions().setWaitForFlush(true);
