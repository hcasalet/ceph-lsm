...
public class WriteBatchWithIndexTest {
  ...
  @Test
  public void readYourOwnWrites() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
      ...
      try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex(true);
           final CabinIterator base = db.newIterator();
           final CabinIterator it = wbwi.newIteratorWithBase(base)) {
        it.seek(k1);
  ...
  @Test
  public void readYourOwnWritesCf() throws CabinDBException {
    ...
    try (final DBOptions options =
      ...
      try {
        ...
        try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex(true);
             ...
             final CabinIterator base = db.newIterator(newCf, readOptions);
             final CabinIterator it = wbwi.newIteratorWithBase(newCf, base, readOptions)) {
          it.seek(k1);
          assertThat(it.isValid()).isTrue();
          assertThat(it.key()).isEqualTo(k1);
          assertThat(it.value()).isEqualTo(v1);
  ...
  @Test
  public void writeBatchWithIndex() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
  ...
  @Test
  public void write_writeBatchWithIndexDirect() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
  ...
  @Test
  public void iterator() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex(true)) {
      ...
      final WBWICabinIterator.WriteEntry[] expected = {
          new WBWICabinIterator.WriteEntry(WBWICabinIterator.WriteType.PUT,
              new DirectSlice(k1), new DirectSlice(v1)),
          new WBWICabinIterator.WriteEntry(WBWICabinIterator.WriteType.PUT,
              new DirectSlice(k2), new DirectSlice(v2)),
          new WBWICabinIterator.WriteEntry(WBWICabinIterator.WriteType.PUT,
              new DirectSlice(k3), new DirectSlice(v3)),
          new WBWICabinIterator.WriteEntry(WBWICabinIterator.WriteType.DELETE,
              new DirectSlice(k4), DirectSlice.NONE),
          new WBWICabinIterator.WriteEntry(WBWICabinIterator.WriteType.SINGLE_DELETE,
              new DirectSlice(k5), DirectSlice.NONE),
      };

      try (final WBWICabinIterator it = wbwi.newIterator()) {
        ...
        for (int i = 0; i < testOffsets.length; i++) {
          ...
          final WBWICabinIterator.WriteEntry entry = it.entry();
  ...
  @Test
  public void zeroByteTests() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex(true)) {
      ...
      try (final WBWICabinIterator it = wbwi.newIterator()) {
        it.seekToFirst();
        final WBWICabinIterator.WriteEntry actual = it.entry();
  ...
  @Test
  public void savePoints() throws CabinDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
         final CabinDB db = CabinDB.open(options,
             dbFolder.getRoot().getAbsolutePath())) {
      try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex(true);
  ...
  @Test
  public void restorePoints() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
  ...
  public void restorePoints_withoutSavePoints() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
      wbwi.rollbackToSavePoint();
    }
  }
  ...
  @Test(expected = CabinDBException.class)
  public void restorePoints_withoutSavePoints_nested() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
  ...
  @Test(expected = CabinDBException.class)
  public void popSavePoint_withoutSavePoints() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
      wbwi.popSavePoint();
    }
  }
  ...
  @Test(expected = CabinDBException.class)
  public void popSavePoint_withoutSavePoints_nested() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
  ...
  @Test
  public void maxBytes() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
      wbwi.setMaxBytes(19);
  ...
  @Test(expected = CabinDBException.class)
  public void maxBytes_over() throws CabinDBException {
    try (final WriteBatchWithIndex wbwi = new WriteBatchWithIndex()) {
      wbwi.setMaxBytes(1);
  ...
  private static String getFromWriteBatchWithIndex(final CabinDB db,
      final ReadOptions readOptions, final WriteBatchWithIndex wbwi,
      final String skey) {
    final byte[] key = skey.getBytes();
    try (final CabinIterator baseIterator = db.newIterator(readOptions);
         final CabinIterator iterator = wbwi.newIteratorWithBase(baseIterator)) {
      iterator.seek(key);

      // Arrays.equals(key, iterator.key()) ensures an exact match in Cabin,
  ...
  @Test
  public void getFromBatch() throws CabinDBException {
  ...
  @Test
  public void getFromBatchAndDB() throws CabinDBException {
    final byte[] k1 = "k1".getBytes();
    final byte[] k2 = "k2".getBytes();
    final byte[] k3 = "k3".getBytes();
    final byte[] k4 = "k4".getBytes();
