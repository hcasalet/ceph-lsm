...
public class WriteBatchThreadedTest {
  ...
  public void setUp() throws Exception {
    ...
    db = CabinDB.open(options, dbFolder.getRoot().getAbsolutePath());
    assert (db != null);
  }
  ...
  public void threadedWrites() throws InterruptedException, ExecutionException {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      callables.add(new Callable<Void>() {
        @Override
        public Void call() throws CabinDBException {
          try (final WriteBatch wb = new WriteBatch();
               final WriteOptions w_opt = new WriteOptions()) {
            for (int i = offset; i < offset + 100; i++) {
              wb.put(ByteBuffer.allocate(4).putInt(i).array(), "parallel cabin test".getBytes());
            }
            db.write(w_opt, wb);
          }
          return null;
