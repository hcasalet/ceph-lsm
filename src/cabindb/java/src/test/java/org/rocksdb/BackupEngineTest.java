...
public class BackupEngineTest {
  ...
  @Test
  public void backupDb() throws CabinDBException {
    // Open empty database.
  ...
  @Test
  public void deleteBackup() throws CabinDBException {
    // Open empty database.
    try(final Options opt = new Options().setCreateIfMissing(true);
        final CabinDB db = CabinDB.open(opt,
            dbFolder.getRoot().getAbsolutePath())) {
  ...
  @Test
  public void purgeOldBackups() throws CabinDBException {
    // Open empty database.
    try(final Options opt = new Options().setCreateIfMissing(true);
        final CabinDB db = CabinDB.open(opt,
            dbFolder.getRoot().getAbsolutePath())) {
  ...
  @Test
  public void restoreLatestBackup() throws CabinDBException {
    try(final Options opt = new Options().setCreateIfMissing(true)) {
      // Open empty database.
      CabinDB db = null;
      try {
        ...
        try (final BackupableDBOptions bopt = new BackupableDBOptions(
          ...
          // Open database again.
          db = CabinDB.open(opt, dbFolder.getRoot().getAbsolutePath());
  ...
  @Test
  public void backupDbWithMetadata() throws CabinDBException {
    // Open empty database.
    try (final Options opt = new Options().setCreateIfMissing(true);
  ...
  private List<BackupInfo> verifyNumberOfValidBackups(final BackupEngine be,
      final int expectedNumberOfBackups) throws CabinDBException {
   ...
   *     part of the library.
   */
  private void prepareDatabase(final CabinDB db)
      throws CabinDBException {
