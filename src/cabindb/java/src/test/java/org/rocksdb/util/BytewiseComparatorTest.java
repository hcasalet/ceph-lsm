...
import static org.cabindb.util.ByteUtil.bytes;

/**
 * This is a direct port of various C++
 * tests from db/comparator_db_test.cc
 * and some code to adapt it to CabinJava
 */
public class BytewiseComparatorTest {
  ...
  public void java_vs_cpp_bytewiseComparator()
      throws IOException, CabinDBException {
    for(int rand_seed = 301; rand_seed < 306; rand_seed++) {
      final Path dbDir =
          FileSystems.getDefault().getPath(dbFolder.newFolder().getAbsolutePath());
      try(final CabinDB db = openDatabase(dbDir,
  ...
  public void java_vs_java_bytewiseComparator()
    ...
    for(int rand_seed = 301; rand_seed < 306; rand_seed++) {
      ...
      try(final ComparatorOptions copt = new ComparatorOptions()
          ...
          final CabinDB db = openDatabase(dbDir, comparator)) {
  ...
  private void doRandomIterationTest(
      final CabinDB db, final java.util.Comparator<String> javaComparator,
      final Random rnd,
      final int num_writes, final int num_iter_ops,
      final int num_trigger_flush) throws CabinDBException {
    ...
    try (final ReadOptions readOptions = new ReadOptions();
         final CabinIterator iter = db.newIterator(readOptions)) {
  ...
  private java.util.Comparator<String> toJavaComparator(
    ...
    return new java.util.Comparator<String>() {
      ...
      public int compare(final String s1, final String s2) {
        ...
        bufS2.put(bytes(s2));
        bufS2.flip();
        return cabinComparator.compare(bufS1, bufS2);
      }
    };
  }

  private static class KVIter<K, V> implements CabinIteratorInterface {
    ...
    public void status() throws CabinDBException {
      if(offset < 0 || offset >= entries.size()) {
        throw new CabinDBException("Index out of bounds. Size is: " +
