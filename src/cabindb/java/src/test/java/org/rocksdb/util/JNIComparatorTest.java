...
public class JNIComparatorTest {
  ...
  @Test
  public void java_comparator_equals_cpp_comparator() throws CabinDBException, IOException {
    final int[] javaKeys;
  ...
  private void storeWithJavaComparator(final Path dir,
    ...
    try (final Options options = new Options()
         ...
         final CabinDB db =
             CabinDB.open(options, dir.toAbsolutePath().toString())) {
  ...
  private void storeWithCppComparator(final Path dir,
      final BuiltinComparator builtinComparator) throws CabinDBException {
    try (final Options options = new Options()
             .setCreateIfMissing(true)
  ...
  private int[] readAllWithJavaComparator(final Path dir,
    ...
    try (final Options options = new Options()
      ...
      try (final CabinIterator it = db.newIterator()) {
        it.seekToFirst();
  ...
  private int[] readAllWithCppComparator(final Path dir,
      final BuiltinComparator comparator) throws CabinDBException {
    try (final Options options = new Options()
        .setCreateIfMissing(true)
        .setComparator(comparator);
         final CabinDB db =
