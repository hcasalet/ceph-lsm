  ...
  Status Run() {
    ...
    if (status.ok()) {
      ...
      CABIN_LOG_WARN(db_options_.info_log,
                     "**** Repaired cabindb %s; "
                     "recovered %" CABINDB_PRIszt " files; %" PRIu64
                     " bytes. "
 ...
 private:
  ...
  Status ConvertLogToTable(uint64_t log) {
    struct LogReporter : public log::Reader::Reporter {
      ...
      void Corruption(size_t bytes, const Status& s) override {
        ...
        CABIN_LOG_ERROR(info_log, "Log #%" PRIu64 ": dropping %d bytes; %s",
    ...
    while (reader.ReadRecord(&record, &scratch)) {
      ...
      if (record_status.ok()) {
        ...
        CABIN_LOG_WARN(db_options_.info_log, "Log #%" PRIu64 ": ignoring %s",
  ...
  void ExtractMetaData() {
    for (size_t i = 0; i < table_fds_.size(); i++) {
      ...
      if (!status.ok()) {
        ...
        CABIN_LOG_WARN(db_options_.info_log, "Table #%s: ignoring %s",
  ...
  Status ScanTable(TableInfo* t) {
    ...
    if (status.ok()) {
      ...
      CABIN_LOG_INFO(db_options_.info_log, "Table #%" PRIu64 ": %d entries %s",
  ...
  void ArchiveFile(const std::string& fname) {
    ...
    CABIN_LOG_INFO(db_options_.info_log, "Archiving %s: %s\n", fname.c_str(),
                   s.ToString().c_str());
  }
};
