...
#include "include/cabindb/iostats_context.h"
...
TEST_P(DBIteratorTest, IteratorProperty) {
  ...
  ropt.pin_data = false;
  {
    ...
    ASSERT_OK(iter->GetProperty("cabindb.iterator.is-key-pinned", &prop_value));
    ASSERT_EQ("0", prop_value);
    ASSERT_OK(iter->GetProperty("cabindb.iterator.internal-key", &prop_value));
...
// SetOptions not defined in CABINDB LITE
#ifndef CABINDB_LITE
  ...
  void PinnedDataIteratorRandomized(TestConfig run_config) {
...

    {
      ...
      for (auto& k : random_keys) {
        ...
        std::string prop_value;
        ASSERT_OK(
            iter->GetProperty("cabindb.iterator.is-key-pinned", &prop_value));
        ASSERT_EQ("1", prop_value);
...
TEST_P(DBIteratorTest, DBIteratorSkipRecentDuplicatesTest) {
  ...
  std::string val;
  ASSERT_TRUE(db_->GetProperty("cabindb.num-files-at-level0", &val));
  EXPECT_EQ("0", val);
#endif
...
// MyCabin may change iterate bounds before seek. Simply test to make sure such
