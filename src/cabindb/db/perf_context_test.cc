...
const std::string kDbName =
    CABINDB_NAMESPACE::test::PerThreadDBPath("perf_context_test");
...
std::shared_ptr<DB> OpenDb(bool read_only = false) {
    ...
    if (FLAGS_use_set_based_memetable) {
      ...
      options.prefix_extractor.reset(
          CABINDB_NAMESPACE::NewFixedPrefixTransform(0));
...
// export CABINDB_TESTS=PerfContextTest.SeekKeyComparison
// For one memtable:
...
TEST_F(PerfContextTest, DBMutexLockCounter) {
  ...
  for (PerfLevel perf_level_test :
    ...
    mutex.Lock();
    CABINDB_NAMESPACE::port::Thread child_thread([&] {
...
TEST_F(PerfContextTest, CPUTimer) {
  if (Env::Default()->NowCPUNanos() == 0) {
    CABINDB_GTEST_SKIP("Target without NowCPUNanos support");
    return;
  }
