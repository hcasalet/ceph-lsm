...
TEST_F(DBSSTTest, DontDeletePendingOutputs) {
  ...
  // /tmp/cabindbtest-1552237650/db_test/000009.sst: No such file or directory
  Compact("a", "b");
}
...
TEST_F(DBSSTTest, SSTsWithLdbSuffixHandling) {
  ...
  for (size_t i = 0; i < filenames.size(); ++i) {
    ...
    std::string const ldb_name = Cabin2LevelTableFileName(rdb_name);
...
TEST_F(DBSSTTest, CancellingManualCompactionsWorks) {
  ...
  dbfull()->CompactFiles(CABINDB_NAMESPACE::CompactionOptions(), l0_files, 0);
...
TEST_F(DBSSTTest, GetTotalSstFilesSize) {
  ...
  ASSERT_TRUE(dbfull()->GetIntProperty("cabindb.total-sst-files-size",
