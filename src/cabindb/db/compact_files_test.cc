...
TEST_F(CompactFilesTest, L0ConflictsFiles) {
  ...
  CABINDB_NAMESPACE::ColumnFamilyMetaData meta;
  ...
  for (auto& file : meta.levels[0].files) {
    ...
    if (file1 == "") {
      ...
      ASSERT_OK(db->CompactFiles(CABINDB_NAMESPACE::CompactionOptions(),
                                 {file1, file2}, 0));
      break;
    }
  }
...
TEST_F(CompactFilesTest, ObsoleteFiles) {
  ...
  options.create_if_missing = true;
  // Disable CabinDB background compaction.
...
TEST_F(CompactFilesTest, CapturingPendingFiles) {
  ...
  CABINDB_NAMESPACE::port::Thread compaction_thread(
...
TEST_F(CompactFilesTest, CompactionFilterWithGetSv) {
  ...
  for (auto& file : meta.levels[0].files) {
    ...
    ASSERT_OK(
        db->CompactFiles(CABINDB_NAMESPACE::CompactionOptions(), {fname}, 0));
  }
...
TEST_F(CompactFilesTest, SentinelCompressionType) {
  ...
  for (auto compaction_style :
    ...
    CABINDB_NAMESPACE::TablePropertiesCollection all_tables_props;
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr,
          "SKIPPED as DBImpl::CompactFiles is not supported in CABINDB_LITE\n");
  return 0;
}
