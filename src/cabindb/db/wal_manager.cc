...
Status WalManager::GetSortedWalFiles(VectorLogPtr& files) {
  ...
  if (!files.empty()) {
    ...
    CABIN_LOG_INFO(db_options_.info_log, "Latest Archived log: %" PRIu64,
  ...
  for (auto& log : logs) {
    if (log->LogNumber() > latest_archived_log_number) {
      ...
      CABIN_LOG_WARN(db_options_.info_log, "%s already moved to archive",
                     log->PathName().c_str());
    }
  }
...
void WalManager::PurgeObsoleteWALFiles() {
  ...
  if (!s.ok()) {
    CABIN_LOG_ERROR(db_options_.info_log, "Can't get current time: %s",
                    s.ToString().c_str());
    assert(false);
    return;
  }
  ...
  if (!s.ok()) {
    CABIN_LOG_ERROR(db_options_.info_log, "Can't get archive files: %s",
                    s.ToString().c_str());
    assert(false);
    return;
  }
    ...
    if (ParseFileName(f, &number, &type) && type == kWalFile) {
      ...
      if (ttl_enabled) {
        ...
        if (now_seconds - file_m_time > db_options_.wal_ttl_seconds) {
          ...
          if (!s.ok()) {
            CABIN_LOG_WARN(db_options_.info_log, "Can't delete file: %s: %s",
  ...
  for (size_t i = 0; i < files_del_num; ++i) {
    ...
    s = DeleteDBFile(&db_options_, db_options_.wal_dir + "/" + file_path,
                     ...
                     /*force_fg=*/!wal_in_db_path_);
    if (!s.ok()) {
      CABIN_LOG_WARN(db_options_.info_log, "Unable to delete file: %s: %s",
                     file_path.c_str(), s.ToString().c_str());
      continue;
    } else {
      MutexLock l(&read_first_record_cache_mutex_);
...
void WalManager::ArchiveWALFile(const std::string& fname, uint64_t number) {
  ...
  CABIN_LOG_INFO(db_options_.info_log, "Move log file %s to %s -- %s\n",
...
Status WalManager::ReadFirstRecord(const WalFileType type,
  ...
  *sequence = 0;
  if (type != kAliveLogFile && type != kArchivedLogFile) {
    CABIN_LOG_ERROR(db_options_.info_log, "[WalManger] Unknown file type %s",
                    ToString(type).c_str());
...
Status WalManager::ReadFirstLine(const std::string& fname,
  ...
  struct LogReporter : public log::Reader::Reporter {
    ...
    void Corruption(size_t bytes, const Status& s) override {
      CABIN_LOG_WARN(info_log, "[WalManager] %s%s: dropping %d bytes; %s",
