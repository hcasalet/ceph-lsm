...
class TestingWrappedBlockBasedFilterPolicy : public FilterPolicy {
 public:
  ...
  void CreateFilter(const CABINDB_NAMESPACE::Slice* keys, int n,
                    std::string* dst) const override {
    std::unique_ptr<CABINDB_NAMESPACE::Slice[]> user_keys(
        new CABINDB_NAMESPACE::Slice[n]);
    for (int i = 0; i < n; ++i) {
  ...
  bool KeyMayMatch(const CABINDB_NAMESPACE::Slice& key,
                   const CABINDB_NAMESPACE::Slice& filter) const override {
    counter_++;
 ...
 private:
  ...
  CABINDB_NAMESPACE::Slice convertKey(
      const CABINDB_NAMESPACE::Slice& key) const {
    return key;
  }
};
}  // namespace
...
TEST_F(DBBloomFilterTest, PrefixExtractorFullFilter) {
  BlockBasedTableOptions bbto;
  // Full Filter Block
  bbto.filter_policy.reset(CABINDB_NAMESPACE::NewBloomFilterPolicy(10, false));
  bbto.whole_key_filtering = false;
  ...
  // Was not added to filter but cabindb will try to read it from the filter
...
TEST_F(DBBloomFilterTest, PrefixExtractorBlockFilter) {
  BlockBasedTableOptions bbto;
  // Block Filter Block
  bbto.filter_policy.reset(CABINDB_NAMESPACE::NewBloomFilterPolicy(10, true));
...
TEST_F(DBBloomFilterTest, MemtableWholeKeyBloomFilter) {
  ...
  options.prefix_extractor.reset(
      CABINDB_NAMESPACE::NewFixedPrefixTransform(kPrefixLen));
 ...
 public:
  BloomStatsTestWithParam() {
    ...
    options_.prefix_extractor.reset(
        CABINDB_NAMESPACE::NewFixedPrefixTransform(4));
