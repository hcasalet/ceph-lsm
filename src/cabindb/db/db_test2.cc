...
TEST_P(TestReadOnlyWithCompressedCache, ReadOnlyWithCompressedCache) {
  if (use_mmap_ && !IsMemoryMappedAccessSupported()) {
    CABINDB_GTEST_SKIP("Test requires MMAP support");
    return;
  }
...
TEST_F(DBTest2, IteratorPropertyVersionNumber) {
  ...
  ASSERT_OK(
      iter1->GetProperty("cabindb.iterator.super-version-number", &prop_value));
  ...
  ASSERT_OK(
      iter2->GetProperty("cabindb.iterator.super-version-number", &prop_value));
  ...
  ASSERT_OK(
      iter3->GetProperty("cabindb.iterator.super-version-number", &prop_value));
...
TEST_F(DBTest2, WalFilterTest) {
  ...
  public:
    ...
    WalProcessingOption LogRecord(const WriteBatch& /*batch*/,
      ...
      // Filter is passed as a const object for CabinDB to not modify the
  ...
  option++) {
    ...
    case WalFilter::WalProcessingOption::kIgnoreCurrentRecord: {
      fprintf(stderr,
        "Testing with ignoring record %" CABINDB_PRIszt " only\n",
        apply_option_for_record_index);
    ...
    case WalFilter::WalProcessingOption::kStopReplay: {
      fprintf(stderr,
        "Testing with stopping replay from record %" CABINDB_PRIszt
        "\n",
        apply_option_for_record_index);
 ...
 public:
  ...
  void OnCompactionCompleted(DB* db, const CompactionJobInfo& ci) override {
    ...
    for (int level = 0; level < db->NumberLevels(); level++) {
      ...
      ASSERT_TRUE(
          db->GetProperty("cabindb.num-files-at-level" + NumberToString(level),
                          &files_at_level));
      if (files_at_level != "0") {
...
TEST_F(DBTest2, CompactionStall) {
  ...
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->ClearTrace();
...
TEST_F(DBTest2, SyncPointMarker) {
  ...
  // | Thread1First  |             |
  // |               | MarkedPoint |
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->LoadDependencyAndMarkers(
...
TEST_F(DBTest2, ReadAmpBitmapLiveInCacheAfterDBClose) {
  ...
  for (size_t k = 0; k < 2; k++) {
    ...
    std::shared_ptr<Statistics> stats = CABINDB_NAMESPACE::CreateDBStatistics();
    ...
    std::shared_ptr<Statistics> new_statistics =
        CABINDB_NAMESPACE::CreateDBStatistics();
...
TEST_F(DBTest2, AutomaticCompactionOverlapManualCompaction) {
  ...
  // Also test that the stats GetMapProperty API reporting the same result
  {
    ...
    ASSERT_TRUE(dbfull()->GetMapProperty("cabindb.cfstats", &prop));
...
TEST_F(DBTest2, ManualCompactionOverlapManualCompaction) {
  ...
  CABINDB_NAMESPACE::port::Thread bg_thread;
...
TEST_F(DBTest2, PausingManualCompaction1) {
  ...
  dbfull()->CompactFiles(CABINDB_NAMESPACE::CompactionOptions(),
...
TEST_F(DBTest2, PausingManualCompaction3) {
  ...
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->ClearCallBack(
      "CompactionJob::Run():PausingManualCompaction:1");
...
TEST_F(DBTest2, IterRaceFlush1) {
  ...
  CABINDB_NAMESPACE::port::Thread t1([&] {
...
TEST_F(DBTest2, TraceAndReplay) {
  ...
  std::string trace_filename = dbname_ + "/cabindb.trace";
  ...
  ASSERT_EQ("bar", value);
  ASSERT_OK(db2->Get(ro, handles[1], "cabindb", &value));
  ASSERT_EQ("cabin", value);
...
TEST_F(DBTest2, TraceWithLimit) {
  ...
  std::string trace_filename = dbname_ + "/cabindb.trace1";
  std::unique_ptr<TraceWriter> trace_writer;
  ASSERT_OK(NewFileTraceWriter(env_, env_opts, trace_filename, &trace_writer));
  ASSERT_OK(db_->StartTrace(trace_opts, std::move(trace_writer)));
  ASSERT_OK(Put(0, "a", "1"));
...
TEST_F(DBTest2, TraceWithSampling) {
  ...
  std::string trace_filename = dbname_ + "/cabindb.trace_sampling";
  std::unique_ptr<TraceWriter> trace_writer;
  ASSERT_OK(NewFileTraceWriter(env_, env_opts, trace_filename, &trace_writer));
  ASSERT_OK(db_->StartTrace(trace_opts, std::move(trace_writer)));
  ASSERT_OK(Put(0, "a", "1"));
...
TEST_F(DBTest2, TraceWithFilter) {
  ...
  ASSERT_TRUE(db2->Get(ro, handles[0], "cabindb", &value).IsNotFound());
  ...
  std::string trace_filename3 = dbname_ + "/cabindb.trace_3";
...
TEST_F(DBTest2, PinnableSliceAndMmapReads) {
  ...
  if (!IsMemoryMappedAccessSupported()) {
    CABINDB_GTEST_SKIP("Test requires default environment");
    return;
  }
...
TEST_F(DBTest2, TestCompactFiles) {
  ...
  CABINDB_NAMESPACE::SstFileWriter sst_file_writer{
      CABINDB_NAMESPACE::EnvOptions(), options};
...
class DummyOldStats : public Statistics {
 public:
  ...
  void histogramData(
      ...
      CABINDB_NAMESPACE::HistogramData* const /*data*/) const override {}
...
TEST_F(DBTest2, BackgroundPurgeTest) {
  ...
  options.write_buffer_manager =
      std::make_shared<CABINDB_NAMESPACE::WriteBufferManager>(1 << 20);
...
#ifdef CABINDB_UNITTESTS_WITH_CUSTOM_OBJECTS_FROM_STATIC_LIBS
extern "C" {
void RegisterCustomObjects(int argc, char** argv);
}
#else
void RegisterCustomObjects(int /*argc*/, char** /*argv*/) {}
#endif  // !CABINDB_UNITTESTS_WITH_CUSTOM_OBJECTS_FROM_STATIC_LIBS
