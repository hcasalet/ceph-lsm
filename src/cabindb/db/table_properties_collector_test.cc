...
class RegularKeysStartWithABackwardCompatible
 ...
 public:
  ...
  Status Finish(UserCollectedProperties* properties) override {
    std::string encoded;
    PutVarint32(&encoded, count_);
    *properties = UserCollectedProperties{{"TablePropertiesTest", "Cabindb"},
                                          {"Count", encoded}};
    return Status::OK();
  }
...
void TestCustomizedTablePropertiesCollector(
  ...
  ASSERT_EQ("Cabindb", user_collected.at("TablePropertiesTest"));
...
#ifndef CABINDB_LITE  // PlainTable is not supported in Lite
    // test plain table
