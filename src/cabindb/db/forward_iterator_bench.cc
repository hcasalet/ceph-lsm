...
struct ShardState {
  ...
  std::unique_ptr<CABINDB_NAMESPACE::Iterator> it;
  std::unique_ptr<CABINDB_NAMESPACE::Iterator> it_cacheonly;
  Key upper_bound;
  CABINDB_NAMESPACE::Slice upper_bound_slice;
  char pad3[128] __attribute__((__unused__));
};
...
struct Reader {
  ...
  void readOnceFromShard(uint64_t shard) {
    ...
    if (!state.it) {
      ...
      if (FLAGS_cache_only_first) {
        options.read_tier = CABINDB_NAMESPACE::ReadTier::kBlockCacheTier;
        state.it_cacheonly.reset(db_->NewIterator(options));
      }
    }

    const uint64_t upto = state.last_written.load();
    for (CABINDB_NAMESPACE::Iterator* it :
      ...
      for (uint64_t seq = state.last_read.load() + 1; seq <= upto; ++seq) {
        if (need_seek) {
          ...
          it->Seek(CABINDB_NAMESPACE::Slice((const char*)&from, sizeof(from)));
          need_seek = false;
        } else {
          it->Next();
        }
 ...
 private:
  ...
  CABINDB_NAMESPACE::DB* db_;
  CABINDB_NAMESPACE::port::Thread thread_;
  sem_t sem_;
    ...
    while (1) {
      ...
      while (!workq.empty() && workq.front() < now) {
        ...
        CABINDB_NAMESPACE::Status status =
            db_->Put(CABINDB_NAMESPACE::WriteOptions(),
                     CABINDB_NAMESPACE::Slice((const char*)&key, sizeof(key)),
...
struct StatsThread {
  explicit StatsThread(CABINDB_NAMESPACE::DB* db)
...
int main(int argc, char** argv) {
  ...
  CABINDB_NAMESPACE::Status status;
  std::string path =
      CABINDB_NAMESPACE::test::PerThreadDBPath("forward_iterator_test");
  fprintf(stderr, "db path is %s\n", path.c_str());
  CABINDB_NAMESPACE::Options options;
  options.create_if_missing = true;
  options.compression = CABINDB_NAMESPACE::CompressionType::kNoCompression;
  options.compaction_style =
      CABINDB_NAMESPACE::CompactionStyle::kCompactionStyleNone;
  ...
  CABINDB_NAMESPACE::BlockBasedTableOptions table_options;
  table_options.block_cache =
      CABINDB_NAMESPACE::NewLRUCache(FLAGS_block_cache_size);
  table_options.block_size = FLAGS_block_size;
  options.table_factory.reset(
      CABINDB_NAMESPACE::NewBlockBasedTableFactory(table_options));

  status = CABINDB_NAMESPACE::DestroyDB(path, options);
  assert(status.ok());
  CABINDB_NAMESPACE::DB* db_raw;
  status = CABINDB_NAMESPACE::DB::Open(options, path, &db_raw);
  assert(status.ok());
  std::unique_ptr<CABINDB_NAMESPACE::DB> db(db_raw);
