...
// DBWithTTL is not supported in CABINDB_LITE
#ifndef CABINDB_LITE
  if (ttl) {
...
class MergeBasedCounters : public Counters {
 ...
 public:
  ...
  // mapped to a cabindb Merge operation
...
void testPartialMerge(Counters* counters, DB* db, size_t max_merge,
  ...
  resetNumPartialMergeCalls();
  tmp_sum = 0;
  db->Put(CABINDB_NAMESPACE::WriteOptions(), "c", "10");
  for (size_t i = 1; i <= count; i++) {
...
int main(int argc, char** argv) {
  CABINDB_NAMESPACE::use_compression = false;
  if (argc > 1) {
    CABINDB_NAMESPACE::use_compression = true;
  }
