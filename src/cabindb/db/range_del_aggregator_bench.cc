...
auto icmp = CABINDB_NAMESPACE::InternalKeyComparator(
...
int main(int argc, char** argv) {
  ...
  Stats stats;
  CABINDB_NAMESPACE::Random64 rnd(FLAGS_seed);
  ...
  std::vector<std::vector<CABINDB_NAMESPACE::PersistentRangeTombstone> >
  ...
  for (int i = 0; i < FLAGS_add_tombstones_per_run; i++) {
    all_persistent_range_tombstones[i] =
        std::vector<CABINDB_NAMESPACE::PersistentRangeTombstone>(
            FLAGS_num_range_tombstones);
  }
  auto mode = CABINDB_NAMESPACE::RangeDelPositioningMode::kForwardTraversal;

  for (int i = 0; i < FLAGS_num_runs; i++) {
    CABINDB_NAMESPACE::ReadRangeDelAggregator range_del_agg(
        &icmp, CABINDB_NAMESPACE::kMaxSequenceNumber /* upper_bound */);

    std::vector<
        std::unique_ptr<CABINDB_NAMESPACE::FragmentedRangeTombstoneList> >
    ...
    for (auto& persistent_range_tombstones : all_persistent_range_tombstones) {
      ...
      for (int j = 0; j < FLAGS_num_range_tombstones; j++) {
        ...
        persistent_range_tombstones[j] =
            CABINDB_NAMESPACE::PersistentRangeTombstone(
                CABINDB_NAMESPACE::Key(start), CABINDB_NAMESPACE::Key(end), j);
      }

      auto range_del_iter =
          CABINDB_NAMESPACE::MakeRangeDelIterator(persistent_range_tombstones);
      fragmented_range_tombstone_lists.emplace_back(
          new CABINDB_NAMESPACE::FragmentedRangeTombstoneList(
              CABINDB_NAMESPACE::MakeRangeDelIterator(
                  persistent_range_tombstones),
              icmp));
      std::unique_ptr<CABINDB_NAMESPACE::FragmentedRangeTombstoneIterator>
          fragmented_range_del_iter(
              new CABINDB_NAMESPACE::FragmentedRangeTombstoneIterator(
                  fragmented_range_tombstone_lists.back().get(), icmp,
                  CABINDB_NAMESPACE::kMaxSequenceNumber));

      CABINDB_NAMESPACE::StopWatchNano stop_watch_add_tombstones(
          CABINDB_NAMESPACE::Env::Default(), true /* auto_start */);
    ...
    CABINDB_NAMESPACE::ParsedInternalKey parsed_key;
    parsed_key.sequence = FLAGS_num_range_tombstones / 2;
    parsed_key.type = CABINDB_NAMESPACE::kTypeValue;
    ...
    for (int j = 0; j < FLAGS_should_deletes_per_run; j++) {
      std::string key_string = CABINDB_NAMESPACE::Key(first_key + j);
      parsed_key.user_key = key_string;

      CABINDB_NAMESPACE::StopWatchNano stop_watch_should_delete(
          CABINDB_NAMESPACE::Env::Default(), true /* auto_start */);
