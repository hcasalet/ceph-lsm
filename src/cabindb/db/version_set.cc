...
class LevelIterator final : public InternalIterator {
 ...
 private:
  ...
  //
  // Note MyCabin may update iterate bounds between seek. To workaround it,
    ...
    *status = table_cache_->Get(
    ...
    switch (get_context.State()) {
      ...
      case GetContext::kUnexpectedBlobIndex:
        CABIN_LOG_ERROR(info_log_, "Encounter unexpected blob index.");
        *status = Status::NotSupported(
            "Encounter unexpected blob index. Please open DB with "
            "CABINDB_NAMESPACE::blob_db::BlobDB instead.");
        return;
    }
    f = fp.GetNextFile();
  }
...
bool Version::MaybeInitializeFileMetaData(FileMetaData* file_meta) {
  ...
  if (!s.ok()) {
    CABIN_LOG_ERROR(vset_->db_options_->info_log,
    ...
    for (auto f : files_[level]) {
      if (!f->being_compacted) {
        ...
        if (file_modification_time == kUnknownOldestAncesterTime) {
          ...
          if (!status.ok()) {
            CABIN_LOG_WARN(ioptions.info_log,
...
const char* VersionStorageInfo::LevelSummary(
  ...
  if (!files_marked_for_compaction_.empty()) {
    snprintf(scratch->buffer + len, sizeof(scratch->buffer) - len,
             " (%" CABINDB_PRIszt " files need compaction)",
  ...
  if (!ioptions.level_compaction_dynamic_level_bytes) {
    ...
    if (max_level_size == 0) {
      ...
      if (cur_level_size <= base_bytes_min) {
        ...
        CABIN_LOG_INFO(ioptions.info_log,
...
Status VersionSet::ProcessManifestWrites(
  ...
  IOStatus io_s;
  {
    ...
    if (s.ok() && new_descriptor_log) {
      ...
      CABIN_LOG_INFO(db_options_->info_log, "Creating manifest %" PRIu64 "\n",
    ...
    if (s.ok()) {
      ...
      for (auto& e : batch_edits) {
        ...
        TEST_KILL_RANDOM("VersionSet::LogAndApply:BeforeAddRecord",
                         cabindb_kill_odds * REDUCE_ODDS2);
#ifndef NDEBUG
      ...
      if (!io_s.ok()) {
        s = io_s;
        CABIN_LOG_ERROR(db_options_->info_log, "MANIFEST write %s\n",
                        s.ToString().c_str());
      }
    }
...
Status VersionSet::Recover(
  ...
  CABIN_LOG_INFO(db_options_->info_log, "Recovering from manifest file: %s\n",
                 manifest_path.c_str());
...
Status VersionSet::TryRecoverFromOneManifest(
  ...
  CABIN_LOG_INFO(db_options_->info_log, "Trying to recover from manifest: %s\n",
                 manifest_path.c_str());
  std::unique_ptr<SequentialFileReader> manifest_file_reader;
  Status s;
  {
  ...
  do {
    ...
    if (s.ok()) {
      ...
      manifest_reader->reset(new log::FragmentBufferedReader(
          ...
          true /* checksum */, 0 /* log_number */));
      CABIN_LOG_INFO(db_options_->info_log, "Switched to new manifest: %s\n",
                     manifest_path.c_str());
