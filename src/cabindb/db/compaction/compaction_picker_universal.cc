...
Compaction* UniversalCompactionBuilder::PickCompaction() {
  ...
  if (sorted_runs_.size() == 0 ||
    ...
    CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: nothing to do\n",
                     cf_name_.c_str());
  ...
  CABIN_LOG_BUFFER_MAX_SZ(
      log_buffer_, 3072,
      "[%s] Universal: sorted runs: %" CABINDB_PRIszt " files: %s\n",
  ...
  if (c == nullptr &&
    ...
    if ((c = PickCompactionToReduceSizeAmp()) != nullptr) {
      CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: compacting for size amp\n",
                       cf_name_.c_str());
    } else {
...
Compaction* UniversalCompactionBuilder::PickCompactionToReduceSortedRuns(
  ...
  for (size_t loop = 0; loop < sorted_runs_.size(); loop++) {
    ...
    if (candidate_count >= (unsigned int)min_merge_width) {
      ...
      for (size_t i = loop;
        ...
        CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: Skipping %s",
  ...
  for (size_t i = start_index; i < first_index_after; i++) {
    ...
    CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: Picking %s",
                     cf_name_.c_str(), file_num_buf);
  }
...
Compaction* UniversalCompactionBuilder::PickCompactionToReduceSizeAmp() {
  ...
  }
  {
    ...
    CABIN_LOG_BUFFER(
        log_buffer_,
        "[%s] Universal: First candidate %s[%" CABINDB_PRIszt "] %s",
...
Compaction* UniversalCompactionBuilder::PickCompactionToOldest(
  ...
  for (size_t loop = start_index; loop < sorted_runs_.size(); loop++) {
    ...
    CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: %s picking %s",
...
Compaction* UniversalCompactionBuilder::PickPeriodicCompaction() {
  CABIN_LOG_BUFFER(log_buffer_, "[%s] Universal: Periodic Compaction",
                   cf_name_.c_str());
