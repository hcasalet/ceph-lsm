...
class FlushJob {
 public:
  ...
  Status ClearMem(LogsWithPrepTracker* prep_tracker = nullptr,
             FileMetaData* file_meta = nullptr);
  void Cancel();
 ...
 private:
  ...
  // flush job. CabinDB uses this variable to select the memtables to flush in
  ...
  // In this case, only after all flush jobs succeed in flush can CabinDB
