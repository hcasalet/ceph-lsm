...
TEST_F(ExternalSSTFileTest, CompactionDeadlock) {
  ...
  CABINDB_NAMESPACE::port::Thread bg_ingest_file([&]() {
    running_threads += 1;
  ...
  CABINDB_NAMESPACE::port::Thread bg_block_put([&]() {
    running_threads += 1;
    ASSERT_OK(Put(Key(10), "memtable"));
    running_threads -= 1;
  });
...
TEST_P(ExternalSSTFileTest, DeltaEncodingWhileGlobalSeqnoPresent) {
  ...
  // Get a Snapshot to make CabinDB assign global seqno to ingested sst files.
  auto snap = dbfull()->GetSnapshot();

  std::string fname = sst_files_dir_ + "test_file";
  CABINDB_NAMESPACE::SstFileWriter writer(EnvOptions(), options);
