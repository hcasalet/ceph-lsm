...
DBOptions SanitizeOptions(const std::string& dbname, const DBOptions& src) {
  ...
  if (result.recycle_log_file_num &&
    ...
    //   (https://github.com/facebook/cabindb/pull/7252#issuecomment-673766236).
  ...
  if (!result.paranoid_checks) {
    ...
    CABIN_LOG_INFO(result.info_log,
...
Status DBImpl::NewDB(std::vector<std::string>* new_filenames) {
  ...
  CABIN_LOG_INFO(immutable_db_options_.info_log, "Creating manifest 1 \n");
...
Status DBImpl::Recover(
  ...
  if (s.ok()) {
    ...
    // produced by an older version of cabindb.
    if (!immutable_db_options_.best_efforts_recovery) {
...
Status DBImpl::PersistentStatsProcessFormatVersion() {
  ...
  if (persistent_stats_cfd_exists_) {
    ...
    // this sst is encoded with a newer CabinDB release, and current compatible
...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& wal_numbers,
  ...
  struct LogReporter : public log::Reader::Reporter {
    ...
    void Corruption(size_t bytes, const Status& s) override {
      CABIN_LOG_WARN(info_log, "%s%s: dropping %d bytes; %s",
  ...
  for (auto wal_number : wal_numbers) {
    ...
    auto logFileDropped = [this, &fname]() {
      ...
      if (env_->GetFileSize(fname, &bytes).ok()) {
        ...
        CABIN_LOG_WARN(info_log, "%s: dropping %d bytes", fname.c_str(),
...
Status DBImpl::WriteLevel0TableForRecovery(int job_id, ColumnFamilyData* cfd,
  ...
  TableProperties table_properties;
  {
    ...
    CABIN_LOG_DEBUG(immutable_db_options_.info_log,
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  *dbptr = nullptr;
  ...
  if (s.ok()) {
    // Persist CabinDB Options before scheduling the compaction.
  ...
  if (s.ok() && sfm) {
    ...
    CABIN_LOG_INFO(impl->immutable_db_options_.info_log,
  ...
  if (s.ok()) {
    CABIN_LOG_HEADER(impl->immutable_db_options_.info_log, "DB pointer %p",
                     impl);
    ...
    CABIN_LOG_WARN(impl->immutable_db_options_.info_log,
