...
#include "include/cabindb/write_buffer_manager.h"
...
const std::string kPersistentStatsColumnFamilyName(
    "___cabindb_stats_history___");
void DumpCabinDBBuildVersion(Logger* log);
...
void DumpSupportInfo(Logger* logger) {
  CABIN_LOG_HEADER(logger, "Compression algorithms supported:");
  for (auto& compression : OptionsHelper::compression_type_string_map) {
    if (compression.second != kNoCompression &&
      ...
      CABIN_LOG_HEADER(logger, "\t%s supported: %d", compression.first.c_str(),
                       CompressionTypeSupported(compression.second));
    }
  }
  CABIN_LOG_HEADER(logger, "Fast CRC32 supported: %s",
...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  DumpCabinDBBuildVersion(immutable_db_options_.info_log.get());
  SetDbSessionId();
...
Status DBImpl::Resume() {
  CABIN_LOG_INFO(immutable_db_options_.info_log, "Resuming DB");
...
Status DBImpl::ResumeImpl(DBRecoverContext context) {
  ...
  if (s.ok()) {
    ...
    if (!s.ok()) {
      CABIN_LOG_INFO(immutable_db_options_.info_log,
  ...
  if (s.ok()) {
    ...
    CABIN_LOG_INFO(immutable_db_options_.info_log, "Successfully resumed DB");
  }
  mutex_.Lock();
...
Status DBImpl::CloseHelper() {
  ...
  CABIN_LOG_INFO(immutable_db_options_.info_log, "Shutdown complete");
...
void DBImpl::MaybeIgnoreError(Status* s) const {
  if (s->ok() || immutable_db_options_.paranoid_checks) {
    ...
    CABIN_LOG_WARN(immutable_db_options_.info_log, "Ignoring error %s",
                   s->ToString().c_str());
    *s = Status::OK();
  }
}
...
void DBImpl::PrintStatistics() {
  ...
  if (dbstats) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "STATISTICS:\n %s",
...
void DBImpl::PersistStats() {
  ...
  if (immutable_db_options_.persist_stats_to_disk) {
    ...
    if (stats_slice_initialized_) {
      CABIN_LOG_INFO(immutable_db_options_.info_log,
                     "Reading %" CABINDB_PRIszt " stats from statistics\n",
                     stats_slice_.size());
    ...
    if (!s.ok()) {
      ...
      CABIN_LOG_INFO(immutable_db_options_.info_log,
                     "Writing %" CABINDB_PRIszt " stats with timestamp %" PRIu64
    ...
    if (stats_slice_initialized_) {
      ...
      CABIN_LOG_INFO(immutable_db_options_.info_log,
                     "Storing %" CABINDB_PRIszt " stats with timestamp %" PRIu64
    ...
    CABIN_LOG_INFO(immutable_db_options_.info_log,
                   "[Pre-GC] In-memory stats history size: %" CABINDB_PRIszt
                   " bytes, slice count: %" CABINDB_PRIszt,
    ...
    CABIN_LOG_INFO(immutable_db_options_.info_log,
                   "[Post-GC] In-memory stats history size: %" CABINDB_PRIszt
                   " bytes, slice count: %" CABINDB_PRIszt,
                   stats_history_size, stats_history_.size());
  }
...
void DBImpl::DumpStats() {
  ...
  CABIN_LOG_INFO(immutable_db_options_.info_log,
                 "------- DUMPING STATS -------");
  CABIN_LOG_INFO(immutable_db_options_.info_log, "%s", stats.c_str());
...
Status DBImpl::SetOptions(
  ...
  (void)column_family;
  (void)options_map;
  return Status::NotSupported("Not supported in CABINDB LITE");
#else
  auto* cfd =
      static_cast_with_check<ColumnFamilyHandleImpl>(column_family)->cfd();
  if (options_map.empty()) {
    CABIN_LOG_WARN(immutable_db_options_.info_log,
  ...
  for (const auto& o : options_map) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "%s: %s\n", o.first.c_str(),
                   o.second.c_str());
  }
  if (s.ok()) {
    CABIN_LOG_INFO(immutable_db_options_.info_log,
    ...
    CABIN_LOG_WARN(immutable_db_options_.info_log, "[%s] SetOptions() failed",
                   cfd->GetName().c_str());
  }
  LogFlush(immutable_db_options_.info_log);
  return s;
...
Status DBImpl::SetDBOptions(
  ...
  CABIN_LOG_INFO(immutable_db_options_.info_log, "SetDBOptions(), inputs:");
  for (const auto& o : options_map) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "%s: %s\n", o.first.c_str(),
                   o.second.c_str());
  }
  if (s.ok()) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "SetDBOptions() succeeded");
    new_options.Dump(immutable_db_options_.info_log.get());
    if (!persist_options_status.ok()) {
    ...
    CABIN_LOG_WARN(immutable_db_options_.info_log, "SetDBOptions failed");
  }
  LogFlush(immutable_db_options_.info_log);
  return s;
#endif  // CABINDB_LITE
...
Status DBImpl::FlushWAL(bool sync) {
  if (manual_wal_flush_) {
    ...
    if (!io_s.ok()) {
      CABIN_LOG_ERROR(immutable_db_options_.info_log, "WAL flush error %s",
                      io_s.ToString().c_str());
    ...
    if (!sync) {
      CABIN_LOG_DEBUG(immutable_db_options_.info_log, "FlushWAL sync=false");
      return std::move(io_s);
    }
  }
  if (!sync) {
  ...
  // sync = true
  CABIN_LOG_DEBUG(immutable_db_options_.info_log, "FlushWAL sync=true");
  return SyncWAL();
}
...
Status DBImpl::SyncWAL() {
  ...
  if (!io_s.ok()) {
    CABIN_LOG_ERROR(immutable_db_options_.info_log, "WAL Sync error %s",
                    io_s.ToString().c_str());
    // In case there is a fs error we should set it globally to prevent the
    // future writes
    IOStatusCheck(io_s);
  ...
  SuperVersionContext sv_context(/* create_superversion */ true);
  {
    ...
    if (s.ok()) {
      ...
      *handle = new ColumnFamilyHandleImpl(cfd, this, &mutex_);
      ...
      CABIN_LOG_ERROR(immutable_db_options_.info_log,
...
Status DBImpl::NewIterators(
  ...
  if (read_options.tailing) {
    ...
    return Status::InvalidArgument(
        "Tailing iterator not supported in CabinDB lite");
#else
...
Status DBImpl::DeleteFile(std::string name) {
  ...
  if (!ParseFileName(name, &number, &type, &log_type) ||
    ...
    CABIN_LOG_ERROR(immutable_db_options_.info_log, "DeleteFile %s failed.\n",
                    name.c_str());
...
Status DBImpl::CheckConsistency() {
  ...
  if (immutable_db_options_.skip_checking_sst_file_sizes_on_db_open) {
      ...
      for (const std::string& fname : dir_files.second) {
        if (!std::binary_search(existing_files.begin(), existing_files.end(),
            ...
            !std::binary_search(existing_files.begin(), existing_files.end(),
                                Cabin2LevelTableFileName(fname))) {
          corruption_messages +=
    ...
    for (const auto& md : metadata) {
      ...
      if (!s.ok() &&
          env_->GetFileSize(Cabin2LevelTableFileName(file_path), &fsize).ok()) {
        s = Status::OK();
      }
      if (!s.ok()) {
        corruption_messages +=
...
Status DBImpl::WriteOptionsFile(bool need_mutex_lock,
  ...
  Status s = PersistCabinDBOptions(db_options, cf_names, cf_opts, file_name,
                                   fs_.get());
...
void DeleteOptionsFilesHelper(const std::map<uint64_t, std::string>& filenames,
  ...
  for (auto iter = std::next(filenames.begin(), num_files_to_keep);
    ...
    if (!env->DeleteFile(iter->second).ok()) {
      CABIN_LOG_WARN(info_log, "Unable to delete options file %s",
                     iter->second.c_str());
    }
  }
}
...
void DumpCabinDBBuildVersion(Logger* log) {
  ...
  CABIN_LOG_HEADER(log, "CabinDB version: %d.%d.%d\n", CABINDB_MAJOR,
                   CABINDB_MINOR, CABINDB_PATCH);
  CABIN_LOG_HEADER(log, "Git sha %s", cabindb_build_git_sha);
  CABIN_LOG_HEADER(log, "Compile date %s", cabindb_build_compile_date);
#else
    ...
    for (int i = 0; i < vstorage->num_non_empty_levels() && s.ok(); i++) {
      for (size_t j = 0; j < vstorage->LevelFilesBrief(i).num_files && s.ok();
        ...
        if (use_file_checksum) {
          ...
          s = CABINDB_NAMESPACE::VerifySstFileChecksum(opts, file_options_,
                                                       read_options, fname);
        }
      }
    }
...
Status DBImpl::VerifySstFileChecksum(const FileMetaData& fmeta,
  ...
  s = CABINDB_NAMESPACE::GenerateOneFileChecksum(
