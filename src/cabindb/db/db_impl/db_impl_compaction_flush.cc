...
Status DBImpl::FlushMemTableToOutputFile(
  ...
  if (s.ok()) {
    ...
    CABIN_LOG_BUFFER(log_buffer, "[%s] Level summary: %s\n",
...
Status DBImpl::ReFitLevel(ColumnFamilyData* cfd, int level, int target_level) {
  ...
  if (to_level != level) {
    ...
    CABIN_LOG_DEBUG(immutable_db_options_.info_log, "[%s] LogAndApply: %s\n",
...
Status DBImpl::Flush(const FlushOptions& flush_options,
                     ColumnFamilyHandle* column_family) {
  auto cfh = static_cast_with_check<ColumnFamilyHandleImpl>(column_family);
  CABIN_LOG_INFO(immutable_db_options_.info_log, "[%s] Manual flush start.",
                 cfh->GetName().c_str());
  Status s;
  if (immutable_db_options_.atomic_flush) {
...
Status DBImpl::Flush(const FlushOptions& flush_options,
  ...
  if (!immutable_db_options_.atomic_flush) {
    ...
    for (auto cfh : column_families) {
      ...
      CABIN_LOG_INFO(immutable_db_options_.info_log, "%s",
                     cfhi->GetName().c_str());
    }
    CABIN_LOG_INFO(immutable_db_options_.info_log,
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    if (s.IsBusy()) {
      ...
      assert(m);
      CABIN_LOG_BUFFER(&log_buffer, "[%s] [JOB %d] Manual compaction paused",
...
Status DBImpl::BackgroundCompaction(bool* made_progress,
  ...
  *made_progress = false;
  ...
  if (!c) {
    // Nothing to do
    CABIN_LOG_BUFFER(log_buffer, "Compaction nothing to do");
    ...
    InstallSuperVersionAndScheduleWork(c->column_family_data(),
                                       &job_context->superversion_contexts[0],
                                       *c->mutable_cf_options());
    CABIN_LOG_BUFFER(log_buffer, "[%s] Deleted %d files\n",
                     c->column_family_data()->GetName().c_str(),
                     c->num_input_files(0));
    *made_progress = true;
  ...
  if (status.ok() || status.IsCompactionTooLarge() ||
    ...
    CABIN_LOG_WARN(immutable_db_options_.info_log, "Compaction error: %s",
