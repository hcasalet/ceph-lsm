...
Status DBImpl::DisableFileDeletionsWithLock() {
  ...
  if (disable_delete_obsolete_files_ == 1) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "File Deletions Disabled");
  } else {
...
Status DBImpl::EnableFileDeletions(bool force) {
  ...
  if (saved_counter == 0) {
    CABIN_LOG_INFO(immutable_db_options_.info_log, "File Deletions Enabled");
  ...
  if (doing_the_full_scan) {
    ...
    for (auto& path : paths) {
      ...
      for (const std::string& file : files) {
        ...
        // https://github.com/facebook/cabindb/issues/3573
...
void DBImpl::PurgeObsoleteFiles(JobContext& state, bool schedule_only) {
  ...
  if (state.prev_total_log_size > 0) {
    CABIN_LOG_INFO(immutable_db_options_.info_log,
                   ...
                   ", number of live WAL files %" CABINDB_PRIszt ".\n",
