...
#include "include/cabindb/listener.h"
...
TEST_F(DBPropertiesTest, Empty) {
  do {
    ...
    ASSERT_TRUE(dbfull()->GetProperty(
        handles_[1], "cabindb.num-entries-active-mem-table", &num));
    ASSERT_EQ("0", num);
    ...
    ASSERT_TRUE(
        dbfull()->GetProperty("cabindb.is-file-deletions-enabled", &num));
    ASSERT_EQ("0", num);
...
TEST_F(DBPropertiesTest, CurrentVersionNumber) {
  uint64_t v1, v2, v3;
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.current-super-version-number", &v1));
  ASSERT_OK(Put("12345678", ""));
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.current-super-version-number", &v2));
  ASSERT_OK(Flush());
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.current-super-version-number", &v3));
...
TEST_F(DBPropertiesTest, ValidateSampleNumber) {
  ...
  // "cabindb.estimate-num-keys" computation, which is the ground truth.
  ...
  std::string num;
  Reopen(options);
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.estimate-num-keys", &num));
  ASSERT_EQ("45", num);
...
TEST_F(DBPropertiesTest, ReadLatencyHistogramByLevel) {
  ...
  options.statistics = CABINDB_NAMESPACE::CreateDBStatistics();
  options.max_open_files = 11;  // Make sure no proloading of table readers

  // CabinDB sanitize max open files to at least 20. Modify it back.
  ...
  std::string prop;
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.dbstats", &prop));

  // Get() after flushes, See latency histogram tracked.
  for (int key = 0; key < key_index; key++) {
    Get(Key(key));
  }
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.cfstats", &prop));
  ...
  ASSERT_TRUE(dbfull()->GetProperty(dbfull()->DefaultColumnFamily(),
                                    "cabindb.options-statistics", &prop));
  ASSERT_NE(std::string::npos, prop.find("cabindb.block.cache.miss"));
  ASSERT_EQ(std::string::npos, prop.find("cabindb.db.f.micros"));

  ASSERT_TRUE(dbfull()->GetProperty(dbfull()->DefaultColumnFamily(),
                                    "cabindb.cf-file-histogram", &prop));
  ASSERT_NE(std::string::npos, prop.find("** Level 0 read latency histogram"));
  ASSERT_NE(std::string::npos, prop.find("** Level 1 read latency histogram"));
  ASSERT_EQ(std::string::npos, prop.find("** Level 2 read latency histogram"));
  ...
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.cf-file-histogram", &prop));
  ...
  ASSERT_TRUE(
      dbfull()->GetProperty(handles_[1], "cabindb.cf-file-histogram", &prop));
  ASSERT_EQ(std::string::npos, prop.find("** Level 0 read latency histogram"));
  ASSERT_EQ(std::string::npos, prop.find("** Level 1 read latency histogram"));
  ASSERT_EQ(std::string::npos, prop.find("** Level 2 read latency histogram"));
...
TEST_F(DBPropertiesTest, NumImmutableMemTable) {
  do {
    ...
    ASSERT_TRUE(dbfull()->GetProperty(handles_[1],
                                      "cabindb.num-immutable-mem-table", &num));
    ASSERT_EQ(num, "0");
    ASSERT_TRUE(dbfull()->GetProperty(
    ...
    ASSERT_EQ(num, "1");
    ASSERT_TRUE(dbfull()->GetProperty(
        handles_[1], "cabindb.num-entries-imm-mem-tables", &num));
    ASSERT_EQ(num, "1");
    ...
    ASSERT_TRUE(dbfull()->GetProperty(
        handles_[1], "cabindb.cur-size-active-mem-table", &num));
    ASSERT_TRUE(dbfull()->GetProperty(handles_[1],
                                      "cabindb.num-immutable-mem-table", &num));
    ASSERT_EQ(num, "2");
    ASSERT_TRUE(dbfull()->GetProperty(
    ...
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.cur-size-active-mem-table", &value));
    ...
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.estimate-num-keys", &base_total_size));
    ...
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.num-deletes-active-mem-table", &int_num));
    ASSERT_EQ(int_num, 2U);
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.num-entries-active-mem-table", &int_num));
    ASSERT_EQ(int_num, 3U);

    ASSERT_OK(dbfull()->Put(writeOpt, handles_[1], "k2", big_value));
    ASSERT_OK(dbfull()->Put(writeOpt, handles_[1], "k2", big_value));
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.num-entries-imm-mem-tables", &int_num));
    ASSERT_EQ(int_num, 4U);
    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.num-deletes-imm-mem-tables", &int_num));
    ASSERT_EQ(int_num, 2U);

    ASSERT_TRUE(dbfull()->GetIntProperty(
        handles_[1], "cabindb.estimate-num-keys", &int_num));
...
TEST_F(DBPropertiesTest, DISABLED_GetProperty) {
  ...
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.estimate-table-readers-mem", &int_num));
  ASSERT_EQ(int_num, 0U);
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.estimate-live-data-size", &int_num));
  ASSERT_EQ(int_num, 0U);

  ASSERT_OK(dbfull()->Put(writeOpt, "k1", big_value));
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.num-immutable-mem-table", &num));
  ASSERT_EQ(num, "0");
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.mem-table-flush-pending", &num));
  ASSERT_EQ(num, "0");
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.compaction-pending", &num));
  ASSERT_EQ(num, "0");
  ASSERT_TRUE(dbfull()->GetProperty("cabindb.estimate-num-keys", &num));
  ASSERT_EQ(num, "1");
  get_perf_context()->Reset();
  ...
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.num-immutable-mem-table", &int_num));
  ASSERT_EQ(int_num, 2U);
  ASSERT_TRUE(
      dbfull()->GetIntProperty("cabindb.mem-table-flush-pending", &int_num));
  ASSERT_EQ(int_num, 1U);
  ASSERT_TRUE(dbfull()->GetIntProperty("cabindb.compaction-pending", &int_num));
  ASSERT_EQ(int_num, 0U);
  ASSERT_TRUE(dbfull()->GetIntProperty("cabindb.estimate-num-keys", &int_num));
  ASSERT_EQ(int_num, 2U);
  ...
  // Wait for compaction to be done. This is important because otherwise CabinDB
  ...
  // Test cabindb.num-live-versions
  {
    options.level0_file_num_compaction_trigger = 20;
    Reopen(options);
    ASSERT_TRUE(
        dbfull()->GetIntProperty("cabindb.num-live-versions", &int_num));
    ASSERT_EQ(int_num, 1U);
...
TEST_F(DBPropertiesTest, ApproximateMemoryUsage) {
  ...
  dbfull()->GetIntProperty("cabindb.cur-size-active-mem-table", &active_mem);
  dbfull()->GetIntProperty("cabindb.cur-size-all-mem-tables", &unflushed_mem);
  dbfull()->GetIntProperty("cabindb.size-all-mem-tables", &all_mem);
...
TEST_F(DBPropertiesTest, EstimatePendingCompBytes) {
  ...
  ASSERT_OK(Flush());
  ASSERT_TRUE(dbfull()->GetIntProperty(
      "cabindb.estimate-pending-compaction-bytes", &int_num));
  ASSERT_EQ(int_num, 0U);
...
TEST_F(DBPropertiesTest, GetUserDefinedTableProperties) {
  ...
  for (const auto& item : props) {
    ...
    ASSERT_TRUE(user_collected.find("CountingUserTblPropCollector") !=
                user_collected.end());
    ASSERT_EQ(user_collected.at("CountingUserTblPropCollector"), "Cabindb");
...
TEST_F(DBPropertiesTest, EstimateNumKeysUnderflow) {
  ...
  ASSERT_OK(Delete("foo"));
  uint64_t num_keys = 0;
  ASSERT_TRUE(dbfull()->GetIntProperty("cabindb.estimate-num-keys", &num_keys));
...
TEST_F(DBPropertiesTest, EstimateOldestKeyTime) {
  ...
  // "cabindb.estimate-oldest-key-time" only available to fifo compaction.
