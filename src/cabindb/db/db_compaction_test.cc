...
#include "include/cabindb/sst_file_writer.h"
#include "include/cabindb/utilities/convenience.h"
...
TEST_P(DBCompactionTestWithParam, PartialCompactionFailure) {
  ...
  // Make sure CabinDB will not get into corrupted state.
  Reopen(options);
...
TEST_F(DBCompactionTest, CompactFilesPendingL0Bug) {
  // https://www.facebook.com/groups/cabindb.dev/permalink/1389452781153232/
...
void IngestOneKeyValue(DBImpl* db, const std::string& key,
  ...
  EnvOptions env;
  CABINDB_NAMESPACE::SstFileWriter writer(env, options);
...
TEST_P(DBCompactionTestWithParam,
  ...
  db_->GetIntProperty("cabindb.background-errors", &error_count);
...
TEST_F(DBCompactionTest, ChangeLevelCompactRangeConflictsWithManual) {
  ...
  CABINDB_NAMESPACE::port::Thread refit_level_thread([&] {
    CompactRangeOptions cro;
    cro.change_level = true;
    cro.target_level = 1;
    ASSERT_OK(dbfull()->CompactRange(cro, nullptr, nullptr));
