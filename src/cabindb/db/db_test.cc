...
#include "include/cabindb/experimental.h"
...
#include "include/cabindb/thread_status.h"
...
// TEST_FlushMemTable() is not supported in CABINDB_LITE
#ifndef CABINDB_LITE
...
// NewMemEnv returns nullptr in CABINDB_LITE since class InMemoryEnv isn't
// defined.
#ifndef CABINDB_LITE
...
TEST_F(DBTest, ReadFromPersistedTier) {
  do {
    ...
    for (int disableWAL = 0; disableWAL <= 1; ++disableWAL) {
      ...
      ASSERT_OK(Flush(1));
      ASSERT_OK(db_->Put(wopt, handles_[1], "cabindb", "hello"));
      ...
      if (wopt.disableWAL) {
        ASSERT_TRUE(
            db_->Get(ropt, handles_[1], "cabindb", &value).IsNotFound());
      } else {
        ASSERT_OK(db_->Get(ropt, handles_[1], "cabindb", &value));
        ASSERT_EQ(value, "hello");
      }

      // Expect same result in multiget
      multiget_cfs.push_back(handles_[1]);
      multiget_keys.push_back("cabindb");
      statuses =
          db_->MultiGet(ropt, multiget_cfs, multiget_keys, &multiget_values);
      ...
      ASSERT_TRUE(db_->Get(ropt, handles_[1], "cabindb", &value).ok());
      ASSERT_EQ(value, "hello");
...
TEST_F(DBTest, ComparatorCheck) {
  class NewComparator : public Comparator {
   public:
    const char* Name() const override { return "cabindb.NewComparator"; }
...
class ModelDB : public DB {
 public:
  ...
  using DB::Flush;
  Status Flush(const CABINDB_NAMESPACE::FlushOptions& /*options*/,
               ColumnFamilyHandle* /*column_family*/) override {
    Status ret;
    return ret;
  }
  Status Flush(
      const CABINDB_NAMESPACE::FlushOptions& /*options*/,
  ...
  Status GetUpdatesSince(
      CABINDB_NAMESPACE::SequenceNumber,
      std::unique_ptr<CABINDB_NAMESPACE::TransactionLogIterator>*,
...
TEST_F(DBTest, BlockBasedTablePrefixIndexTest) {
  ...
  // CabinDB should just fall back to the binary index.
...
TEST_F(DBTest, FileCreationRandomFailure) {
  ...
  // If cabindb does not do the correct job, internal assert will fail here.
  dbfull()->TEST_WaitForFlushMemTable();
  dbfull()->TEST_WaitForCompact();
...
TEST_F(DBTest, CompactFilesShouldTriggerAutoCompaction) {
  ...
  CABINDB_NAMESPACE::ColumnFamilyMetaData cf_meta_data;
...
TEST_F(DBTest, LargeBatchWithColumnFamilies) {
  ...
  for (int i = 0; i < 5; i++) {
    for (int pass = 1; pass <= 3; pass++) {
      ...
      fprintf(stderr, "prepare: %" CABINDB_PRIszt " MB, pass:%d\n",
      ...
      fprintf(stderr, "write: %" CABINDB_PRIszt " MB\n",
...
#if !defined(CABINDB_LITE) && !defined(CABINDB_DISABLE_STALL_NOTIFICATION)
...
#endif  // !defined(CABINDB_LITE) && !defined(CABINDB_DISABLE_STALL_NOTIFICATION)
