...
class CorruptionTest : public testing::Test {
 public:
  ...
  void RepairDB() {
    delete db_;
    db_ = nullptr;
    ASSERT_OK(::CABINDB_NAMESPACE::RepairDB(dbname_, options_));
  }
...
TEST_F(CorruptionTest, CompactionInputError) {
  ...
  ASSERT_OK(dbi->TEST_CompactRange(0, nullptr, nullptr));
  ASSERT_OK(dbi->TEST_CompactRange(1, nullptr, nullptr));
  ASSERT_EQ(1, Property("cabindb.num-files-at-level2"));
...
TEST_F(CorruptionTest, CompactionInputErrorParanoid) {
  ...
  ASSERT_EQ(1, Property("cabindb.num-files-at-level0"));
...
TEST_F(CorruptionTest, VerifyWholeTableChecksum) {
  ...
  options.file_checksum_gen_factory =
      CABINDB_NAMESPACE::GetFileChecksumGenCrc32cFactory();
  Reopen(&options);
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr, "SKIPPED as RepairDB() is not supported in CABINDB_LITE\n");
  return 0;
}
