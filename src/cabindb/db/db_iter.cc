...
Status DBIter::GetProperty(std::string prop_name, std::string* prop) {
  ...
  if (prop_name == "cabindb.iterator.super-version-number") {
    // First try to pass the value returned from inner iterator.
    return iter_.iter()->GetProperty(prop_name, prop);
  } else if (prop_name == "cabindb.iterator.is-key-pinned") {
    if (valid_) {
    ...
    return Status::OK();
  } else if (prop_name == "cabindb.iterator.internal-key") {
...
bool DBIter::ParseKey(ParsedInternalKey* ikey) {
  ...
  if (!s.ok()) {
    ...
    valid_ = false;
    CABIN_LOG_ERROR(logger_, "In DBIter: %s", status_.getState());
    return false;
  } else {
    return true;
  }
          ...
          case kTypeBlobIndex:
            if (start_seqnum_ > 0) {
              ...
              if (range_del_agg_.ShouldDelete(
                ...
                if (!allow_blob_) {
                  CABIN_LOG_ERROR(logger_, "Encounter unexpected blob index.");
...
bool DBIter::MergeValuesNewToOld() {
  if (!merge_operator_) {
    CABIN_LOG_ERROR(logger_, "Options::merge_operator is null.");
