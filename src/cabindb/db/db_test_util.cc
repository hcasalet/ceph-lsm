...
#include "include/cabindb/env_encryption.h"
...
#ifdef CABINDB_LITE
    // These options are not supported in CABINDB_LITE
...
void DBTestBase::Destroy(const Options& options, bool delete_cf_paths) {
  ...
  if (delete_cf_paths) {
    for (size_t i = 0; i < handles_.size(); ++i) {
      ...
      // GetDescriptor is not implemented for CABINDB_LITE
...
uint64_t DBTestBase::GetNumSnapshots() {
  uint64_t int_num;
  EXPECT_TRUE(dbfull()->GetIntProperty("cabindb.num-snapshots", &int_num));
  return int_num;
}
...
uint64_t DBTestBase::GetTimeOldestSnapshots() {
  uint64_t int_num;
  EXPECT_TRUE(
      dbfull()->GetIntProperty("cabindb.oldest-snapshot-time", &int_num));
  return int_num;
}
...
uint64_t DBTestBase::GetSequenceOldestSnapshots() {
  uint64_t int_num;
  EXPECT_TRUE(
      dbfull()->GetIntProperty("cabindb.oldest-snapshot-sequence", &int_num));
  return int_num;
}
...
int DBTestBase::NumTableFilesAtLevel(int level, int cf) {
  ...
  if (cf == 0) {
    ...
    EXPECT_TRUE(db_->GetProperty(
        "cabindb.num-files-at-level" + NumberToString(level), &property));
  } else {
    EXPECT_TRUE(db_->GetProperty(
        handles_[cf], "cabindb.num-files-at-level" + NumberToString(level),
        &property));
  }
...
double DBTestBase::CompressionRatioAtLevel(int level, int cf) {
  ...
  if (cf == 0) {
    // default cfd
    EXPECT_TRUE(db_->GetProperty(
        "cabindb.compression-ratio-at-level" + NumberToString(level),
        &property));
  } else {
    EXPECT_TRUE(db_->GetProperty(
        handles_[cf],
...
std::string DBTestBase::DumpSSTableList() {
  std::string property;
  db_->GetProperty("cabindb.sstables", &property);
  return property;
}
