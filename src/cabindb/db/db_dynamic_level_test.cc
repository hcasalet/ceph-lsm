...
TEST_F(DBTestDynamicLevel, DynamicLevelMaxBytesBase) {
  ...
  for (int ordered_insert = 0; ordered_insert <= 1; ordered_insert++) {
    ...
    for (int max_background_compactions = 1; max_background_compactions < 4;
      ...
      uint64_t int_prop;
      ASSERT_TRUE(db_->GetIntProperty("cabindb.background-errors", &int_prop));
...
TEST_F(DBTestDynamicLevel, DynamicLevelMaxBytesBase2) {
  ...
  ASSERT_TRUE(db_->GetIntProperty("cabindb.base-level", &int_prop));
  ...
  ASSERT_TRUE(db_->GetProperty("cabindb.num-files-at-level1", &str_prop));
  ASSERT_EQ("0", str_prop);
  ASSERT_TRUE(db_->GetProperty("cabindb.num-files-at-level2", &str_prop));
  ASSERT_EQ("0", str_prop);
  ...
  CABINDB_NAMESPACE::port::Thread thread([this] {
...
TEST_F(DBTestDynamicLevel, DynamicLevelMaxBytesCompactRange) {
  ...
  ASSERT_TRUE(db_->GetProperty("cabindb.num-files-at-level0", &str_prop));
  ASSERT_EQ("0", str_prop);
  ASSERT_TRUE(db_->GetProperty("cabindb.num-files-at-level3", &str_prop));
  ASSERT_EQ("0", str_prop);
...
TEST_F(DBTestDynamicLevel, DISABLED_MigrateToDynamicLevelMaxBytesBase) {
  ...
  // in main thread.
  CABINDB_NAMESPACE::port::Thread t([&]() {
