...
TEST_F(FileNameTest, Parse) {
  ...
  static struct {
    ...
    char mode;
      ...
      {"cabindb_dir_LOG", 0, kInfoLogFile, kDifferentInfoLogDir},
      {"cabindb_dir_LOG.old", 0, kInfoLogFile, kDifferentInfoLogDir},
      {"cabindb_dir_LOG.old.6688", 6688, kInfoLogFile, kDifferentInfoLogDir},
  ...
  for (char mode : {kDifferentInfoLogDir, kDefautInfoLogDir, kNoCheckLogDir}) {
    for (unsigned int i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
      InfoLogPrefix info_log_prefix(mode != kDefautInfoLogDir, "/cabindb/dir");
...
TEST_F(FileNameTest, InfoLogFileName) {
  std::string dbname = ("/data/cabindb");
  std::string db_absolute_path;
  ASSERT_OK(Env::Default()->GetAbsolutePath(dbname, &db_absolute_path));

  ASSERT_EQ("/data/cabindb/LOG", InfoLogFileName(dbname, db_absolute_path, ""));
  ASSERT_EQ("/data/cabindb/LOG.old.666",
            OldInfoLogFileName(dbname, 666u, db_absolute_path, ""));

  ASSERT_EQ("/data/cabindb_log/data_cabindb_LOG",
            InfoLogFileName(dbname, db_absolute_path, "/data/cabindb_log"));
  ASSERT_EQ(
      "/data/cabindb_log/data_cabindb_LOG.old.666",
      OldInfoLogFileName(dbname, 666u, db_absolute_path, "/data/cabindb_log"));
}
