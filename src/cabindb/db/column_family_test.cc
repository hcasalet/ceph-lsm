...
class ColumnFamilyTestBase : public testing::Test {
 public:
...
#ifndef CABINDB_LITE  // ReadOnlyDB is not supported
...
#ifndef CABINDB_LITE
    uint64_t v;
    EXPECT_TRUE(dbfull()->GetIntProperty("cabindb.is-write-stopped", &v));
    return (v == 1);
#else
  ...
  uint64_t GetDbDelayedWriteRate() {
    ...
    uint64_t v;
    EXPECT_TRUE(
        dbfull()->GetIntProperty("cabindb.actual-delayed-write-rate", &v));
    return v;
#else
...
#ifndef CABINDB_LITE  // CabinDBLite does not support GetDescriptor
...
#ifndef CABINDB_LITE  // TEST functions in DB are not supported in lite
  ...
  int NumTableFilesAtLevel(int level, int cf) {
    return GetProperty(cf,
                       "cabindb.num-files-at-level" + ToString(level));
  }
...
#ifndef CABINDB_LITE  // GetLiveFilesMetaData is not supported
  int CountLiveFiles() {
...
#ifndef CABINDB_LITE  // GetSortedWalFiles is not supported
...
#ifndef CABINDB_LITE  // GetProperty is not supported in lite
...
TEST_P(ColumnFamilyTest, CreateCFRaceWithGetAggProperty) {
  ...
  CABINDB_NAMESPACE::port::Thread thread(
...
#ifndef CABINDB_LITE  // TEST functions used are not supported
...
#ifndef CABINDB_LITE  // WaitForFlush() is not supported
// Makes sure that obsolete log files get deleted
...
// re-enable the test.
//
// #ifndef CABINDB_LITE  // Cuckoo is not supported in lite
...
class TestComparator : public Comparator {
  int Compare(const CABINDB_NAMESPACE::Slice& /*a*/,
              const CABINDB_NAMESPACE::Slice& /*b*/) const override {
    return 0;
  }
  const char* Name() const override { return "Test"; }
  void FindShortestSeparator(
      std::string* /*start*/,
      const CABINDB_NAMESPACE::Slice& /*limit*/) const override {}
...
#ifndef CABINDB_LITE
// Sync points not supported in CabinDB Lite
...
TEST_P(ColumnFamilyTest, AutomaticAndManualCompactions) {
  ...
  CABINDB_NAMESPACE::port::Thread threads([&] {
    CompactRangeOptions compact_options;
    compact_options.exclusive_manual_compaction = false;
    ASSERT_OK(
        db_->CompactRange(compact_options, handles_[2], nullptr, nullptr));
...
TEST_P(ColumnFamilyTest, SameCFManualManualCompactions) {
  ...
  CABINDB_NAMESPACE::port::Thread threads1([&] {
    CompactRangeOptions compact_options;
    compact_options.exclusive_manual_compaction = false;
    ASSERT_OK(
        db_->CompactRange(compact_options, handles_[1], nullptr, nullptr));
...
#ifndef CABINDB_LITE  // Tailing iterator not supported
namespace {
...
#ifndef CABINDB_LITE  //  WaitForFlush() is not supported in lite
...
#ifndef CABINDB_LITE  //  WaitForCompaction() is not supported in lite
...
TEST_P(ColumnFamilyTest, ReadDroppedColumnFamily) {
  ...
  for (int iter = 0; iter < 2; ++iter) {
    ...
    // Since we didn't delete CF handle, CabinDB's contract guarantees that
...
#ifndef CABINDB_LITE
// skipped as persisting options is not supported in CABINDB_LITE
namespace {
...
TEST_P(ColumnFamilyTest, CreateAndDropRace) {
  ...
  CABINDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
      "PersistCabinDBOptions:start", [&](void* /*arg*/) {
  ...
  // and its comparator
  CABINDB_NAMESPACE::port::Thread drop_cf_thread(DropSingleColumnFamily, this,
                                                 1, &comparators);
...
#ifndef CABINDB_LITE  // TEST functions are not supported in lite
...
TEST_P(ColumnFamilyTest, LogSyncConflictFlush) {
  ...
  CABINDB_NAMESPACE::port::Thread thread([&] { ASSERT_OK(db_->SyncWAL()); });
