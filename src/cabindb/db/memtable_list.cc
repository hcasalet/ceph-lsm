...
// Instead of flushing memtable just release the filled and re-create a new one
Status MemTableList::TryReleaseMemtableAndCreateNew(
    ColumnFamilyData* cfd, const MutableCFOptions& mutable_cf_options,
    const autovector<MemTable*>& mems, LogsWithPrepTracker* prep_tracker,
    VersionSet* vset, InstrumentedMutex* mu, uint64_t file_number,
    autovector<MemTable*>* to_delete, FSDirectory* db_directory,
...
void MemTableList::RemoveMemTablesOrRestoreFlags(
  ...
  // iterators. CabinDB contract requires that the iterator can be created
...
Status InstallMemtableAtomicFlushResults(
  ...
  // TODO(cc): after https://github.com/facebook/cabindb/pull/7570, handle 2pc
  // here.
  std::unique_ptr<VersionEdit> wal_deletion;
  if (vset->db_options()->track_and_verify_wals_in_manifest) {
