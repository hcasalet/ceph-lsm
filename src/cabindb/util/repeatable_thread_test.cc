...
class RepeatableThreadTest : public testing::Test {
 public:
  RepeatableThreadTest()
      : mock_env_(new CABINDB_NAMESPACE::MockTimeEnv(
            CABINDB_NAMESPACE::Env::Default())) {}

 protected:
  std::unique_ptr<CABINDB_NAMESPACE::MockTimeEnv> mock_env_;
};
...
TEST_F(RepeatableThreadTest, TimedTest) {
  ...
  CABINDB_NAMESPACE::port::Mutex mutex;
  CABINDB_NAMESPACE::port::CondVar test_cv(&mutex);
  int count = 0;
  uint64_t prev_time = env->NowMicros();
  CABINDB_NAMESPACE::RepeatableThread thread(
      [&] {
        CABINDB_NAMESPACE::MutexLock l(&mutex);
        count++;
