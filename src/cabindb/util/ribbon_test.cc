...
// Default is for general CabinDB regression test runs.
...
struct StandardKeyGen {
  StandardKeyGen(const std::string& prefix, uint64_t id)
      : id_(id), str_(prefix) {
    CABINDB_NAMESPACE::PutFixed64(&str_, /*placeholder*/ 0);
  }
  ...
  const std::string& operator*() {
    ...
    CABINDB_NAMESPACE::EncodeFixed64(&str_[str_.size() - 8],
...
struct SmallKeyGen {
  SmallKeyGen(const std::string& prefix, uint64_t id) : id_(id) {
    ...
    id_ += CABINDB_NAMESPACE::GetSliceHash64(prefix);
    CABINDB_NAMESPACE::PutFixed64(&str_, id_);
  }
  ...
  const std::string& operator*() {
    CABINDB_NAMESPACE::EncodeFixed64(&str_[str_.size() - 8], id_);
    return str_;
  }
...
struct Hash32KeyGenWrapper : public KeyGen {
  ...
  uint32_t operator*() {
    ...
    // unseeded
    return CABINDB_NAMESPACE::GetSliceHash(key);
  }
};
...
struct Hash64KeyGenWrapper : public KeyGen {
  ...
  uint64_t operator*() {
    auto& key = *static_cast<KeyGen&>(*this);
    // unseeded
    return CABINDB_NAMESPACE::GetSliceHash64(key);
  }
};

}  // namespace

using CABINDB_NAMESPACE::ribbon::ExpectedCollisionFpRate;
using CABINDB_NAMESPACE::ribbon::StandardHasher;
using CABINDB_NAMESPACE::ribbon::StandardRehasherAdapter;

struct DefaultTypesAndSettings {
  using CoeffRow = CABINDB_NAMESPACE::Unsigned128;
  using ResultRow = uint8_t;
  using Index = uint32_t;
  using Hash = uint64_t;
  using Seed = uint32_t;
  using Key = CABINDB_NAMESPACE::Slice;
  ...
  static Hash HashFn(const Key& key, uint64_t raw_seed) {
    ...
    return CABINDB_NAMESPACE::Hash64(key.data(), key.size(), raw_seed);
  }
  // For testing
...
struct TypesAndSettings_Hash32 : public DefaultTypesAndSettings {
  ...
  static Hash HashFn(const Key& key, Hash raw_seed) {
    ...
    return CABINDB_NAMESPACE::Hash(key.data(), key.size(), raw_seed);
  }
};
...
TYPED_TEST(RibbonTypeParamTest, CompactnessAndBacktrackAndFpRate) {
  ...
  const auto log2_thoroughness =
      static_cast<Hash>(CABINDB_NAMESPACE::FloorLog2(FLAGS_thoroughness));
  ...
  CABINDB_NAMESPACE::Random32 rnd(FLAGS_thoroughness);
  ...
  for (uint32_t i = 0; i < FLAGS_thoroughness; ++i) {
    ...
    std::string prefix;
    CABINDB_NAMESPACE::PutFixed32(&prefix, rnd.Next());
    ...
    {
      CABINDB_NAMESPACE::StopWatchNano timer(CABINDB_NAMESPACE::Env::Default(),
                                             true);
    ...
    if (ibytes >= /* minimum Bloom impl bytes*/ 64) {
      ...
      while (cur != other_keys_end) {
        ...
        uint32_t h1 = CABINDB_NAMESPACE::Lower32of64(h);
        uint32_t h2 = sizeof(Hash) >= 8 ? CABINDB_NAMESPACE::Upper32of64(h)
                                        : h1 * 0x9e3779b9;
        bfp_count += CABINDB_NAMESPACE::FastLocalBloomImpl::HashMayMatch(
...
struct PhsfInputGen {
  PhsfInputGen(const std::string& prefix, uint64_t id) : id_(id) {
    val_.first = prefix;
    CABINDB_NAMESPACE::PutFixed64(&val_.first, /*placeholder*/ 0);
  }
  ...
  const std::pair<std::string, uint8_t>& operator*() {
    // Use multiplication to mix things up a little in the key
    CABINDB_NAMESPACE::EncodeFixed64(&val_.first[val_.first.size() - 8],
                                     id_ * uint64_t{0x1500000001});
