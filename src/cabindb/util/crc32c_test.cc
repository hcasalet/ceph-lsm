...
int main(int argc, char** argv) {
  ...
  const uint8_t* src = (uint8_t*)CABINDB_NAMESPACE::crc32c::buffer;
  uint64_t* dst = (uint64_t*)CABINDB_NAMESPACE::crc32c::buffer;
  const uint64_t* end =
      (const uint64_t*)(CABINDB_NAMESPACE::crc32c::buffer +
                        CABINDB_NAMESPACE::crc32c::BUFFER_SIZE);
  *dst++ = 0;
  while (dst < end) {
    CABINDB_NAMESPACE::EncodeFixed64(
