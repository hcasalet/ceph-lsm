...
#if defined(CABINDB_JEMALLOC) && defined(OS_WIN) && \
    defined(ZSTD_STATIC_LINKING_ONLY)
#define CABINDB_ZSTD_CUSTOM_MEM
namespace port {
ZSTD_customMem GetJeZstdAllocationOverrides();
}  // namespace port
#endif  // defined(CABINDB_JEMALLOC) && defined(OS_WIN) &&
        // defined(ZSTD_STATIC_LINKING_ONLY)
...
class ZSTDUncompressCachedData {
 public:
  ...
  ZSTDUncompressCachedData(ZSTDUncompressCachedData&& o) CABINDB_NOEXCEPT
      : ZSTDUncompressCachedData() {
    *this = std::move(o);
  }
...
#ifdef CABINDB_ZSTD_CUSTOM_MEM
      zstd_ctx_ =
          ZSTD_createDCtx_advanced(port::GetJeZstdAllocationOverrides());
#else   // CABINDB_ZSTD_CUSTOM_MEM
      zstd_ctx_ = ZSTD_createDCtx();
#endif  // CABINDB_ZSTD_CUSTOM_MEM
      cache_idx_ = -1;
    }
  }
