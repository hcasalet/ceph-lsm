...
// a mapping from a JavaScript object to a C++ code that can use the CabinDB
// API.
class DBWrapper : public node::ObjectWrap {
  ...
  private:
    ...
    static bool AddToBatch(CABINDB_NAMESPACE::WriteBatch& batch, bool del,
                           Handle<Array> array);
    static bool AddToBatch(CABINDB_NAMESPACE::WriteBatch& batch, bool del,
    ...
    // Internal fields
    CABINDB_NAMESPACE::Options options_;
    CABINDB_NAMESPACE::Status status_;
    CABINDB_NAMESPACE::DB* db_;
    std::unordered_map<std::string, CABINDB_NAMESPACE::ColumnFamilyHandle*>
        columnFamilies_;
};
