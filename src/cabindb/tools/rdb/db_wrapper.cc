...
Handle<Value> DBWrapper::Open(const Arguments& args) {
  ...
  std::vector<std::string> cfs = {CABINDB_NAMESPACE::kDefaultColumnFamilyName};
  ...
  if (cfs.size() == 1) {
    db_wrapper->status_ = CABINDB_NAMESPACE::DB::Open(
        db_wrapper->options_, db_file, &db_wrapper->db_);

    return scope.Close(Boolean::New(db_wrapper->status_.ok()));
  }

  std::vector<CABINDB_NAMESPACE::ColumnFamilyDescriptor> families;

  for (std::vector<int>::size_type i = 0; i < cfs.size(); i++) {
    families.push_back(CABINDB_NAMESPACE::ColumnFamilyDescriptor(
        cfs[i], CABINDB_NAMESPACE::ColumnFamilyOptions()));
  }
...
Handle<Value> DBWrapper::Get(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Get(CABINDB_NAMESPACE::ReadOptions(), key, &value);
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ =
        db_wrapper->db_->Get(CABINDB_NAMESPACE::ReadOptions(),
...
Handle<Value> DBWrapper::Put(const Arguments& args) {
  ...
  if (args[2]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Put(CABINDB_NAMESPACE::WriteOptions(), key, value);
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ =
        db_wrapper->db_->Put(CABINDB_NAMESPACE::WriteOptions(),
                             db_wrapper->columnFamilies_[cf], key, value);
  } else {
    return scope.Close(Boolean::New(false));
  }
...
Handle<Value> DBWrapper::Delete(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Delete(CABINDB_NAMESPACE::WriteOptions(), arg0);
  } else {
    if (!db_wrapper->HasFamilyNamed(arg1, db_wrapper)) {
      return scope.Close(Boolean::New(false));
    }
    db_wrapper->status_ =
        db_wrapper->db_->Delete(CABINDB_NAMESPACE::WriteOptions(),
...
Handle<Value> DBWrapper::Dump(const Arguments& args) {
  HandleScope scope;
  std::unique_ptr<CABINDB_NAMESPACE::Iterator> iterator;
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  std::string arg0      = *v8::String::Utf8Value(args[0]->ToString());

  if (args[0]->IsUndefined()) {
    iterator.reset(
        db_wrapper->db_->NewIterator(CABINDB_NAMESPACE::ReadOptions()));
  } else {
    ...
    iterator.reset(db_wrapper->db_->NewIterator(
        CABINDB_NAMESPACE::ReadOptions(), db_wrapper->columnFamilies_[arg0]));
  }
...
Handle<Value> DBWrapper::CreateColumnFamily(const Arguments& args) {
  ...
  CABINDB_NAMESPACE::ColumnFamilyHandle* cf;
  db_wrapper->status_ = db_wrapper->db_->CreateColumnFamily(
      CABINDB_NAMESPACE::ColumnFamilyOptions(), cf_name, &cf);
...
bool DBWrapper::AddToBatch(CABINDB_NAMESPACE::WriteBatch& batch, bool del,
                           Handle<Array> array) {
...
Handle<Value> DBWrapper::WriteBatch(const Arguments& args) {
  ...
  CABINDB_NAMESPACE::WriteBatch batch;
  bool well_formed;
  ...
  db_wrapper->status_ =
      db_wrapper->db_->Write(CABINDB_NAMESPACE::WriteOptions(), &batch);
...
Handle<Value> DBWrapper::CompactRangeDefault(const Arguments& args) {
  ...
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  CABINDB_NAMESPACE::Slice begin = *v8::String::Utf8Value(args[0]->ToString());
  CABINDB_NAMESPACE::Slice end = *v8::String::Utf8Value(args[1]->ToString());
