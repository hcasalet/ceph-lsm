...
#include "cabindb/db_bench_tool.h"
...
class DBBenchTest : public testing::Test {
 public:
  ...
  void VerifyOptions(const Options& opt) {
    ...
    ASSERT_OK(CabinDBOptionsParser::VerifyDBOptions(exact, DBOptions(opt),
                                                    loaded_db_opts));
    ASSERT_OK(CabinDBOptionsParser::VerifyCFOptions(
        exact, ColumnFamilyOptions(opt), cf_descs[0].options));

    // check with the default cabindb options and expect failure
    ASSERT_NOK(CabinDBOptionsParser::VerifyDBOptions(exact, DBOptions(),
                                                     loaded_db_opts));
...
TEST_F(DBBenchTest, OptionsFile) {
  ...
  ASSERT_OK(PersistCabinDBOptions(DBOptions(opt), {"default"},
...
[TableOptions/BlockBasedTable "default"]
  ...
  filter_policy=cabindb.BuiltinBloomFilter
)OPTIONS_FILE";
