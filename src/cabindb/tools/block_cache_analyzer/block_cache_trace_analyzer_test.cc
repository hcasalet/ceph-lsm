...
class BlockCacheTracerTest : public testing::Test {
 public:
  ...
  void RunBlockCacheTraceAnalyzer() {
    ...
    ASSERT_EQ(0,
              CABINDB_NAMESPACE::block_cache_trace_analyzer_tool(argc, argv));
  }
...
TEST_F(BlockCacheTracerTest, MixedBlocks) {
...

  {
    ...
    ASSERT_EQ(static_cast<uint32_t>(kMajorVersion),
              header.cabindb_major_version);
    ASSERT_EQ(static_cast<uint32_t>(kMinorVersion),
              header.cabindb_minor_version);
    // Read blocks.
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr,
          "block_cache_trace_analyzer_test is not supported in CABINDB_LITE\n");
  return 0;
}
#endif  // CABINDB_LITE
