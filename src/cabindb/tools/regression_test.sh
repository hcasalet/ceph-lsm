...
#
# = Examples =
# * Run the regression test using cabindb commit abcdef that outputs results
...
#   abcdef and with the cabindb options specified in /my/path/to/OPTIONS-012345
...
#       Default: "/tmp/cabindb/regression_test"
...
#   DB_PATH: the path where the cabindb database will be created during the
#       regression test.  Default:  $TEST_PATH/db
#   WAL_PATH: the path where the cabindb WAL will be outputed.
#       Default:  $TEST_PATH/wal
#   OPTIONS_FILE:  If specified, then the regression test will use the specified
#       file to initialize the CabinDB options in its benchmarks.  Note that
#       this feature only work for commits after 88acd93 or cabindb version
#       later than 4.9.
...
#   CACHE_SIZE:  The size of CabinDB block cache used in db_bench.  Default: 1G
...
function main {
  TEST_ROOT_DIR=${TEST_PATH:-"/tmp/cabindb/regression_test"}
