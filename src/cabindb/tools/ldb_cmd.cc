...
void InternalDumpCommand::DoCommand() {
  ...
  if (print_stats_) {
    std::string stats;
    if (db_->GetProperty(GetCfHandle(), "cabindb.stats", &stats)) {
      fprintf(stdout, "%s\n", stats.c_str());
    }
  }
...
void DBDumperCommand::DoDumpCommand() {
  ...
  if (print_stats_) {
    std::string stats;
    if (db_->GetProperty("cabindb.stats", &stats)) {
      fprintf(stdout, "%s\n", stats.c_str());
    }
  }
...
std::vector<std::string> ReduceDBLevelsCommand::PrepareArgs(
  ...
  ret.push_back("--" + ARG_NEW_LEVELS + "=" +
                CABINDB_NAMESPACE::ToString(new_levels));
  if(print_old_level) {
...
void ChangeCompactionStyleCommand::DoCommand() {
  ...
  for (int i = 0; i < db_->NumberLevels(GetCfHandle()); i++) {
    db_->GetProperty(GetCfHandle(),
                     "cabindb.num-files-at-level" + NumberToString(i),
                     &property);
...
void DumpSstFile(Options options, std::string filename, bool output_hex,
  ...
  CABINDB_NAMESPACE::SstFileDumper dumper(
...
void DBFileDumperCommand::DoCommand() {
  ...
  std::cout << "==============================" << std::endl;
  CABINDB_NAMESPACE::VectorLogPtr wal_files;
  ...
  if (allow_global_seqno_) {
    if (!write_global_seqno_) {
      fprintf(stderr,
              ...
              "prevent older versions of CabinDB from being able to open it\n");
    }
  } else {
    if (write_global_seqno_) {
      exec_state_ = LDBCommandExecuteResult::Failed(
