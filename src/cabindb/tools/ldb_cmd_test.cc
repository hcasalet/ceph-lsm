...
#include "cabindb/utilities/ldb_cmd.h"
...
TEST_F(LdbCmdTest, HexToString) {
  ...
  for (const auto& inPair : inputMap) {
    auto actual = CABINDB_NAMESPACE::LDBCommand::HexToString(inPair.first);
    ...
    auto reverse = CABINDB_NAMESPACE::LDBCommand::StringToHex(actual);
...
TEST_F(LdbCmdTest, HexToStringBadInputs) {
  ...
  for (const auto& badInput : badInputs) {
    try {
      CABINDB_NAMESPACE::LDBCommand::HexToString(badInput);
...
TEST_F(LdbCmdTest, OptionParsing) {
  ...
  opts.env = TryLoadCustomOrDefaultEnv();
  {
    ...
    LDBCommand* command = CABINDB_NAMESPACE::LDBCommand::InitFromCmdLineArgs(
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr, "SKIPPED as LDBCommand is not supported in CABINDB_LITE\n");
  return 0;
}
