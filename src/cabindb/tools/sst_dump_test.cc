...
TEST_F(SSTDumpToolTest, EmptyFilter) {
  Options opts;
  opts.env = env();
  std::string file_path = MakeFilePath("cabindb_sst_test.sst");
...
TEST_F(SSTDumpToolTest, FilterBlock) {
  ...
  table_opts.filter_policy.reset(
      CABINDB_NAMESPACE::NewBloomFilterPolicy(10, true));
...
TEST_F(SSTDumpToolTest, FullFilterBlock) {
  ...
  BlockBasedTableOptions table_opts;
  table_opts.filter_policy.reset(
      CABINDB_NAMESPACE::NewBloomFilterPolicy(10, false));
  opts.table_factory.reset(new BlockBasedTableFactory(table_opts));
  std::string file_path = MakeFilePath("cabindb_sst_test.sst");
  createSST(opts, file_path);
...
TEST_F(SSTDumpToolTest, ValidSSTPath) {
  ...
  std::string sst_file = MakeFilePath("cabindb_sst_test.sst");
...
TEST_F(SSTDumpToolTest, RawOutput) {
  ...
  const std::string raw_path = MakeFilePath("cabindb_sst_test_dump.txt");
...
int main(int /*argc*/, char** /*argv*/) {
  fprintf(stderr, "SKIPPED as SSTDumpTool is not supported in CABINDB_LITE\n");
  return 0;
}

#endif  // !CABINDB_LITE  return RUN_ALL_TESTS();
