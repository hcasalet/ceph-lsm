...
class BenchmarkRunner(ABC):
    ...
    def get_info_log_file_name(log_dir, db_path):
        ...
        # db_log_dir=/tmp/cabin/, then the name of the log file will be
        # 'dev_shm_LOG' and its location will be /tmp/cabin. If db_log_dir is
        # not specified in the OPTIONS file, then the location of the log file
        # will be /dev/shm and the name of the file will be 'LOG'
        file_name = ''
        if log_dir:
            # refer GetInfoLogPrefix() in cabindb/util/filename.cc
