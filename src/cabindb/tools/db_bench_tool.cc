...
DEFINE_int64(db_write_buffer_size,
             CABINDB_NAMESPACE::Options().db_write_buffer_size,
             "Number of bytes to buffer in all memtables before compacting");

DEFINE_bool(cost_write_buffer_to_cache, false,
            "The usage of memtable is costed to the block cache");

DEFINE_int64(write_buffer_size, CABINDB_NAMESPACE::Options().write_buffer_size,
             "Number of bytes to buffer in memtable before compacting");

DEFINE_int32(max_write_buffer_number,
             CABINDB_NAMESPACE::Options().max_write_buffer_number,
             "The number of in-memory memtables. Each memtable is of size"
             " write_buffer_size bytes.");

DEFINE_int32(min_write_buffer_number_to_merge,
             CABINDB_NAMESPACE::Options().min_write_buffer_number_to_merge,
...
DEFINE_int32(max_write_buffer_number_to_maintain,
             CABINDB_NAMESPACE::Options().max_write_buffer_number_to_maintain,
...
DEFINE_int64(max_write_buffer_size_to_maintain,
             CABINDB_NAMESPACE::Options().max_write_buffer_size_to_maintain,
...
DEFINE_int32(max_background_jobs,
             CABINDB_NAMESPACE::Options().max_background_jobs,
...
DEFINE_int32(max_background_compactions,
             CABINDB_NAMESPACE::Options().max_background_compactions,
             "The maximum number of concurrent background compactions"
             " that can occur in parallel.");
...
DEFINE_int32(max_background_flushes,
             CABINDB_NAMESPACE::Options().max_background_flushes,
             "The maximum number of concurrent background flushes"
             " that can occur in parallel.");

static CABINDB_NAMESPACE::CompactionStyle FLAGS_compaction_style_e;
DEFINE_int32(compaction_style,
             (int32_t)CABINDB_NAMESPACE::Options().compaction_style,
             "style of compaction: level-based, universal and fifo");

static CABINDB_NAMESPACE::CompactionPri FLAGS_compaction_pri_e;
DEFINE_int32(compaction_pri,
             (int32_t)CABINDB_NAMESPACE::Options().compaction_pri,
...
DEFINE_bool(
    ...
    CABINDB_NAMESPACE::BlockBasedTableOptions().optimize_filters_for_memory,
...
DEFINE_int64(metadata_block_size,
             CABINDB_NAMESPACE::BlockBasedTableOptions().metadata_block_size,
...
DEFINE_int32(block_size,
             static_cast<int32_t>(
                 CABINDB_NAMESPACE::BlockBasedTableOptions().block_size),
             "Number of bytes in a block.");

DEFINE_int32(format_version,
             static_cast<int32_t>(
                 CABINDB_NAMESPACE::BlockBasedTableOptions().format_version),
             "Format version of SST files.");

DEFINE_int32(block_restart_interval,
             CABINDB_NAMESPACE::BlockBasedTableOptions().block_restart_interval,
             "Number of keys between restart points "
             "for delta encoding of keys in data block.");

DEFINE_int32(
    index_block_restart_interval,
    CABINDB_NAMESPACE::BlockBasedTableOptions().index_block_restart_interval,
    "Number of keys between restart points "
    "for delta encoding of keys in index block.");

DEFINE_int32(read_amp_bytes_per_bit,
             CABINDB_NAMESPACE::BlockBasedTableOptions().read_amp_bytes_per_bit,
             "Number of bytes per bit to be used in block read-amp bitmap");

DEFINE_bool(
    enable_index_compression,
    CABINDB_NAMESPACE::BlockBasedTableOptions().enable_index_compression,
    "Compress the index block");

DEFINE_bool(block_align,
            CABINDB_NAMESPACE::BlockBasedTableOptions().block_align,
...
DEFINE_int32(open_files, CABINDB_NAMESPACE::Options().max_open_files,
             "Maximum number of files to keep open at the same time"
             " (use default if == 0)");

DEFINE_int32(file_opening_threads,
             CABINDB_NAMESPACE::Options().max_file_opening_threads,
...
DEFINE_int32(stats_level, CABINDB_NAMESPACE::StatsLevel::kExceptDetailedTimers,
             "stats level for statistics");
DEFINE_string(statistics_string, "", "Serialized statistics string");
static class std::shared_ptr<CABINDB_NAMESPACE::Statistics> dbstats;
...
DEFINE_int64(target_file_size_base,
             CABINDB_NAMESPACE::Options().target_file_size_base,
             "Target file size at level-1");

DEFINE_int32(target_file_size_multiplier,
             CABINDB_NAMESPACE::Options().target_file_size_multiplier,
             "A multiplier to compute target level-N file size (N >= 2)");

DEFINE_uint64(max_bytes_for_level_base,
              CABINDB_NAMESPACE::Options().max_bytes_for_level_base,
...
DEFINE_int32(level0_stop_writes_trigger,
             CABINDB_NAMESPACE::Options().level0_stop_writes_trigger,
             "Number of files in level-0"
             " that will trigger put stop.");

DEFINE_int32(level0_slowdown_writes_trigger,
             CABINDB_NAMESPACE::Options().level0_slowdown_writes_trigger,
             "Number of files in level-0"
             " that will slow down writes.");

DEFINE_int32(level0_file_num_compaction_trigger,
             CABINDB_NAMESPACE::Options().level0_file_num_compaction_trigger,
             "Number of files in level-0"
             " when compactions start");

DEFINE_uint64(periodic_compaction_seconds,
              CABINDB_NAMESPACE::Options().periodic_compaction_seconds,
...
DEFINE_string(
    options_file, "",
    "The path to a CabinDB options file.  If specified, then db_bench will "
    "run with the CabinDB options in the default column family of the "
    "specified options file. "
    "Note that with this setting, db_bench will ONLY accept the following "
    "CabinDB options related command-line arguments, all other arguments "
    "that are related to CabinDB options will be ignored:\n"
    "\t--use_existing_db\n"
    "\t--use_existing_keys\n"
    "\t--statistics\n"
    "\t--row_cache_size\n"
...
DEFINE_bool(
    blob_db_enable_gc,
    CABINDB_NAMESPACE::blob_db::BlobDBOptions().enable_garbage_collection,
    "Enable BlobDB garbage collection.");

DEFINE_double(
    blob_db_gc_cutoff,
    CABINDB_NAMESPACE::blob_db::BlobDBOptions().garbage_collection_cutoff,
    "Cutoff ratio for BlobDB garbage collection.");

DEFINE_bool(blob_db_is_fifo,
            CABINDB_NAMESPACE::blob_db::BlobDBOptions().is_fifo,
            "Enable FIFO eviction strategy in BlobDB.");

DEFINE_uint64(blob_db_max_db_size,
              CABINDB_NAMESPACE::blob_db::BlobDBOptions().max_db_size,
...
DEFINE_uint64(blob_db_ttl_range_secs,
              CABINDB_NAMESPACE::blob_db::BlobDBOptions().ttl_range_secs,
              "TTL bucket size to use when creating blob files.");

DEFINE_uint64(blob_db_min_blob_size,
              CABINDB_NAMESPACE::blob_db::BlobDBOptions().min_blob_size,
...
DEFINE_uint64(blob_db_bytes_per_sync,
              CABINDB_NAMESPACE::blob_db::BlobDBOptions().bytes_per_sync,
              "Bytes to sync blob file at.");

DEFINE_uint64(blob_db_file_size,
              CABINDB_NAMESPACE::blob_db::BlobDBOptions().blob_file_size,
              "Target size of each blob file.");

DEFINE_string(blob_db_compression_type, "snappy",
              "Algorithm to use to compress blob in blob file");
static enum CABINDB_NAMESPACE::CompressionType
    FLAGS_blob_db_compression_type_e = CABINDB_NAMESPACE::kSnappyCompression;

// Secondary DB instance Options
DEFINE_bool(use_secondary_db, false,
            "Open a CabinDB secondary instance. A primary instance can be "
...
static enum CABINDB_NAMESPACE::CompressionType StringToCompressionType(
    const char* ctype) {
  assert(ctype);

  if (!strcasecmp(ctype, "none"))
    return CABINDB_NAMESPACE::kNoCompression;
  else if (!strcasecmp(ctype, "snappy"))
    return CABINDB_NAMESPACE::kSnappyCompression;
  else if (!strcasecmp(ctype, "zlib"))
    return CABINDB_NAMESPACE::kZlibCompression;
  else if (!strcasecmp(ctype, "bzip2"))
    return CABINDB_NAMESPACE::kBZip2Compression;
  else if (!strcasecmp(ctype, "lz4"))
    return CABINDB_NAMESPACE::kLZ4Compression;
  else if (!strcasecmp(ctype, "lz4hc"))
    return CABINDB_NAMESPACE::kLZ4HCCompression;
  else if (!strcasecmp(ctype, "xpress"))
    return CABINDB_NAMESPACE::kXpressCompression;
  else if (!strcasecmp(ctype, "zstd"))
    return CABINDB_NAMESPACE::kZSTD;

  fprintf(stdout, "Cannot parse compression type '%s'\n", ctype);
  return CABINDB_NAMESPACE::kSnappyCompression;  // default value
}

static std::string ColumnFamilyName(size_t i) {
  if (i == 0) {
    return CABINDB_NAMESPACE::kDefaultColumnFamilyName;
  } else {
    char name[100];
...
static enum CABINDB_NAMESPACE::CompressionType FLAGS_compression_type_e =
    CABINDB_NAMESPACE::kSnappyCompression;

DEFINE_int64(sample_for_compression, 0, "Sample every N block for compression");

DEFINE_int32(compression_level, CABINDB_NAMESPACE::CompressionOptions().level,
...
DEFINE_int32(compression_max_dict_bytes,
             CABINDB_NAMESPACE::CompressionOptions().max_dict_bytes,
             "Maximum size of dictionary used to prime the compression "
             "library.");

DEFINE_int32(compression_zstd_max_train_bytes,
             CABINDB_NAMESPACE::CompressionOptions().zstd_max_train_bytes,
...
static std::shared_ptr<CABINDB_NAMESPACE::Env> env_guard;

static CABINDB_NAMESPACE::Env* FLAGS_env = CABINDB_NAMESPACE::Env::Default();
...
DEFINE_int32(perf_level, CABINDB_NAMESPACE::PerfLevel::kDisable,
             "Level of perf collection");

#ifndef CABINDB_LITE
static CABINDB_NAMESPACE::Env* GetCompositeEnv(
    std::shared_ptr<CABINDB_NAMESPACE::FileSystem> fs) {
  static std::shared_ptr<CABINDB_NAMESPACE::Env> composite_env =
      CABINDB_NAMESPACE::NewCompositeEnv(fs);
...
DEFINE_uint64(
    ...
    "If non-zero, db_bench will rate-limit the writes going into CabinDB. This "
...
DEFINE_uint64(
    ...
    "If non-zero, db_bench will rate-limit the reads from CabinDB. This "
    "is the global rate in ops/second.");

DEFINE_uint64(max_compaction_bytes,
              CABINDB_NAMESPACE::Options().max_compaction_bytes,
...
DEFINE_bool(mmap_read, CABINDB_NAMESPACE::Options().allow_mmap_reads,
            "Allow reads to occur via mmap-ing files");

DEFINE_bool(mmap_write, CABINDB_NAMESPACE::Options().allow_mmap_writes,
            "Allow writes to occur via mmap-ing files");

DEFINE_bool(use_direct_reads, CABINDB_NAMESPACE::Options().use_direct_reads,
            "Use O_DIRECT for reading data");

DEFINE_bool(use_direct_io_for_flush_and_compaction,
            CABINDB_NAMESPACE::Options().use_direct_io_for_flush_and_compaction,
            "Use O_DIRECT for background flush and compaction writes");

DEFINE_bool(advise_random_on_open,
            CABINDB_NAMESPACE::Options().advise_random_on_open,
            "Advise random access on table file open");

DEFINE_string(compaction_fadvice, "NORMAL",
              "Access pattern advice when a file is compacted");
static auto FLAGS_compaction_fadvice_e =
    CABINDB_NAMESPACE::Options().access_hint_on_compaction_start;

DEFINE_bool(use_tailing_iterator, false,
            "Use tailing iterator to access a series of keys instead of get");

DEFINE_bool(use_adaptive_mutex, CABINDB_NAMESPACE::Options().use_adaptive_mutex,
            "Use adaptive mutex");

DEFINE_uint64(bytes_per_sync, CABINDB_NAMESPACE::Options().bytes_per_sync,
              "Allows OS to incrementally sync SST files to disk while they are"
              " being written, in the background. Issue one request for every"
              " bytes_per_sync written. 0 turns it off.");

DEFINE_uint64(wal_bytes_per_sync,
              CABINDB_NAMESPACE::Options().wal_bytes_per_sync,
...
DEFINE_uint64(stats_dump_period_sec,
              CABINDB_NAMESPACE::Options().stats_dump_period_sec,
              "Gap between printing stats to log in seconds");
DEFINE_uint64(stats_persist_period_sec,
              CABINDB_NAMESPACE::Options().stats_persist_period_sec,
              "Gap between persisting stats in seconds");
DEFINE_bool(persist_stats_to_disk,
            CABINDB_NAMESPACE::Options().persist_stats_to_disk,
            "whether to persist stats to disk");
DEFINE_uint64(stats_history_buffer_size,
              CABINDB_NAMESPACE::Options().stats_history_buffer_size,
...
class ReporterAgent {
 ...
 private:
  ...
  CABINDB_NAMESPACE::port::Thread reporting_thread_;
  std::mutex mutex_;
  // will notify on stop
          ...
          if (id_ == 0 && FLAGS_stats_per_interval) {
            ...
            if (db_with_cfh && db_with_cfh->num_created.load()) {
              for (size_t i = 0; i < db_with_cfh->num_created.load(); ++i) {
                if (db->GetProperty(db_with_cfh->cfh[i], "cabindb.cfstats",
                                    &stats))
                  fprintf(stderr, "%s\n", stats.c_str());
                if (FLAGS_show_table_properties) {
                  for (int level = 0; level < FLAGS_num_levels; ++level) {
                    if (db->GetProperty(
                            db_with_cfh->cfh[i],
                            "cabindb.aggregated-table-properties-at-level" +
                                ToString(level),
                            &stats)) {
              ...
              if (db->GetProperty("cabindb.stats", &stats)) {
                fprintf(stderr, "%s\n", stats.c_str());
              }
              if (FLAGS_show_table_properties) {
                for (int level = 0; level < FLAGS_num_levels; ++level) {
...
#endif
    if (FLAGS_compression_type_e != CABINDB_NAMESPACE::kNoCompression) {
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  void PrintEnvironment() {
    fprintf(stderr, "CabinDB:    version %d.%d\n",
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (name == "fillseqdeterministic" ||
        ...
        PrintStats("cabindb.stats");
      } else if (name == "resetstats") {
        ResetStats();
      } else if (name == "verify") {
        VerifyDBFromDB(FLAGS_truth_db);
      } else if (name == "levelstats") {
        PrintStats("cabindb.levelstats");
      } else if (name == "sstables") {
        PrintStats("cabindb.sstables");
 ...
 private:
...
#ifndef CABINDB_LITE
    printf("Initializing CabinDB Options from the specified file\n");
    DBOptions db_opts;
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    printf("Initializing CabinDB Options from command-line flags\n");
    ...
    if (FLAGS_use_plain_table) {
      ...
      CABINDB_NAMESPACE::CuckooTableOptions table_options;
      ...
      if (FLAGS_use_data_block_hash_index) {
        block_based_options.data_block_index_type =
            CABINDB_NAMESPACE::BlockBasedTableOptions::kDataBlockBinaryAndHash;
      } else {
        block_based_options.data_block_index_type =
            CABINDB_NAMESPACE::BlockBasedTableOptions::kDataBlockBinarySearch;
      }
    ...
    if (FLAGS_user_timestamp_size > 0) {
      ...
      options.comparator = CABINDB_NAMESPACE::test::ComparatorWithU64Ts();
    }
    ...
    if (compaction_style == kCompactionStyleLevel) {
      ...
      for (size_t i = 0; i < num_db; i++) {
        if (sorted_runs[i].size() < num_levels - 1) {
          fprintf(stderr, "n is too small to fill %" CABINDB_PRIszt " levels\n", num_levels);
          exit(1);
        }
      }
      for (size_t i = 0; i < num_db; i++) {
...
// optimization, so skip the check of the max level.
      ...
      for (auto& levelMeta : meta.levels) {
        ...
        if (levelMeta.level == 0) {
          for (auto& fileMeta : levelMeta.files) {
            fprintf(stdout, "Level[%d]: %s(size: %" CABINDB_PRIszt " bytes)\n",
    ...
    (void)write_mode;
    fprintf(stderr, "Cabindb Lite doesn't support filldeterministic\n");
    return Status::NotSupported(
        "Cabindb Lite doesn't support filldeterministic");
#endif  // CABINDB_LITE
  }
  ...
  void ReadSequential(ThreadState* thread, DB* db) {
    ...
    delete iter;
    thread->stats.AddBytes(bytes);
    if (FLAGS_perf_level > CABINDB_NAMESPACE::PerfLevel::kDisable) {
  ...
  void ReadRandomMergeRandom(ThreadState* thread) {
    ...
    snprintf(msg, sizeof(msg),
             ...
             " hits:%" PRIu64 " maxlength:%" CABINDB_PRIszt ")",
  ...
  //
  // This benchmark is intended to partially replicate the behavior of MyCabin
...
int db_bench_tool(int argc, char** argv) {
  ...
  FLAGS_compaction_style_e =
      (CABINDB_NAMESPACE::CompactionStyle)FLAGS_compaction_style;
#ifndef CABINDB_LITE
...
#endif  // CABINDB_LITE
  if (FLAGS_statistics) {
    dbstats = CABINDB_NAMESPACE::CreateDBStatistics();
  }
  if (dbstats) {
    dbstats->set_stats_level(static_cast<StatsLevel>(FLAGS_stats_level));
  }
  FLAGS_compaction_pri_e =
      (CABINDB_NAMESPACE::CompactionPri)FLAGS_compaction_pri;

  std::vector<std::string> fanout = CABINDB_NAMESPACE::StringSplit(
  ...
  if (!FLAGS_hdfs.empty()) {
    FLAGS_env = new CABINDB_NAMESPACE::HdfsEnv(FLAGS_hdfs);
  }

  if (!strcasecmp(FLAGS_compaction_fadvice.c_str(), "NONE"))
    FLAGS_compaction_fadvice_e = CABINDB_NAMESPACE::Options::NONE;
  else if (!strcasecmp(FLAGS_compaction_fadvice.c_str(), "NORMAL"))
    FLAGS_compaction_fadvice_e = CABINDB_NAMESPACE::Options::NORMAL;
  else if (!strcasecmp(FLAGS_compaction_fadvice.c_str(), "SEQUENTIAL"))
    FLAGS_compaction_fadvice_e = CABINDB_NAMESPACE::Options::SEQUENTIAL;
  else if (!strcasecmp(FLAGS_compaction_fadvice.c_str(), "WILLNEED"))
    FLAGS_compaction_fadvice_e = CABINDB_NAMESPACE::Options::WILLNEED;
  else {
  ...
  FLAGS_env->SetBackgroundThreads(FLAGS_num_high_pri_threads,
                                  CABINDB_NAMESPACE::Env::Priority::HIGH);
  FLAGS_env->SetBackgroundThreads(FLAGS_num_bottom_pri_threads,
                                  CABINDB_NAMESPACE::Env::Priority::BOTTOM);
  FLAGS_env->SetBackgroundThreads(FLAGS_num_low_pri_threads,
                                  CABINDB_NAMESPACE::Env::Priority::LOW);
  ...
  CABINDB_NAMESPACE::Benchmark benchmark;
  benchmark.Run();

#ifndef CABINDB_LITE
  if (FLAGS_print_malloc_stats) {
    std::string stats_string;
    CABINDB_NAMESPACE::DumpMallocStats(&stats_string);
