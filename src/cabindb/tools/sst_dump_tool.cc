...
#include "port/port.h"
#include "include/cabindb/utilities/ldb_cmd.h"
...
int SSTDumpTool::Run(int argc, char const* const* argv, Options options) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strncmp(argv[i], "--env_uri=", 10) == 0) {
      ...
      try {
        in_key = CABINDB_NAMESPACE::LDBCommand::HexToString(in_key);
      } catch (...) {
        std::cerr << "ERROR: Invalid key input '"
          << in_key
          << "' Use 0x{hex representation of internal cabindb key}" << std::endl;
        return -1;
      }
      Slice sl_key = CABINDB_NAMESPACE::Slice(in_key);
      ...
      printf("sst_dump from CabinDB %d.%d.%d\n", CABINDB_MAJOR, CABINDB_MINOR,
             CABINDB_PATCH);
      return 0;
    } else {
  ...
  if (input_key_hex) {
    if (has_from || use_from_as_prefix) {
      from_key = CABINDB_NAMESPACE::LDBCommand::HexToString(from_key);
    }
    if (has_to) {
      to_key = CABINDB_NAMESPACE::LDBCommand::HexToString(to_key);
    }
  }
  ...
  std::shared_ptr<CABINDB_NAMESPACE::Env> env_guard;
  ...
  if (!options.env || options.env == CABINDB_NAMESPACE::Env::Default()) {
    Env* env = Env::Default();
  ...
  CABINDB_NAMESPACE::Env* env = options.env;
  CABINDB_NAMESPACE::Status st = env->GetChildren(dir_or_file, &filenames);
  bool dir = true;
  ...
  // List of CabinDB SST file without corruption
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    CABINDB_NAMESPACE::SstFileDumper dumper(options, filename, readahead_size,
      ...
      if (valid_sst_files.size() == 1) {
        ...
        if (command == "check" || command == "scan" || command == "") {
          fprintf(stdout, "from [%s] to [%s]\n",
                  CABINDB_NAMESPACE::Slice(from_key).ToString(true).c_str(),
                  CABINDB_NAMESPACE::Slice(to_key).ToString(true).c_str());
        }
      }
    }
    ...
    if (show_properties || show_summary) {
      const CABINDB_NAMESPACE::TableProperties* table_properties;
