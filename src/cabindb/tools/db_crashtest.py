...
def get_dbname(test_name):
    test_dir_name = "cabindb_crashtest_" + test_name
...
def setup_expected_values_file():
    ...
    expected_file_name = "cabindb_crashtest_" + "expected"
    test_tmpdir = os.environ.get(_TEST_DIR_ENV_VAR)
    if test_tmpdir is None or test_tmpdir == "":
...
cf_consistency_params = {
    ...
    # use small value for write_buffer_size so that CabinDB triggers flush
    # more frequently
...
def finalize_and_sanitize(src_params):
    ...
    # Only under WritePrepared txns, unordered_write would provide the same guarnatees as vanilla cabindb
...
# in case of unsafe crashes in CabinDB.
...
# kill_random_test that causes cabindb to crash at various points in code.
