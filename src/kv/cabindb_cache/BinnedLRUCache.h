...
#ifndef CABINDB_BINNED_LRU_CACHE
#define CABINDB_BINNED_LRU_CACHE
...
struct BinnedLRUHandle {
  void* value;
  void (*deleter)(const cabindb::Slice&, void* value);
  ...
  cabindb::Slice key() const {
    // For cheaper lookups, we allow a temporary Handle object
    // to store a pointer to a key in "value".
    if (next == this) {
      return *(reinterpret_cast<cabindb::Slice*>(value));
    } else {
      return cabindb::Slice(key_data, key_length);
    }
  }
  ...
  void SetPriority(cabindb::Cache::Priority priority) {
    if (priority == cabindb::Cache::Priority::HIGH) {
      flags |= 2;
    } else {
      flags &= ~2;
    }
...
class BinnedLRUHandleTable {
 public:
  ...
  BinnedLRUHandle* Lookup(const cabindb::Slice& key, uint32_t hash);
  BinnedLRUHandle* Insert(BinnedLRUHandle* h);
  BinnedLRUHandle* Remove(const cabindb::Slice& key, uint32_t hash);
 ...
 private:
  ...
  BinnedLRUHandle** FindPointer(const cabindb::Slice& key, uint32_t hash);
...
class alignas(CACHE_LINE_SIZE) BinnedLRUCacheShard : public CacheShard {
 public:
  ...
  virtual cabindb::Status Insert(const cabindb::Slice& key, uint32_t hash, void* value,
                        ...
                        cabindb::Cache::Handle** handle,
                        cabindb::Cache::Priority priority) override;
  virtual cabindb::Cache::Handle* Lookup(const cabindb::Slice& key, uint32_t hash) override;
  virtual bool Ref(cabindb::Cache::Handle* handle) override;
  virtual bool Release(cabindb::Cache::Handle* handle,
                       bool force_erase = false) override;
  virtual void Erase(const cabindb::Slice& key, uint32_t hash) override;
...
class BinnedLRUCache : public ShardedCache {
 public:
  ...
  virtual std::string get_cache_name() const {
    return "CabinDB Binned LRU Cache";
  }
...
#endif // CABINDB_BINNED_LRU_CACHE
