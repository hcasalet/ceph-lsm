...
BinnedLRUHandle* BinnedLRUHandleTable::Lookup(const cabindb::Slice& key, uint32_t hash) {
...
BinnedLRUHandle* BinnedLRUHandleTable::Remove(const cabindb::Slice& key, uint32_t hash) {
...
BinnedLRUHandle** BinnedLRUHandleTable::FindPointer(const cabindb::Slice& key, uint32_t hash) {
...
cabindb::Cache::Handle* BinnedLRUCacheShard::Lookup(const cabindb::Slice& key, uint32_t hash) {
  std::lock_guard<std::mutex> l(mutex_);
  ...
  return reinterpret_cast<cabindb::Cache::Handle*>(e);
}

bool BinnedLRUCacheShard::Ref(cabindb::Cache::Handle* h) {
...
bool BinnedLRUCacheShard::Release(cabindb::Cache::Handle* handle, bool force_erase) {
  if (handle == nullptr) {
    return false;
  }
...
cabindb::Status BinnedLRUCacheShard::Insert(const cabindb::Slice& key, uint32_t hash, void* value,
                             size_t charge,
                             void (*deleter)(const cabindb::Slice& key, void* value),
                             cabindb::Cache::Handle** handle, cabindb::Cache::Priority priority) {
...

  {
    ...
    if (usage_ - lru_usage_ + charge > capacity_ &&
      ...
      if (handle == nullptr) {
        ...
        delete e;
        *handle = nullptr;
        s = cabindb::Status::Incomplete("Insert failed due to LRU cache being full.");
      }
    } else {
      ...
      if (handle == nullptr) {
        ...
        *handle = reinterpret_cast<cabindb::Cache::Handle*>(e);
      }
      s = cabindb::Status::OK();
    }
  }
...
void BinnedLRUCacheShard::Erase(const cabindb::Slice& key, uint32_t hash) {
  BinnedLRUHandle* e;
  bool last_reference = false;
  {
    std::lock_guard<std::mutex> l(mutex_);
...
int64_t BinnedLRUCache::request_cache_bytes(PriorityCache::Priority pri, uint64_t total_cache) const
{
  ...
  switch (pri) {
  // PRI0 is for cabindb's high priority items (indexes/filters)
...
std::shared_ptr<cabindb::Cache> NewBinnedLRUCache(
    CephContext *c, 
    size_t capacity,
    int num_shard_bits,
    bool strict_capacity_limit,
