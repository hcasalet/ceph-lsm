...
#include "cabindb/include/cabindb/env.h"
#include "cabindb/include/cabindb/slice.h"
#include "cabindb/include/cabindb/cache.h"
#include "cabindb/include/cabindb/filter_policy.h"
#include "cabindb/include/cabindb/utilities/convenience.h"
#include "cabindb/include/cabindb/merge_operator.h"
...
#define dout_subsys ceph_subsys_cabindb
#undef dout_prefix
#define dout_prefix *_dout << "cabindb: "
...
static bufferlist to_bufferlist(cabindb::Slice in) {
  bufferlist bl;
  bl.append(bufferptr(in.data(), in.size()));
  return bl;
}

static cabindb::SliceParts prepare_sliceparts(const bufferlist &bl,
					      vector<cabindb::Slice> *slices)
{
  unsigned n = 0;
  ...
  return cabindb::SliceParts(slices->data(), slices->size());
}


//
// One of these for the default cabindb column family, routing each prefix
// to the appropriate MergeOperator.
//
class CabinDBStore::MergeOperatorRouter
  : public cabindb::AssociativeMergeOperator
{
  CabinDBStore& store;
public:
  const char *Name() const override {
    // Construct a name that cabinDB will validate against. We want to
  ...
  explicit MergeOperatorRouter(CabinDBStore &_store) : store(_store) {}

  bool Merge(const cabindb::Slice& key,
	     const cabindb::Slice* existing_value,
	     const cabindb::Slice& value,
	     std::string* new_value,
	     cabindb::Logger* logger) const override {
...
//
class CabinDBStore::MergeOperatorLinker
  : public cabindb::AssociativeMergeOperator
{
private:
...
int CabinDBStore::set_merge_operator(
  const string& prefix,
  std::shared_ptr<KeyValueDB::MergeOperator> mop)
{
...
class CephCabindbLogger : public cabindb::Logger {
  CephContext *cct;
public:
  explicit CephCabindbLogger(CephContext *c) : cct(c) {
    cct->get();
  }
  ...
  void Logv(const char* format, va_list ap) override {
    Logv(cabindb::INFO_LEVEL, format, ap);
  }
  ...
  // printed.
  void Logv(const cabindb::InfoLogLevel log_level, const char* format,
	    va_list ap) override {
    int v = cabindb::NUM_INFO_LOG_LEVELS - log_level - 1;
...
cabindb::Logger *create_cabindb_ceph_logger()
{
  return new CephCabindbLogger(g_ceph_context);
}
...
// this function is a modification of cabindb's StringToMap:
// 1) accepts ' \n ; as separators
// 2) leaves compound options with enclosing { and }
cabindb::Status StringToMap2(const std::string& opts_str,
			    std::unordered_map<std::string, std::string>* opts_map)
{
  using cabindb::Status;
  using cabindb::trim;
  assert(opts_map);
  // Example:
...
int CabinDBStore::tryInterpret(const string &key, const string &val, cabindb::Options &opt)
{
  if (key == "compaction_threads") {
    ...
    opt.env->SetBackgroundThreads(f, cabindb::Env::Priority::LOW);
    ...
    opt.env->SetBackgroundThreads(f, cabindb::Env::Priority::HIGH);
...
int CabinDBStore::ParseOptionsFromString(const string &opt_str, cabindb::Options &opt)
{
  return ParseOptionsFromStringStatic(cct, opt_str, opt,
    [&](const string& k, const string& v, cabindb::Options& o) {
      return tryInterpret(k, v, o);
    }
  );
}

int CabinDBStore::ParseOptionsFromStringStatic(
  CephContext *cct,
  const string& opt_str,
  cabindb::Options& opt,
  function<int(const string&, const string&, cabindb::Options&)> interp)
{
  ...
  int r;
  cabindb::Status status;
  std::unordered_map<std::string, std::string> str_map;
  status = StringToMap2(opt_str, &str_map);
  if (!status.ok()) {
  ...
  for (auto it = str_map.begin(); it != str_map.end(); ++it) {
    ...
    cabindb::Status status =
      cabindb::GetOptionsFromString(opt, this_opt, &opt);
    if (!status.ok()) {
      if (interp != nullptr) {
    ...
    lgeneric_dout(cct, 1) << " set cabindb option " << it->first
      << " = " << it->second << dendl;
  }
  return 0;
}

int CabinDBStore::init(string _options_str)
{
...
int CabinDBStore::create_db_dir()
{
  if (env) {
    unique_ptr<cabindb::Directory> dir;
...
int CabinDBStore::install_cf_mergeop(
  const string &key_prefix,
  cabindb::ColumnFamilyOptions *cf_opt)
{
...
std::shared_ptr<cabindb::Cache> CabinDBStore::create_block_cache(
  ...
  std::shared_ptr<cabindb::Cache> cache;
  auto shard_bits = cct->_conf->cabindb_cache_shard_bits;
  if (cache_type == "binned_lru") {
    cache = cabindb_cache::NewBinnedLRUCache(cct, cache_size, shard_bits, false, cache_prio_high);
  } else if (cache_type == "lru") {
    cache = cabindb::NewLRUCache(cache_size, shard_bits);
  } else if (cache_type == "clock") {
    cache = cabindb::NewClockCache(cache_size, shard_bits);
    if (!cache) {
      derr << "cabindb_cache_type '" << cache
           << "' chosen, but CabinDB not compiled with LibTBB. "
           << dendl;
    }
  } else {
    derr << "unrecognized cabindb_cache_type '" << cache_type << "'" << dendl;
  }
  return cache;
}

int CabinDBStore::load_cabindb_options(bool create_if_missing, cabindb::Options& opt)
{
  cabindb::Status status;
  ...
  if (cct->_conf->cabindb_perf)  {
    dbstats = cabindb::CreateDBStatistics();
  ...
  try {
	...
	opt.db_paths.push_back(cabindb::DbPath(path, size));
  ...
  if (cct->_conf->cabindb_log_to_ceph_log) {
    opt.info_log.reset(new CephCabindbLogger(cct));
  }

  if (priv) {
    dout(10) << __func__ << " using custom Env " << priv << dendl;
    opt.env = static_cast<cabindb::Env*>(priv);
  } else {
    env = opt.env;
  }
  ...
  // caches
  if (!set_cache_flag) {
    cache_size = cct->_conf->cabindb_cache_size;
  }
  uint64_t row_cache_size = cache_size * cct->_conf->cabindb_cache_row_ratio;
  uint64_t block_cache_size = cache_size - row_cache_size;

  bbt_opts.block_cache = create_block_cache(cct->_conf->cabindb_cache_type, block_cache_size);
  if (!bbt_opts.block_cache) {
    return -EINVAL;
  }
  bbt_opts.block_size = cct->_conf->cabindb_block_size;

  if (row_cache_size > 0)
    opt.row_cache = cabindb::NewLRUCache(row_cache_size,
				     cct->_conf->cabindb_cache_shard_bits);
  uint64_t bloom_bits = cct->_conf.get_val<uint64_t>("cabindb_bloom_bits_per_key");
  if (bloom_bits > 0) {
    dout(10) << __func__ << " set bloom filter bits per key to "
	     << bloom_bits << dendl;
    bbt_opts.filter_policy.reset(cabindb::NewBloomFilterPolicy(bloom_bits));
  }
  using std::placeholders::_1;
  if (cct->_conf.with_val<std::string>("cabindb_index_type",
				    std::bind(std::equal_to<std::string>(), _1,
					      "binary_search")))
    bbt_opts.index_type = cabindb::BlockBasedTableOptions::IndexType::kBinarySearch;
  if (cct->_conf.with_val<std::string>("cabindb_index_type",
				    std::bind(std::equal_to<std::string>(), _1,
					      "hash_search")))
    bbt_opts.index_type = cabindb::BlockBasedTableOptions::IndexType::kHashSearch;
  if (cct->_conf.with_val<std::string>("cabindb_index_type",
				    std::bind(std::equal_to<std::string>(), _1,
					      "two_level")))
    bbt_opts.index_type = cabindb::BlockBasedTableOptions::IndexType::kTwoLevelIndexSearch;
  if (!bbt_opts.no_block_cache) {
    bbt_opts.cache_index_and_filter_blocks =
        cct->_conf.get_val<bool>("cabindb_cache_index_and_filter_blocks");
    bbt_opts.cache_index_and_filter_blocks_with_high_priority =
        cct->_conf.get_val<bool>("cabindb_cache_index_and_filter_blocks_with_high_priority");
    bbt_opts.pin_l0_filter_and_index_blocks_in_cache =
      cct->_conf.get_val<bool>("cabindb_pin_l0_filter_and_index_blocks_in_cache");
  }
  bbt_opts.partition_filters = cct->_conf.get_val<bool>("cabindb_partition_filters");
  if (cct->_conf.get_val<Option::size_t>("cabindb_metadata_block_size") > 0)
    bbt_opts.metadata_block_size = cct->_conf.get_val<Option::size_t>("cabindb_metadata_block_size");

  opt.table_factory.reset(cabindb::NewBlockBasedTableFactory(bbt_opts));
  dout(10) << __func__ << " block size " << cct->_conf->cabindb_block_size
           << ", block_cache size " << byte_u_t(block_cache_size)
	   << ", row_cache size " << byte_u_t(row_cache_size)
	   << "; shards "
	   << (1 << cct->_conf->cabindb_cache_shard_bits)
	   << ", type " << cct->_conf->cabindb_cache_type
	   << dendl;
...
void CabinDBStore::add_column_family(const std::string& cf_name, uint32_t hash_l, uint32_t hash_h,
...
bool CabinDBStore::is_column_family(const std::string& prefix) {
  return cf_handles.count(prefix);
}

cabindb::ColumnFamilyHandle *CabinDBStore::get_cf_handle(const std::string& prefix, const std::string& key) {
...
cabindb::ColumnFamilyHandle *CabinDBStore::get_cf_handle(const std::string& prefix, const char* key, size_t keylen) {
  auto iter = cf_handles.find(prefix);
  if (iter == cf_handles.end()) {
    return nullptr;
  } else {
 ...
 */
bool CabinDBStore::parse_sharding_def(const std::string_view text_def_in,
...
void CabinDBStore::sharding_def_to_columns(const std::vector<ColumnFamily>& sharding_def,
				...
				const std::vector<ColumnFamily>& sharding_def)
{
  for (auto& p : sharding_def) {
    ...
    cabindb::Status status;
    status = cabindb::GetColumnFamilyOptionsFromMap(cf_opt, column_opts_map, &cf_opt);
    if (!status.ok()) {
...
int CabinDBStore::apply_sharding(const cabindb::Options& opt,
				 const std::string& sharding_text)
{
  ...
  if (!sharding_text.empty()) {
    ...
    status = cabindb::WriteStringToFile(opt.env, sharding_text,
...
// linking to cabindb function defined in options_helper.cc
// it can parse nested params like "nested_opt={opt1=1;opt2=2}"

extern cabindb::Status cabindb::StringToMap(const std::string& opts_str,
				   std::unordered_map<std::string, std::string>* opts_map);

int CabinDBStore::extract_block_cache_options(const std::string& opts_str,
					      std::unordered_map<std::string, std::string>* column_opts_map,
					      std::string* block_cache_opt)
{
  dout(5) << __func__ << " opts_str=" << opts_str << dendl;
  cabindb::Status status = cabindb::StringToMap(opts_str, column_opts_map);
  if (!status.ok()) {
    dout(5) << __func__ << " error '" << status.getState() <<
...
int CabinDBStore::verify_sharding(const cabindb::Options& opt,
				  ...
				  std::vector<std::pair<size_t, CabinDBStore::ColumnFamily> >& missing_cfs_shard)
{
  ...
  if (status.ok()) {
    status = cabindb::ReadFileToString(opt.env,
				       sharding_def_file,
  ...
  std::vector<string> cabindb_cfs;
  status = cabindb::DB::ListColumnFamilies(cabindb::DBOptions(opt),
					   path, &cabindb_cfs);
  if (!status.ok()) {
    return -EIO;
  }
  dout(5) << __func__ << " column families from cabindb: " << cabindb_cfs << dendl;

  auto emplace_cf = [&] (const CabinDBStore::ColumnFamily& column,
			 int32_t shard_id,
			 const std::string& shard_name,
			 const cabindb::ColumnFamilyOptions& opt) {
    if (std::find(cabindb_cfs.begin(), cabindb_cfs.end(), shard_name) != cabindb_cfs.end()) {
  ...
  for (auto& column : stored_sharding_def) {
    ...
    status = cabindb::GetColumnFamilyOptionsFromMap(cf_opt, options_map, &cf_opt);
    if (!status.ok()) {
    ...
    if (!block_cache_opt.empty()) {
      ...
      status = cabindb::StringToMap(block_cache_opt, &cache_options_map);
      if (!status.ok()) {
	...
	return -EINVAL;
      ...
      std::string cache_type = cct->_conf->cabindb_cache_type;
	...
	require_new_block_cache = true;
      ...
      size_t cache_size = cct->_conf->cabindb_cache_size;
	...
	require_new_block_cache = true;
      ...
      cabindb::BlockBasedTableOptions column_bbt_opts;
	...
	return -EINVAL;
      ...
      std::shared_ptr<cabindb::Cache> block_cache;
      if (column_bbt_opts.no_block_cache) {
	// clear all settings except no_block_cache
	// cabindb does not like then
	column_bbt_opts = cabindb::BlockBasedTableOptions();
...
std::ostream& operator<<(std::ostream& out, const CabinDBStore::ColumnFamily& cf)
{
  out << "(";
  out << cf.name;
  out << ",";
...
int CabinDBStore::do_open(ostream &out,
			  ...
			  const std::string& sharding_text)
{
  ...
  cabindb::Options opt;
  int r = load_cabindb_options(create_if_missing, opt);
  if (r) {
    dout(1) << __func__ << " load cabindb options failed" << dendl;
    return r;
  }
  cabindb::Status status;
  if (create_if_missing) {
    ...
    if (existing_cfs.empty()) {
      // no column families
      if (open_readonly) {
        status = cabindb::DB::OpenForReadOnly(opt, path, &db);
      } else {
        status = cabindb::DB::Open(opt, path, &db);
      }
      if (!status.ok()) {
	...
	return -EINVAL;
      ...
      if (open_readonly) {
        status = cabindb::DB::OpenForReadOnly(cabindb::DBOptions(opt),
				              path, existing_cfs,
					      &handles, &db);
      } else {
        status = cabindb::DB::Open(cabindb::DBOptions(opt),
  ...
  PerfCountersBuilder plb(cct, "cabindb", l_cabindb_first, l_cabindb_last);
  plb.add_u64_counter(l_cabindb_gets, "get", "Gets");
  plb.add_time_avg(l_cabindb_get_latency, "get_latency", "Get latency");
  plb.add_time_avg(l_cabindb_submit_latency, "submit_latency", "Submit Latency");
  plb.add_time_avg(l_cabindb_submit_sync_latency, "submit_sync_latency", "Submit Sync Latency");
  plb.add_u64_counter(l_cabindb_compact, "compact", "Compactions");
  plb.add_u64_counter(l_cabindb_compact_range, "compact_range", "Compactions by range");
  plb.add_u64_counter(l_cabindb_compact_queue_merge, "compact_queue_merge", "Mergings of ranges in compaction queue");
  plb.add_u64(l_cabindb_compact_queue_len, "compact_queue_len", "Length of compaction queue");
  plb.add_time_avg(l_cabindb_write_wal_time, "cabindb_write_wal_time", "Cabindb write wal time");
  plb.add_time_avg(l_cabindb_write_memtable_time, "cabindb_write_memtable_time", "Cabindb write memtable time");
  plb.add_time_avg(l_cabindb_write_delay_time, "cabindb_write_delay_time", "Cabindb write delay time");
  plb.add_time_avg(l_cabindb_write_pre_and_post_process_time, 
      "cabindb_write_pre_and_post_time", "total time spent on writing a record, excluding write process");
  logger = plb.create_perf_counters();
  cct->get_perfcounters_collection()->add(logger);

  if (compact_on_mount) {
    derr << "Compacting cabindb store..." << dendl;
    compact();
    derr << "Finished compacting cabindb store" << dendl;
  }
  return 0;
}

int CabinDBStore::_test_init(const string& dir)
{
  cabindb::Options options;
  options.create_if_missing = true;
  cabindb::DB *db;
  cabindb::Status status = cabindb::DB::Open(options, dir, &db);
  delete db;
  db = nullptr;
  return status.ok() ? 0 : -EIO;
}

CabinDBStore::~CabinDBStore()
{
  close();
  if (priv) {
    delete static_cast<cabindb::Env*>(priv);
  }
}

void CabinDBStore::close()
{
...
int CabinDBStore::repair(std::ostream &out)
{
  cabindb::Status status;
  cabindb::Options opt;
  int r = load_cabindb_options(false, opt);
  if (r) {
    dout(1) << __func__ << " load cabindb options failed" << dendl;
    out << "load cabindb options failed" << std::endl;
    return r;
  }
  ...
  status = cabindb::RepairDB(path, opt);
  bool repaired = status.ok();
  if (!stored_sharding_text.empty()) {
    //recreate markers even if repair failed
    opt.env->CreateDir(sharding_def_dir);
    status = cabindb::WriteStringToFile(opt.env, stored_sharding_text,
					sharding_def_file, true);
    if (!status.ok()) {
      derr << __func__ << " cannot write to " << sharding_def_file << dendl;
      return -1;
    }
    status = cabindb::WriteStringToFile(opt.env, "1",
  ...
  if (repaired && status.ok()) {
    ...
    out << "repair cabindb failed : " << status.ToString() << std::endl;
    return -1;
  }
}

void CabinDBStore::split_stats(const std::string &s, char delim, std::vector<std::string> &elems) {
    std::stringstream ss;
    ss.str(s);
    std::string item;
...
bool CabinDBStore::get_property(
...
int64_t CabinDBStore::estimate_prefix_size(const string& prefix,
					   const string& key_prefix)
{
  uint64_t size = 0;
  uint8_t flags =
    //cabindb::DB::INCLUDE_MEMTABLES |  // do not include memtables...
    cabindb::DB::INCLUDE_FILES;
...
void CabinDBStore::get_statistics(Formatter *f)
{
  if (!cct->_conf->cabindb_perf)  {
    dout(20) << __func__ << " CabinDB perf is disabled, can't probe for stats"
	     << dendl;
    return;
  }

  if (cct->_conf->cabindb_collect_compaction_stats) {
    std::string stat_str;
    bool status = db->GetProperty("cabindb.stats", &stat_str);
    if (status) {
      f->open_object_section("cabindb_statistics");
      f->dump_string("cabindb_compaction_statistics", "");
      vector<string> stats;
  ...
  if (cct->_conf->cabindb_collect_extended_stats) {
    if (dbstats) {
      f->open_object_section("cabindb_extended_statistics");
      string stat_str = dbstats->ToString();
      vector<string> stats;
      split_stats(stat_str, '\n', stats);
      f->dump_string("cabindb_extended_statistics", "");
      for (auto st :stats) {
        f->dump_string(".", st);
      }
      f->close_section();
    }
    f->open_object_section("cabindbstore_perf_counters");
    logger->dump_formatted(f,0);
    f->close_section();
  }
  if (cct->_conf->cabindb_collect_memory_stats) {
    f->open_object_section("cabindb_memtable_statistics");
    std::string str;
    if (!bbt_opts.no_block_cache) {
    ...
    db->GetProperty("cabindb.cur-size-all-mem-tables", &str);
    f->dump_string("cabindb_memtable_usage", str);
    str.clear();
    db->GetProperty("cabindb.estimate-table-readers-mem", &str);
    f->dump_string("cabindb_index_filter_blocks_usage", str);
    f->close_section();
  }
}

struct CabinDBStore::CabinWBHandler: public cabindb::WriteBatch::Handler {
  CabinWBHandler(const CabinDBStore& db) : db(db) {}
  const CabinDBStore& db;
  std::stringstream seen;
  int num_seen = 0;

  void dump(const char* op_name,
	    uint32_t column_family_id,
	    const cabindb::Slice& key_in,
	    const cabindb::Slice* value = nullptr) {
    string prefix;
    string key;
  ...
  void Put(const cabindb::Slice& key,
	   const cabindb::Slice& value) override {
    dump("Put", 0, key, &value);
  }
  cabindb::Status PutCF(uint32_t column_family_id, const cabindb::Slice& key,
			const cabindb::Slice& value) override {
    dump("PutCF", column_family_id, key, &value);
    return cabindb::Status::OK();
  }
  void SingleDelete(const cabindb::Slice& key) override {
    dump("SingleDelete", 0, key);
  }
  cabindb::Status SingleDeleteCF(uint32_t column_family_id, const cabindb::Slice& key) override {
    dump("SingleDeleteCF", column_family_id, key);
    return cabindb::Status::OK();
  }
  void Delete(const cabindb::Slice& key) override {
    dump("Delete", 0, key);
  }
  cabindb::Status DeleteCF(uint32_t column_family_id, const cabindb::Slice& key) override {
    dump("DeleteCF", column_family_id, key);
    return cabindb::Status::OK();
  }
  void Merge(const cabindb::Slice& key,
	     const cabindb::Slice& value) override {
    dump("Merge", 0, key, &value);
  }
  cabindb::Status MergeCF(uint32_t column_family_id, const cabindb::Slice& key,
			  const cabindb::Slice& value) override {
...
int CabinDBStore::submit_common(cabindb::WriteOptions& woptions, KeyValueDB::Transaction t) 
{
  // enable cabindb breakdown
  // considering performance overhead, default is disabled
  if (cct->_conf->cabindb_perf) {
    cabindb::SetPerfLevel(cabindb::PerfLevel::kEnableTimeExceptForMutex);
    cabindb::get_perf_context()->Reset();
  }

  CabinDBTransactionImpl * _t =
    static_cast<CabinDBTransactionImpl *>(t.get());
  woptions.disableWAL = disableWAL;
  lgeneric_subdout(cct, cabindb, 30) << __func__;
  ...
  *_dout << " Cabindb transaction: " << bat_txc.seen.str() << dendl;
  ...
  if (!s.ok()) {
    CabinWBHandler cabin_txc(*this);
    _t->bat.Iterate(&cabin_txc);
    derr << __func__ << " error: " << s.ToString() << " code = " << s.code()
         << " Cabindb transaction: " << cabin_txc.seen.str() << dendl;
  }
  ...
  if (cct->_conf->cabindb_perf) {
	...
	static_cast<double>(cabindb::get_perf_context()->write_wal_time)/1000000000);
    write_memtable_time.set_from_double(
	static_cast<double>(cabindb::get_perf_context()->write_memtable_time)/1000000000);
    write_delay_time.set_from_double(
	static_cast<double>(cabindb::get_perf_context()->write_delay_time)/1000000000);
    write_pre_and_post_process_time.set_from_double(
	static_cast<double>(cabindb::get_perf_context()->write_pre_and_post_process_time)/1000000000);
    logger->tinc(l_cabindb_write_memtable_time, write_memtable_time);
    logger->tinc(l_cabindb_write_delay_time, write_delay_time);
    logger->tinc(l_cabindb_write_wal_time, write_wal_time);
    logger->tinc(l_cabindb_write_pre_and_post_process_time, write_pre_and_post_process_time);
  }

  return s.ok() ? 0 : -1;
}

int CabinDBStore::submit_transaction(KeyValueDB::Transaction t) 
{
  ...
  logger->tinc(l_cabindb_submit_latency, lat);
  
  return result;
}

int CabinDBStore::submit_transaction_sync(KeyValueDB::Transaction t)
{
  utime_t start = ceph_clock_now();
  cabindb::WriteOptions woptions;
  ...
  utime_t lat = ceph_clock_now() - start;
  logger->tinc(l_cabindb_submit_sync_latency, lat);

  return result;
}

CabinDBStore::CabinDBTransactionImpl::CabinDBTransactionImpl(CabinDBStore *_db)
{
  db = _db;
}

void CabinDBStore::CabinDBTransactionImpl::put_bat(
  ...
  const bufferlist &to_set_bl)
{
  ...
  if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
    bat.Put(cf,
	    cabindb::Slice(key),
	    cabindb::Slice(to_set_bl.buffers().front().c_str(),
			   to_set_bl.length()));
  } else {
    cabindb::Slice key_slice(key);
    vector<cabindb::Slice> value_slices(to_set_bl.get_num_buffers());
    bat.Put(cf,
	    cabindb::SliceParts(&key_slice, 1),
            prepare_sliceparts(to_set_bl, &value_slices));
  }
}

void CabinDBStore::CabinDBTransactionImpl::set(
  const string &prefix,
  const string &k,
  const bufferlist &to_set_bl)
{
...
void CabinDBStore::CabinDBTransactionImpl::rmkey(const string &prefix,
					         const string &k)
{
  auto cf = db->get_cf_handle(prefix, k);
  if (cf) {
    bat.Delete(cf, cabindb::Slice(k));
  } else {
						 ...
						 size_t keylen)
{
  auto cf = db->get_cf_handle(prefix, k, keylen);
  if (cf) {
    bat.Delete(cf, cabindb::Slice(k, keylen));
  } else {
    string key;
    combine_strings(prefix, k, keylen, &key);
    bat.Delete(db->default_cf, cabindb::Slice(key));
  }
}

void CabinDBStore::CabinDBTransactionImpl::rm_single_key(const string &prefix,
					                 const string &k)
{
  auto cf = db->get_cf_handle(prefix, k);
  if (cf) {
...
void CabinDBStore::CabinDBTransactionImpl::rmkeys_by_prefix(const string &prefix)
{
...
void CabinDBStore::CabinDBTransactionImpl::rm_range_keys(const string &prefix,
                                                         const string &start,
                                                         const string &end)
{
  auto p_iter = db->cf_handles.find(prefix);
  if (p_iter == db->cf_handles.end()) {
    ...
    if (cnt == 0) {
      bat.RollbackToSavePoint();
      bat.DeleteRange(db->default_cf,
		      cabindb::Slice(combine_strings(prefix, start)),
		      cabindb::Slice(combine_strings(prefix, end)));
    } else {
      bat.PopSavePoint();
    }
  } else {
    ...
    for (auto cf : p_iter->second.handles) {
      uint64_t cnt = db->delete_range_threshold;
      bat.SetSavePoint();
      cabindb::Iterator* it = db->new_shard_iterator(cf);
	...
	bat.Delete(cf, it->key());
      ...
      if (cnt == 0) {
	bat.RollbackToSavePoint();
	bat.DeleteRange(cf, cabindb::Slice(start), cabindb::Slice(end));
      } else {
	bat.PopSavePoint();
      }
      delete it;
...
void CabinDBStore::CabinDBTransactionImpl::merge(
  const string &prefix,
  const string &k,
  const bufferlist &to_set_bl)
{
  ...
  if (cf) {
	...
	cf,
	cabindb::Slice(k),
	cabindb::Slice(to_set_bl.buffers().front().c_str(), to_set_bl.length()));
    } else {
      // make a copy
      cabindb::Slice key_slice(k);
      vector<cabindb::Slice> value_slices(to_set_bl.get_num_buffers());
      bat.Merge(cf, cabindb::SliceParts(&key_slice, 1),
		prepare_sliceparts(to_set_bl, &value_slices));
    }
  } else {
    string key = combine_strings(prefix, k);
...
int CabinDBStore::get(
    const string &prefix,
    const std::set<string> &keys,
    std::map<string, bufferlist> *out)
{
  cabindb::PinnableSlice value;
  utime_t start = ceph_clock_now();
  if (cf_handles.count(prefix) > 0) {
    for (auto& key : keys) {
      auto cf_handle = get_cf_handle(prefix, key);
      auto status = db->Get(cabindb::ReadOptions(),
			    cf_handle,
			    cabindb::Slice(key),
			    &value);
      if (status.ok()) {
  ...
  utime_t lat = ceph_clock_now() - start;
  logger->inc(l_cabindb_gets);
  logger->tinc(l_cabindb_get_latency, lat);
  return 0;
}
...
int CabinDBStore::split_key(cabindb::Slice in, string *prefix, string *key)
{
  size_t prefix_len = 0;
...
void CabinDBStore::compact()
{
  logger->inc(l_cabindb_compact);
  cabindb::CompactRangeOptions options;
...
void CabinDBStore::compact_thread_entry()
{
  ...
  while (!compact_queue_stop) {
    if (!compact_queue.empty()) {
      ...
      logger->set(l_cabindb_compact_queue_len, compact_queue.size());
      l.unlock();
      logger->inc(l_cabindb_compact_range);
...
void CabinDBStore::compact_range_async(const string& start, const string& end)
{
  ...
  while (p != compact_queue.end()) {
    ...
    if (start <= p->first && p->first <= end) {
      ...
      logger->inc(l_cabindb_compact_queue_merge);
      break;
    }
...
bool CabinDBStore::check_omap_dir(string &omap_dir)
{
  cabindb::Options options;
  options.create_if_missing = true;
  cabindb::DB *db;
  cabindb::Status status = cabindb::DB::Open(options, omap_dir, &db);
  delete db;
  db = nullptr;
  return status.ok();
}

void CabinDBStore::compact_range(const string& start, const string& end)
{
  cabindb::CompactRangeOptions options;
  cabindb::Slice cstart(start);
  cabindb::Slice cend(end);
...
CabinDBStore::CabinDBWholeSpaceIteratorImpl::~CabinDBWholeSpaceIteratorImpl()
{
  delete dbiter;
}
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::seek_to_first()
{
  dbiter->SeekToFirst();
  ceph_assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
}
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::seek_to_first(const string &prefix)
{
  cabindb::Slice slice_prefix(prefix);
  dbiter->Seek(slice_prefix);
  ceph_assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
}
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::seek_to_last()
{
  dbiter->SeekToLast();
  ceph_assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
}
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::seek_to_last(const string &prefix)
{
  string limit = past_prefix(prefix);
  cabindb::Slice slice_limit(limit);
...
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::upper_bound(const string &prefix, const string &after)
{
...
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::lower_bound(const string &prefix, const string &to)
{
  string bound = combine_strings(prefix, to);
  cabindb::Slice slice_bound(bound);
  dbiter->Seek(slice_bound);
  return dbiter->status().ok() ? 0 : -1;
}
bool CabinDBStore::CabinDBWholeSpaceIteratorImpl::valid()
{
  return dbiter->Valid();
}
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::next()
{
  if (valid()) {
    dbiter->Next();
  }
...
int CabinDBStore::CabinDBWholeSpaceIteratorImpl::prev()
{
  if (valid()) {
    dbiter->Prev();
  }
...
string CabinDBStore::CabinDBWholeSpaceIteratorImpl::key()
{
  string out_key;
  split_key(dbiter->key(), 0, &out_key);
  return out_key;
}
pair<string,string> CabinDBStore::CabinDBWholeSpaceIteratorImpl::raw_key()
{
  string prefix, key;
...
bool CabinDBStore::CabinDBWholeSpaceIteratorImpl::raw_key_is_prefixed(const string &prefix) {
  // Look for "prefix\0" right in cabinb::Slice
  cabindb::Slice key = dbiter->key();
...
bufferlist CabinDBStore::CabinDBWholeSpaceIteratorImpl::value()
{
  return to_bufferlist(dbiter->value());
}

size_t CabinDBStore::CabinDBWholeSpaceIteratorImpl::key_size()
{
  return dbiter->key().size();
}

size_t CabinDBStore::CabinDBWholeSpaceIteratorImpl::value_size()
{
  return dbiter->value().size();
}

bufferptr CabinDBStore::CabinDBWholeSpaceIteratorImpl::value_as_ptr()
{
  cabindb::Slice val = dbiter->value();
  return bufferptr(val.data(), val.size());
}

int CabinDBStore::CabinDBWholeSpaceIteratorImpl::status()
{
  return dbiter->status().ok() ? 0 : -1;
}

string CabinDBStore::past_prefix(const string &prefix)
{
  string limit = prefix;
  limit.push_back(1);
  return limit;
...
public:
  WholeMergeIteratorImpl(CabinDBStore* db)
    : db(db)
...
private:
  ...
  public:
    KeyLess(const cabindb::Comparator* comparator) : comparator(comparator) { };

    bool operator()(cabindb::Iterator* a, cabindb::Iterator* b) const
    {
      if (a->Valid()) {
	if (b->Valid()) {
  ...
  KeyLess keyless;
  string prefix;
  std::vector<cabindb::Iterator*> iters;
public:
  explicit ShardMergeIteratorImpl(const CabinDBStore* db,
				  const std::string& prefix,
				  const std::vector<cabindb::ColumnFamilyHandle*>& shards)
    : db(db), keyless(db->comparator), prefix(prefix)
  {
    iters.reserve(shards.size());
    for (auto& s : shards) {
      iters.push_back(db->db->NewIterator(cabindb::ReadOptions(), s));
    }
  }
  ...
  int upper_bound(const string &after) override {
    cabindb::Slice slice_bound(after);
    for (auto& it : iters) {
      it->Seek(slice_bound);
  ...
  // 4. sort
  int prev() override {
    std::vector<cabindb::Iterator*> prev_done;
    //1
    for (auto it: iters) {
      if (it->Valid()) {
	it->Prev();
    ...
    //2,3
    cabindb::Iterator* highest = prev_done[0];
  ...
  bufferptr value_as_ptr() override {
    cabindb::Slice val = iters[0]->value();
    return bufferptr(val.data(), val.size());
  }
  int status() override {
...
KeyValueDB::Iterator CabinDBStore::get_iterator(const std::string& prefix, IteratorOpts opts)
{
  ...
  if (cf_it != cf_handles.end()) {
    if (cf_it->second.handles.size() == 1) {
      return std::make_shared<CFIteratorImpl>(
        prefix,
        db->NewIterator(cabindb::ReadOptions(), cf_it->second.handles[0]));
    } else {
...
cabindb::Iterator* CabinDBStore::new_shard_iterator(cabindb::ColumnFamilyHandle* cf)
{
  return db->NewIterator(cabindb::ReadOptions(), cf);
}

CabinDBStore::WholeSpaceIterator CabinDBStore::get_wholespace_iterator(IteratorOpts opts)
{
  if (cf_handles.size() == 0) {
    cabindb::ReadOptions opt = cabindb::ReadOptions();
    if (opts & ITERATOR_NOCACHE)
      opt.fill_cache=false;
    return std::make_shared<CabinDBWholeSpaceIteratorImpl>(
...
CabinDBStore::WholeSpaceIterator CabinDBStore::get_default_cf_iterator()
{
  return std::make_shared<CabinDBWholeSpaceIteratorImpl>(
    db->NewIterator(cabindb::ReadOptions(), default_cf));
}

int CabinDBStore::prepare_for_reshard(const std::string& new_sharding,
				      CabinDBStore::columns_t& to_process_columns)
{
  //0. lock db from opening
  //1. list existing columns
  //2. apply merge operator to (main + columns) opts
  //3. prepare std::vector<cabindb::ColumnFamilyDescriptor> existing_cfs
  ...
  if (stored_sharding_text.find(resharding_column_lock) == string::npos) {
    ...
    status = cabindb::WriteStringToFile(env, stored_sharding_text,
					sharding_def_file, true);
    if (!status.ok()) {
      derr << __func__ << " cannot write to " << sharding_def_file << dendl;
      return -EIO;
  ...
  status = cabindb::DB::ListColumnFamilies(cabindb::DBOptions(opt), path, &existing_columns);
  if (!status.ok()) {
  ...
  //2. apply merge operator to (main + columns) opts
  //3. prepare std::vector<cabindb::ColumnFamilyDescriptor> cfs_to_open

  std::vector<cabindb::ColumnFamilyDescriptor> cfs_to_open;
  for (const auto& full_name : existing_columns) {
    ...
    status = cabindb::GetColumnFamilyOptionsFromString(cf_opt, options, &cf_opt);
    if (!status.ok()) {
      derr << __func__ << " failure parsing column options: " << options << dendl;
      return -EINVAL;
    }
    if (base_name != cabindb::kDefaultColumnFamilyName)
  ...
  for (size_t i = 0; i < existing_columns.size(); i++) {
    ...
    cabindb::ColumnFamilyHandle *cf = handles[i];
    std::string base_name;
    size_t shard_idx = 0;
    size_t pos = full_name.find('-');
    ...
    if (cabindb::kDefaultColumnFamilyName == base_name) {
      default_cf = handles[i];
      must_close_default_cf = true;
      std::unique_ptr<cabindb::ColumnFamilyHandle, cf_deleter_t> ptr{
        cf, [](cabindb::ColumnFamilyHandle*) {}};
	...
	if (nsd.name == base_name) {
      ...
      std::unique_ptr<cabindb::ColumnFamilyHandle, cf_deleter_t> ptr{
        cf, [this](cabindb::ColumnFamilyHandle* handle) {
...
int CabinDBStore::reshard_cleanup(const CabinDBStore::columns_t& current_columns)
{
  std::vector<std::string> new_sharding_columns;
  ...
  for (auto& [name, handle] : current_columns) {
    ...
    if (found || name == cabindb::kDefaultColumnFamilyName) {
    ...
    std::unique_ptr<cabindb::Iterator> it{
      db->NewIterator(cabindb::ReadOptions(), handle.get())};
    ceph_assert(it);
    it->SeekToFirst();
    ceph_assert(!it->Valid());

    if (cabindb::Status status = db->DropColumnFamily(handle.get()); !status.ok()) {
...
int CabinDBStore::reshard(const std::string& new_sharding, const CabinDBStore::resharding_ctrl* ctrl_in)
{
  ...
  auto flush_batch = [&](cabindb::WriteBatch* batch) {
    dout(10) << "flushing batch, " << keys_in_batch << " keys, for "
             << bytes_in_batch << " bytes" << dendl;
    cabindb::WriteOptions woptions;
    woptions.sync = true;
    cabindb::Status s = db->Write(woptions, batch);
    ceph_assert(s.ok());
    bytes_in_batch = 0;
    keys_in_batch = 0;
    batch->Clear();
  ...
  auto process_column = [&](cabindb::ColumnFamilyHandle* handle,
			    const std::string& fixed_prefix)
  {
    dout(5) << " column=" << (void*)handle << " prefix=" << fixed_prefix << dendl;
    std::unique_ptr<cabindb::Iterator> it{
      db->NewIterator(cabindb::ReadOptions(), handle)};
    ceph_assert(it);

    cabindb::WriteBatch bat;
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
      cabindb::Slice raw_key = it->key();
  ...
  for (auto& [name, handle] : to_process_columns) {
    ...
    if (name == cabindb::kDefaultColumnFamilyName) {
  ...
  env->CreateDir(sharding_def_dir);
  if (auto status = cabindb::WriteStringToFile(env, new_sharding,
					       sharding_def_file, true);
      !status.ok()) {
    derr << __func__ << " cannot write to " << sharding_def_file << dendl;
    return -EIO;
...
bool CabinDBStore::get_sharding(std::string& sharding) {
  cabindb::Status status;
  std::string stored_sharding_text;
  bool result = false;
  sharding.clear();
  ...
  if (status.ok()) {
    status = cabindb::ReadFileToString(env,
				       sharding_def_file,
				       &stored_sharding_text);
    if(status.ok()) {
      result = true;
